...
位置建议：
    Babel_Simulater/dataset_analyse_p0/AI_VECTOR_ROUTER_With_topo/ml_engine_phase2/train_topo_phase2_candidate_v4_groupsplit_cleaned.py

用途：
    Phase 2 Topology Micro-Refiner 的候选式训练脚本（Cleaned edit_history 版本）。
...
输入数据：
    ...
    edit_history 会先经过 phase2_edit_history_cleaner 清理，再根据 before/after 反向还原成“操作前状态”。
...
from phase2_edit_history_cleaner import clean_edit_history
...
class Cfg:
    ...
    # 防止数据泄漏：
    #   "font_char"：同一个字体同一个字符的所有 edit_history 增广样本只会出现在 train 或 val 一边
    #   "font"：整个字体级别划分，更严格，但如果字体数量少会导致 val 不稳定
    group_split_level: str = "font_char"
    ...
    best_name: str = "topo_phase2_candidate_v4_cleaned_best_loss.pth"
    best_f1_name: str = "topo_phase2_candidate_v4_cleaned_best_candidate_f1.pth"
    latest_name: str = "topo_phase2_candidate_v4_cleaned_latest.pth"
...
class CandidateDataset(Dataset):
    ...
    def _load_file(self, fp):
        ...
        font_name = os.path.basename(fp).replace("_topo.json", "")
        ...
        for hex_key, bundle in data.items():
            ...
            for _ in range(self.cfg.final_noop_per_char):
                self.samples.append(self._make_sample(font_name, hex_key, strokes, events, None))

            hist = clean_edit_history(bundle.get("edit_history", []))
            if isinstance(hist, list):
                for op in hist:
                    ...
                    for _ in range(repeat):
                        self.samples.append(self._make_sample(font_name, hex_key, pre, events, label))
    ...
    def _make_sample(self, font_name, hex_key, strokes, events, label):
        n = len(strokes)
        ...
        if self.cfg.group_split_level == "font":
            group_key = str(font_name)
        else:
            group_key = f"{font_name}::{hex_key}"
...
def evaluate(model, loader, device, cfg, rel_ce, bce, huber):
    ...
    for batch in loader:
        ...
        if cm.any():
            all_probs.append(prob[cm].detach().cpu())
            all_true.append(true[cm].detach().cpu().float())
    ...
    # Threshold sweep：在验证集上扫描 accept probability 阈值。
    sweep = []
    best_sweep = {"threshold": 0.5, "precision": prec, "recall": rec, "f1": cand_f1, "tp": tp, "fp": fp, "fn": fn}
    if all_probs:
        probs = torch.cat(all_probs, dim=0)
        truths = torch.cat(all_true, dim=0) > 0.5
        thresholds = [round(x, 2) for x in np.linspace(0.05, 0.95, 19)]
        for th in thresholds:
            pred_th = probs >= th
            tp_th = int((pred_th & truths).sum().item())
            fp_th = int((pred_th & (~truths)).sum().item())
            fn_th = int(((~pred_th) & truths).sum().item())
            p_th = tp_th / max(1, tp_th + fp_th)
            r_th = tp_th / max(1, tp_th + fn_th)
            f_th = 2 * p_th * r_th / max(1e-8, p_th + r_th)
            row = {"threshold": th, "precision": p_th, "recall": r_th, "f1": f_th, "tp": tp_th, "fp": fp_th, "fn": fn_th}
            sweep.append(row)
            if f_th > best_sweep["f1"]:
                best_sweep = row
    ...
    m.update({
        ...
        "best_threshold": float(best_sweep["threshold"]),
        "best_threshold_precision": float(best_sweep["precision"]),
        "best_threshold_recall": float(best_sweep["recall"]),
        "best_threshold_f1": float(best_sweep["f1"]),
        "best_threshold_tp": int(best_sweep["tp"]),
        "best_threshold_fp": int(best_sweep["fp"]),
        "best_threshold_fn": int(best_sweep["fn"]),
    })
    model.train()
    return m
...
def print_metrics(epoch, train_loss, m, lr):
    print(
        ...
        f"candidate@0.50 P/R/F1={m['cand_precision']:.3f}/{m['cand_recall']:.3f}/{m['cand_f1']:.3f} "
        f"(tp={m['cand_tp']} fp={m['cand_fp']} fn={m['cand_fn']}) | "
        f"best_th={m['best_threshold']:.2f} "
        f"P/R/F1={m['best_threshold_precision']:.3f}/{m['best_threshold_recall']:.3f}/{m['best_threshold_f1']:.3f} "
        f"(tp={m['best_threshold_tp']} fp={m['best_threshold_fp']} fn={m['best_threshold_fn']}) | "
        f"delta_abs={m['delta_abs']:.4f} host_t_abs={m['host_t_abs']:.4f}"
    )

    # 只打印最有参考价值的 5 个 threshold，避免日志过长。
    if "threshold_sweep" in m and m["threshold_sweep"]:
        top = sorted(m["threshold_sweep"], key=lambda x: x["f1"], reverse=True)[:5]
        msg = "  threshold top5: " + " | ".join([
...
def split_dataset(ds, cfg):
    """
    Group split，避免同一个字符的 edit_history 增广样本同时进入 train/val。

    默认 cfg.group_split_level="font_char"：
        group_key = 字体名::U+XXXX
    若改成 "font"：
        group_key = 字体名
    """
    n = len(ds)
    if n < 10:
        idxs = list(range(n))
        return Subset(ds, idxs), Subset(ds, idxs), {"train_groups": 1, "val_groups": 1, "note": "too_few_samples"}

    group_to_indices = {}
    for i, s in enumerate(ds.samples):
        g = s.get("group_key", f"sample_{i}")
    ...
    val_group_n = max(1, int(round(len(groups) * cfg.val_ratio)))
    val_groups = set(groups[:val_group_n])
    train_groups = set(groups[val_group_n:])
    ...
    if not train_idx or not val_idx:
        ...
        return Subset(ds, train_idx), Subset(ds, val_idx), {
            ...
            "note": "fallback_random_split_due_empty_side",
        }
    ...
    return Subset(ds, train_idx), Subset(ds, val_idx), {
        ...
        "group_split_level": cfg.group_split_level,
    }
...
def train():
    ...
    print(f"📦 group split train={len(train_set)} val={len(val_set)} | {split_info}")
    ...
    best_f1_path = os.path.join(MODEL_SAVE_DIR, cfg.best_f1_name)
    ...
    for epoch in range(1, cfg.epochs + 1):
        ...
        if epoch == 1 or epoch % 10 == 0 or epoch == cfg.epochs:
            ...
            ckpt = {
                ...
                "epoch": epoch,
                "val_loss": m["loss"],
                "candidate_f1_at_0_5": m["cand_f1"],
                "best_threshold": m["best_threshold"],
                "best_threshold_f1": m["best_threshold_f1"],
                "best_threshold_precision": m["best_threshold_precision"],
                "best_threshold_recall": m["best_threshold_recall"],
            ...
            if m["loss"] < best:
                ...
                print(f"  ✅ best-loss saved: {best_path}")

            if m["best_threshold_f1"] > best_f1:
                best_f1 = m["best_threshold_f1"]
                torch.save(ckpt, best_f1_path)
                print(f"  ✅ best-candidate-f1 saved: {best_f1_path} | th={m['best_threshold']:.2f} f1={best_f1:.4f}")

    print(f"🎉 done. best_loss={best:.4f} best_candidate_f1={best_f1:.4f}")
    print(f"best loss:         {best_path}")
    print(f"best candidate f1: {best_f1_path}")

...
import os, sys, json, math, time, copy, hashlib, traceback, threading
...
# 兜底 Merge 扫描：
#   模型有时只合并一对就停止/转向 Done，导致页面上明显 E2E 链没继续 merge。
#   这里在 Stage1 结束前额外做一次几何保守 sweep，只尝试近端点 pair，仍受 try_merge 的拟合/轮廓护栏约束。
FINAL_MERGE_SWEEP_ENABLE = True
FINAL_MERGE_SWEEP_MAX_CANDIDATES = 24

# 近端点 E2E 且 Bézier 拟合误差很低时，允许更低的 mask_iou。
# 这是为了处理“端点完全相接、视觉上明显该 merge，但 width 回归/抗锯齿导致 iou 轻微低于 0.86”的情况。
MERGE_ADAPTIVE_GUARD_ENABLE = True
MERGE_E2E_EXACT_DIST_MAX = 1.0
MERGE_E2E_LOW_RMS_MAX = 1.2
MERGE_E2E_LOW_MAXERR_MAX = 3.0
MERGE_E2E_RELAXED_MIN_MASK_IOU = 0.82
MERGE_E2E_RELAXED_MAX_AREA_DELTA_RATIO = 0.10

DELETE_MIN_COVERAGE_BY_OTHERS = 0.78
DELETE_MAX_GLYPH_AREA_LOSS = 0.075

# 新增一阶段删除逻辑：
#   若目标曲线中心线被其它曲线按“小宽度线条”覆盖 95% 以上，则允许删除。
#   为了提速，不做完整 mask 渲染，优先用 KDTree 的 centerline distance 覆盖率近似小宽度渲染。
SMALL_WIDTH_LINE_DELETE_ENABLE = True
SMALL_WIDTH_LINE_DELETE_RADIUS = 2.2
SMALL_WIDTH_LINE_DELETE_THRESHOLD = 0.95
SMALL_WIDTH_LINE_DELETE_MIN_POINTS = 16

# 当前页面批量多线程计算。注意：过大可能导致 torch / numpy 内部线程过度竞争。
# 当前页批量计算后端：
#   process = 真正多进程，适合吃满多核 CPU，Windows 下每个 worker 会独立加载模型。
#   thread  = 轻量但受 GIL 限制，主要用于后台不卡 UI。
PAGE_BATCH_BACKEND = "process"  # "process" / "thread"

# 智能读取 CPU 资源：默认最多使用 75% 逻辑核心。
_CPU_COUNT = os.cpu_count() or 4
PAGE_BATCH_WORKERS_MAX = max(1, int(_CPU_COUNT * 0.75))
PAGE_BATCH_WORKERS = max(1, PAGE_BATCH_WORKERS_MAX)

# 多进程 worker 默认强制 CPU，避免多个进程抢同一张 GPU 导致不稳定。
FORCE_CPU_FOR_PAGE_BATCH_PROCESS = True

# 限制单个进程内 torch / OpenMP 线程数，防止 16 个进程 * 每进程 N 线程导致过度抢占。
PER_PROCESS_TORCH_THREADS = 1
PER_PROCESS_INTEROP_THREADS = 1
...
# 智能读取：扫描 ml_engine_phase2 下 raw v4 与 cleaned v4 的 3+3 个 checkpoint，
# 根据 checkpoint 指标 + 文件名策略自动选择更适合自动 cleaner 的版本。
PHASE2_MODEL_FILENAME = "AUTO"
PHASE2_MODEL_REL_DIR = os.path.join("AI_VECTOR_ROUTER_With_topo", "ml_engine_phase2")
PHASE2_FORCE_MODEL_FILENAME = ""  # 需要强制指定时填具体文件名，或设置环境变量 PHASE2_FORCE_MODEL_FILENAME
PHASE2_MODEL_POLICY = "auto_engineering"
PHASE2_CANDIDATE_MODEL_FILENAMES = [
    "topo_phase2_candidate_v4_best_loss.pth",
    "topo_phase2_candidate_v4_best_candidate_f1.pth",
    "topo_phase2_candidate_v4_latest.pth",
    "topo_phase2_candidate_v4_cleaned_best_loss.pth",
    "topo_phase2_candidate_v4_cleaned_best_candidate_f1.pth",
    "topo_phase2_candidate_v4_cleaned_latest.pth",
]
...
class TimingProbe:
    ...
    def as_dict(self):
        return {
            ...
            "steps": [
                {"step": r["step"], "dt_sec": round(r["dt"], 6), "total_sec": round(r["total"], 6)}
                for r in self.rows
            ],
        }

    def print(self, prefix="[Timing]"):
        print(f"{prefix} {self.label} total={self.total():.4f}s")
        for r in self.rows:
            print(f"{prefix}   {r['step']:<38} dt={r['dt']:.4f}s total={r['total']:.4f}s")
...
def small_width_line_coverage(edges: List[Dict[str, Any]], idx: int, radius: float = SMALL_WIDTH_LINE_DELETE_RADIUS) -> Tuple[float, Dict[str, Any]]:
    """
    优化版“小宽度线条覆盖”：
      - target 不带宽度，只取中心线采样点；
      - others 视作小宽度线条，用 centerline distance <= radius 近似小宽度 mask 覆盖；
      - 有 cKDTree 时 O(N log N)，避免逐条 render_mask。
    """
    if idx < 0 or idx >= len(edges):
        return 0.0, {"mode": "small_width_line_coverage", "reason": "bad_idx"}

    target = np.asarray(edges[idx].get("path", []), dtype=np.float32)
    if len(target) < SMALL_WIDTH_LINE_DELETE_MIN_POINTS:
        return 0.0, {
            "mode": "small_width_line_coverage",
            "reason": f"too_few_target_points:{len(target)}",
            "radius": radius,
        }
    ...
    for j, e in enumerate(edges):
        ...
        if len(p) > 0:
            others.append(p)
            other_ids.append(int(e.get("id", j + 1)))
    ...
    allp = np.vstack(others).astype(np.float32)
    if len(allp) == 0:
        return 0.0, {
            "mode": "small_width_line_coverage",
            "reason": "empty_others",
    ...
    if cKDTree is not None:
        tree = cKDTree(allp)
        d, _ = tree.query(target, k=1, workers=-1)
    else:
        # fallback：分块向量化，避免一次性构造过大的 [T,O,2]
        chunks = []
        bs = 256
        for st in range(0, len(target), bs):
            q = target[st:st+bs]
            dd = np.linalg.norm(q[:, None, :] - allp[None, :, :], axis=2)
            chunks.append(np.min(dd, axis=1))
        d = np.concatenate(chunks, axis=0)

    covered = d <= float(radius)
    coverage = float(np.mean(covered)) if len(covered) else 0.0
    return coverage, {
        "mode": "small_width_line_coverage",
        "radius": round(float(radius), 4),
        "threshold": SMALL_WIDTH_LINE_DELETE_THRESHOLD,
        "target_points": int(len(target)),
        "other_points": int(len(allp)),
        "covered_points": int(np.sum(covered)),
        "nearest_dist_p50": round(float(np.percentile(d, 50)), 4),
        "nearest_dist_p90": round(float(np.percentile(d, 90)), 4),
        "nearest_dist_p95": round(float(np.percentile(d, 95)), 4),
...
class Stage1Refiner:
    def __init__(self):
        ...
        if not self.model_path:
            ...
            return
        try:
            force_cpu = os.environ.get("PCG_FORCE_CPU", "0") == "1"
    ...
    def try_merge(self, edges, ia, ib):
        ...
        # 自适应轮廓护栏：
        # 如果两个 stroke 的端点已经完全相接，且合并后的 cubic 拟合误差极低，
        # 那么 mask_iou 轻微低于全局 0.86 通常来自 width 回归/抗锯齿/采样误差。
        # 这类 case 允许使用更宽松但仍安全的阈值。
        effective_min_iou = MERGE_MIN_MASK_IOU
        effective_max_area_delta = MERGE_MAX_AREA_DELTA_RATIO
        relaxed_e2e = (
            MERGE_ADAPTIVE_GUARD_ENABLE
            and ed <= MERGE_E2E_EXACT_DIST_MAX
            and rms <= MERGE_E2E_LOW_RMS_MAX
            and mx <= MERGE_E2E_LOW_MAXERR_MAX
            and area_delta <= MERGE_E2E_RELAXED_MAX_AREA_DELTA_RATIO
        )
        if relaxed_e2e:
            effective_min_iou = MERGE_E2E_RELAXED_MIN_MASK_IOU
            effective_max_area_delta = MERGE_E2E_RELAXED_MAX_AREA_DELTA_RATIO
            rep["merge_guard_mode"] = "relaxed_exact_e2e_low_error"
            rep["effective_min_mask_iou"] = effective_min_iou
            rep["effective_max_area_delta_ratio"] = effective_max_area_delta
        else:
            rep["merge_guard_mode"] = "strict_outline_guard"
            rep["effective_min_mask_iou"] = effective_min_iou
            rep["effective_max_area_delta_ratio"] = effective_max_area_delta

        if iou < effective_min_iou or area_delta > effective_max_area_delta:
            reasons = []
            if iou < effective_min_iou:
                reasons.append(f"mask_iou {iou:.4f} < threshold {effective_min_iou}")
            if area_delta > effective_max_area_delta:
                reasons.append(f"area_delta_ratio {area_delta:.4f} > threshold {effective_max_area_delta}")
    ...
    def try_delete(self, edges, idx):
        ...
        # 新增快速删除规则：中心线被其它小宽度线条覆盖 95% 以上，则直接删除。
        # 这个规则比 full-width mask guard 更便宜，优先执行。
        if SMALL_WIDTH_LINE_DELETE_ENABLE:
            cov, cov_dbg = small_width_line_coverage(edges, idx, radius=SMALL_WIDTH_LINE_DELETE_RADIUS)
            rep["small_width_line_coverage"] = round(float(cov), 4)
            rep["small_width_line_debug"] = cov_dbg
            if cov >= SMALL_WIDTH_LINE_DELETE_THRESHOLD:
                rep["accepted"] = True
                rep["delete_mode"] = "small_width_line_coverage"
                rep["coverage_by_others"] = round(float(cov), 4)
                rep["glyph_area_loss"] = None
                return True, rem, rep
    ...
    def try_final_merge_sweep(self, edges, taboo, locked, ops, rejected):
        """
        Stage1 结束前的最终几何兜底：
          - 不依赖模型 action；
          - 只扫描 endpoint 距离足够近的 pair；
          - 每次只接受一个最安全的 pair，然后重新扫描；
          - 所有实际合并仍走 try_merge 的 Bézier 拟合 + mask_iou + area_delta 护栏。
        """
        if not FINAL_MERGE_SWEEP_ENABLE:
            return edges

        sweep_round = 0
        while len(ops) < MAX_ACCEPTED_OPS and len(edges) > MIN_STROKES_AFTER_CLEAN:
            candidates = self.all_close_merge_candidates(
                edges,
                taboo,
                locked=locked,
                max_candidates=FINAL_MERGE_SWEEP_MAX_CANDIDATES,
            )

            summary = {
                "type": "FinalMergeSweep",
                "round": sweep_round,
                "accepted": False,
                "candidate_count": len(candidates),
                "top_candidates": [],
            ...
            for rank, (dist, i, j, mode, pair) in enumerate(candidates):
                ok, new_edges, rep = self.try_merge(edges, i, j)
                rep["step"] = f"final_merge_sweep_{sweep_round}"
                rep["fallback_rank"] = rank
                rep["fallback_trigger"] = "final_merge_sweep"
                rep["fallback_pair"] = [int(edges[i]["id"]), int(edges[j]["id"])]
                rep["fallback_endpoint_dist"] = round(float(dist), 3)
                rep["fallback_mode"] = mode

                if ok:
                    rep["final_merge_sweep"] = True
                    edges = new_edges
                    ops.append(rep)
                    accepted_this_round = True
                    summary["accepted"] = True
                    summary["accepted_pair"] = rep.get("fallback_pair")
                    break

                taboo.add(pair)
                if len(summary["top_candidates"]) < 8:
                    summary["top_candidates"].append(rep)

            if not accepted_this_round:
                # 没有任何候选通过 guard，把失败原因写进去，方便看是 rms / max / iou / area 哪个卡住。
    ...
    def refine(self, bundle, uid):
        ...
        # 最后一轮兜底：模型没有继续给 Merge 时，仍尝试几何上明显的近端点 merge。
        edges = self.try_final_merge_sweep(edges, taboo, locked, ops, rejected)
...
def _torch_load_checkpoint_meta(path: str):
    """
    读取 checkpoint 元信息。
    PyTorch >=2.6 默认 weights_only=True 会拒绝包含 numpy scalar 的 ckpt，
    所以这里对可信的自训练 checkpoint 使用 weights_only=False。
    """
    if torch is None:
        return None
    try:
        ...
        print(f"[Phase2ModelScan] load failed | {path} | {repr(e)}")
        return None


def _phase2_ckpt_metric(ckpt, key, default=None):
    if not isinstance(ckpt, dict):
        return default
    try:
        v = ckpt.get(key, default)
        if v is None:
            return default
        return float(v)
    except Exception:
...
def _score_phase2_candidate(path: str, ckpt) -> Dict[str, Any]:
    """
    工程选择策略：
      - 自动 cleaner 优先 precision 和稳定性，避免误吸附。
      - cleaned_best_loss 在当前实验里更适合作为自动执行模型，给额外偏置。
      - best_candidate_f1 可作为指标模型，但 host_t/delta 可能更差，所以只给小偏置。
      - latest 默认扣分，避免加载过拟合末轮。
    """
    fn = os.path.basename(path)
    val_loss = _phase2_ckpt_metric(ckpt, "val_loss", 999.0)
    f1 = _phase2_ckpt_metric(ckpt, "best_threshold_f1", None)
    if f1 is None:
        f1 = _phase2_ckpt_metric(ckpt, "candidate_f1_at_0_5", 0.0)
    precision = _phase2_ckpt_metric(ckpt, "best_threshold_precision", 0.0)
    recall = _phase2_ckpt_metric(ckpt, "best_threshold_recall", 0.0)
    th = _phase2_ckpt_metric(ckpt, "best_threshold", PHASE2_MIN_ACCEPT_THRESHOLD)
    epoch = int(_phase2_ckpt_metric(ckpt, "epoch", -1) or -1)

    score = 0.0
    score += 3.0 * float(precision or 0.0)
    score += 1.0 * float(f1 or 0.0)
    score += 0.35 * float(recall or 0.0)
    score -= 0.05 * min(float(val_loss or 999.0), 50.0)

    if "cleaned" in fn:
        score += 0.50
    if fn.endswith("_best_loss.pth"):
        score += 1.00
    if "cleaned_best_loss" in fn:
        score += 2.00
    if "best_candidate_f1" in fn:
        score += 0.30
    if "latest" in fn:
        score -= 2.00

    cfg_d = ckpt.get("config", {}) if isinstance(ckpt, dict) else {}
    return {
        "path": path,
        "filename": fn,
        ...
        "best_threshold": float(th) if th is not None else None,
        "best_threshold_f1": float(f1) if f1 is not None else None,
        "best_threshold_precision": float(precision) if precision is not None else None,
        "best_threshold_recall": float(recall) if recall is not None else None,
        ...
        "hidden_dim": int(cfg_d.get("hidden_dim", -1)) if isinstance(cfg_d, dict) else -1,
    }


def find_phase2_model_candidates(verbose=True) -> List[Dict[str, Any]]:
    roots = [
        os.path.join(DATASET_ANALYSE_DIR, PHASE2_MODEL_REL_DIR),
        os.path.join(TOOL_DIR, "..", "..", "AI_VECTOR_ROUTER_With_topo", "ml_engine_phase2"),
        os.getcwd(),
    ]
    ...
    forced = (PHASE2_FORCE_MODEL_FILENAME or os.environ.get("PHASE2_FORCE_MODEL_FILENAME", "")).strip()
    names = [forced] if forced else list(PHASE2_CANDIDATE_MODEL_FILENAMES)
    ...
    for root in roots:
        for name in names:
            ...
            if verbose:
                print(f"[Phase2ModelPathCheck] exists={exists} | {p}")
            if not exists:
                continue
            ckpt = _torch_load_checkpoint_meta(p)
            if ckpt is None:
                continue
            row = _score_phase2_candidate(p, ckpt)
            row["_ckpt"] = ckpt
            rows.append(row)

    rows.sort(key=lambda x: x.get("score", -1e9), reverse=True)

    if verbose:
        if rows:
            print("[Phase2ModelSelect] candidates ranked:")
            for i, r in enumerate(rows, start=1):
                print(
                    f"[Phase2ModelSelect] #{i} score={r['score']:.4f} | "
                    f"file={r['filename']} | epoch={r['epoch']} | "
                    f"val_loss={r['val_loss']} | th={r['best_threshold']} | "
                    f"P/R/F1={r['best_threshold_precision']}/{r['best_threshold_recall']}/{r['best_threshold_f1']} | "
                    f"hidden={r['hidden_dim']}"
                )
        else:
            print("[Phase2ModelSelect] no usable phase2 checkpoint found.")
    return rows


def find_phase2_pth(verbose=True):
    rows = find_phase2_model_candidates(verbose=verbose)
    if rows:
        return rows[0]["path"]
    return None
...
class Phase2TopoRefiner:
    def __init__(self):
        ...
        self.model_ranked = find_phase2_model_candidates(verbose=True)
        if not self.model_ranked:
            self.status = "no phase2 checkpoint found; phase2 disabled."
            print("[Phase2Refiner]", self.status)
            return

        selected = self.model_ranked[0]
        self.model_selection = {k: v for k, v in selected.items() if k != "_ckpt"}
        self.model_path = selected["path"]

        try:
            force_cpu = os.environ.get("PCG_FORCE_CPU", "0") == "1"
            self.device = torch.device("cpu" if force_cpu else ("cuda" if torch.cuda.is_available() else "cpu"))
            ckpt = selected.get("_ckpt")
            if ckpt is None:
                ckpt = _torch_load_checkpoint_meta(self.model_path)
            ...
            geom_dim = int(ckpt.get("geom_dim", 22)) if isinstance(ckpt, dict) else 22
            self.threshold = max(
                PHASE2_MIN_ACCEPT_THRESHOLD,
                float(ckpt.get("best_threshold", PHASE2_MIN_ACCEPT_THRESHOLD)) if isinstance(ckpt, dict) else PHASE2_MIN_ACCEPT_THRESHOLD
            )
            ...
            self.status = (
                f"loaded pth: {self.model_path} | threshold={self.threshold:.2f} | "
                f"auto_score={selected.get('score', 0):.4f} | "
                f"P/R/F1={selected.get('best_threshold_precision')}/"
                f"{selected.get('best_threshold_recall')}/{selected.get('best_threshold_f1')}"
            )
            print("[Phase2Refiner]", self.status)
        except Exception as e:
            self.model = None
    ...
    def refine(self, bundle, uid):
        timer = TimingProbe(f"Phase2.refine uid={uid}")
        before = copy.deepcopy(bundle)
        timer.mark("copy_before")
        before_topo = compute_topo(before)
        timer.mark("compute_topo_before")
        ...
        timer.mark("copy_strokes")
        candidates = self.build_candidates(strokes)
        timer.mark(f"build_candidates n={len(candidates)}")
        scored = self.predict_candidates(strokes, candidates)
        timer.mark(f"model_predict_candidates n={len(scored)}")

        accepted, rejected = [], []
        guard_time_total = 0.0
        op_apply_time_total = 0.0
        guard_count = 0
        used_points = set()
        ...
        for rank, cand in enumerate(scored):
            ...
            try:
                ...
                _t_op_apply = time.perf_counter()
                cand_bundle = phase2_copy_bundle_with_strokes(bundle, new_strokes)
                op_apply_time_total += time.perf_counter() - _t_op_apply

                _t_guard = time.perf_counter()
                ok, guard_rep = self._guard(before_op_bundle, cand_bundle, float(np.linalg.norm(delta)))
                guard_dt = time.perf_counter() - _t_guard
                guard_time_total += guard_dt
                guard_count += 1
                rep["guard_time_sec"] = round(guard_dt, 6)
        ...
        timer.mark(f"candidate_loop accepted={len(accepted)} rejected={len(rejected)} guard_count={guard_count} guard_total={guard_time_total:.4f}s")
        out = phase2_copy_bundle_with_strokes(bundle, strokes)
        timer.mark("copy_output_bundle")
        topo_after = compute_topo(out)
        timer.mark("compute_topo_after")
        if PRINT_PREVIEW_TIMING:
            timer.print(prefix="[Phase2Timing]")
        ...
        cm["stage2_model_selection"] = self.model_selection
        cm["stage2_threshold"] = self.threshold
        cm["stage2_timing"] = timer.as_dict()
...
# =============================================================================
# 6.5 Multiprocessing page-batch worker
# =============================================================================
...
def _pcg_worker_init(force_cpu=True, torch_threads=1, interop_threads=1):
    """
    每个进程初始化一次模型实例。
    Windows ProcessPoolExecutor 使用 spawn，每个 worker 是独立 Python 解释器。
    """
    global _WORKER_STAGE1, _WORKER_STAGE2, _WORKER_PID
    _WORKER_PID = os.getpid()

    if force_cpu:
        os.environ["PCG_FORCE_CPU"] = "1"

    for k in ["OMP_NUM_THREADS", "MKL_NUM_THREADS", "OPENBLAS_NUM_THREADS", "VECLIB_MAXIMUM_THREADS", "NUMEXPR_NUM_THREADS"]:
        os.environ[k] = str(max(1, int(torch_threads)))
    ...
    _WORKER_STAGE1 = Stage1Refiner()
    _WORKER_STAGE2 = Phase2TopoRefiner()
    print(
        f"[PageProcessWorkerInit] pid={_WORKER_PID} force_cpu={force_cpu} "
        f"torch_threads={torch_threads} stage1={_WORKER_STAGE1.status} stage2={_WORKER_STAGE2.status}",
        flush=True,
    )


def _pcg_worker_compute(uid, original, variant):
    """
    多进程 worker 入口。
    返回 {uid, variant, bundles, timing, pid}。
    bundles 会包含为了当前 variant 计算出的中间结果，主进程据此更新 page_bundle_cache。
    """
    global _WORKER_STAGE1, _WORKER_STAGE2, _WORKER_PID
    if _WORKER_STAGE1 is None or _WORKER_STAGE2 is None:
        _pcg_worker_init(
            force_cpu=FORCE_CPU_FOR_PAGE_BATCH_PROCESS,
            torch_threads=PER_PROCESS_TORCH_THREADS,
            interop_threads=PER_PROCESS_INTEROP_THREADS,
        )

    timer = TimingProbe(f"ProcessWorker uid={uid} variant={variant} pid={os.getpid()}")
    out = {"uid": uid, "variant": variant, "bundles": {}, "pid": os.getpid()}

    if variant == "original":
        timer.mark("original_no_compute")
        out["timing"] = timer.as_dict()
        return out

    stage1 = _WORKER_STAGE1.refine(original, uid)
    out["bundles"]["stage1"] = stage1
    timer.mark("stage1_refine")

    if variant == "stage1":
        out["timing"] = timer.as_dict()
        return out

    stage2 = _WORKER_STAGE2.refine(stage1, uid)
    out["bundles"]["stage2"] = stage2
    timer.mark("stage2_refine")
    ...
    if variant == "recursive":
        class _WorkerRecursiveRunner:
            pass
        runner = _WorkerRecursiveRunner()
        runner.refiner = _WORKER_STAGE1
        runner.phase2_refiner = _WORKER_STAGE2
        recursive = App.recursive_stage1_stage2_clean(
            runner, uid, original, cached_stage1=stage1, cached_stage2=stage2
        )
        out["bundles"]["recursive"] = recursive
        timer.mark("recursive_refine_cached_first_iter")
...
class App:
    def __init__(self, root):
        ...
        self.batch_workers = tk.IntVar(value=PAGE_BATCH_WORKERS)
        self.batch_backend = tk.StringVar(value=PAGE_BATCH_BACKEND)
        self.page_idx = 0
        ...
        self.photos = []
        self.page_preview_variant = "original"
        self.page_bundle_cache = {}   # uid -> {"stage1": bundle, "stage2": bundle, "recursive": bundle}
        self.page_compute_jobs = []
        self.page_executor = None  # 每次批量任务按 GUI workers 动态创建 executor
    ...
    def _ui(self):
        ...
        ttk.Label(top, text="Workers:").pack(side="left", padx=(12,2))
        ttk.Spinbox(top, from_=1, to=PAGE_BATCH_WORKERS_MAX, textvariable=self.batch_workers, width=5).pack(side="left")
        ttk.Label(top, text="Backend:").pack(side="left", padx=(12,2))
        ttk.Combobox(top, textvariable=self.batch_backend, values=["process", "thread"], width=8, state="readonly").pack(side="left")
        ...
        # 当前页批量渲染按钮：只替换示意图，不额外加入拓扑文字。
        render_row = ttk.Frame(self.root); render_row.pack(side="top", fill="x", padx=8, pady=2)
        ttk.Label(render_row, text="当前页批量渲染:").pack(side="left", padx=(0, 6))
        tk.Button(render_row, text="图=一阶段 clean", bg="#d9ead3", command=lambda:self.render_page_variant("stage1")).pack(side="left", padx=3)
        tk.Button(render_row, text="图=一阶段+二阶段 clean", bg="#cfe2f3", command=lambda:self.render_page_variant("stage2")).pack(side="left", padx=3)
        tk.Button(render_row, text="图=递归 clean", bg="#d9d2e9", command=lambda:self.render_page_variant("recursive")).pack(side="left", padx=3)
        tk.Button(render_row, text="还原图=原图", bg="#eeeeee", command=self.restore_page_original_images).pack(side="left", padx=3)

        # 当前页批量保存按钮：和上面的缓存联动，缺缓存时自动先计算。
        save_row = ttk.Frame(self.root); save_row.pack(side="top", fill="x", padx=8, pady=2)
        ttk.Label(save_row, text="当前页批量保存:").pack(side="left", padx=(0, 6))
        tk.Button(save_row, text="保存当前页=原图", bg="#fce5cd", command=lambda:self.save_page_variant("original")).pack(side="left", padx=3)
        tk.Button(save_row, text="保存当前页=一阶段", bg="#f4cccc", command=lambda:self.save_page_variant("stage1")).pack(side="left", padx=3)
        tk.Button(save_row, text="保存当前页=二阶段", bg="#ead1dc", command=lambda:self.save_page_variant("stage2")).pack(side="left", padx=3)
        tk.Button(save_row, text="保存当前页=递归", bg="#fff2cc", command=lambda:self.save_page_variant("recursive")).pack(side="left", padx=3)
    ...
    def current_page_uids(self):
        ...
        return self.pending[start_i:end_i]

    def _bundle_for_page_view(self, uid):
        if self.page_preview_variant == "original":
            return self.good_by_uid[uid]
        return self.page_bundle_cache.get(uid, {}).get(self.page_preview_variant, self.good_by_uid[uid])
    ...
    def render_grid(self):
        ...
        view_name = self.page_preview_variant

        info = ttk.Label(
            self.scroll.inner,
            text=(
                f"Page {self.page_idx + 1}/{total_pages} | showing pending[{start_i}:{end_i}] "
                f"of {len(self.pending)} | grid={rows}x{cols} | image_view={view_name}"
            ),
            foreground="blue",
        )
        ...
        for local_idx, uid in enumerate(visible):
            ...
            b_original = self.good_by_uid[uid]
            b_view = self._bundle_for_page_view(uid)
            ...
            img = render_bundle(b_view, size)
            ph = ImageTk.PhotoImage(img); self.photos.append(ph)
            ttk.Label(frame, image=ph).pack(side="top", padx=4, pady=4)

            # 主界面文字仍按原始 bundle 输出，批量渲染按钮只换示意图，不额外加入拓扑信息。
            topo = compute_topo(b_original)
            gi = glyph_info(b_original)
            cache_flags = ",".join(sorted(self.page_bundle_cache.get(uid, {}).keys()))
            txt = (
                f"{idx:04d} view={view_name} cache=[{cache_flags}]\\n"
                f"outer={self.uid_outer.get(uid,'')}\\n"
                f"hex={gi.get('hex_key', gi.get('unicode_hex',''))}\\n{uid}\\n"
                f"S={topo['stats']['stroke_count']} CC={topo['stats']['connected_components']} "
                f"Cy={topo['stats']['cycle_count']} X={topo['stats']['x_count']}\\n"
                f"{topo['topology_text'][:80]}"
            )
        ...
        self.status.set(
            ...
            f"| Page={self.page_idx + 1}/{total_pages} size={page_size} | ImageView={view_name} "
            f"| Backend={self.batch_backend.get()} Workers={self.batch_workers.get()} | Stage1: {self.refiner.status} | Stage2: {self.phase2_refiner.status}"
        )

    def _compute_uid_variant(self, uid, variant):
        """
        Worker thread 使用：只计算 bundle，不碰 Tk UI。
        会暂存到 self.page_bundle_cache，后续渲染/保存复用。
        """
        ...
        with self.cache_lock:
            cache = self.page_bundle_cache.setdefault(uid, {})
            if variant in cache:
                return cache[variant]

        # Stage1
        with self.cache_lock:
            stage1_cached = self.page_bundle_cache.setdefault(uid, {}).get("stage1")
        if stage1_cached is None:
            stage1 = self.refiner.refine(original, uid)
            with self.cache_lock:
                self.page_bundle_cache.setdefault(uid, {})["stage1"] = stage1
        else:
            stage1 = stage1_cached
        ...
        # Stage2
        with self.cache_lock:
            stage2_cached = self.page_bundle_cache.setdefault(uid, {}).get("stage2")
        if stage2_cached is None:
            stage2 = self.phase2_refiner.refine(stage1, uid)
            with self.cache_lock:
                self.page_bundle_cache.setdefault(uid, {})["stage2"] = stage2
        else:
            stage2 = stage2_cached
        ...
        # Recursive
        with self.cache_lock:
            rec_cached = self.page_bundle_cache.setdefault(uid, {}).get("recursive")
        if rec_cached is None:
            recursive = self.recursive_stage1_stage2_clean(uid, original, cached_stage1=stage1, cached_stage2=stage2)
            with self.cache_lock:
                self.page_bundle_cache.setdefault(uid, {})["recursive"] = recursive
        else:
            recursive = rec_cached
        return recursive

    def _apply_process_worker_result(self, result):
        uid = result.get("uid")
        ...
        with self.cache_lock:
            cache = self.page_bundle_cache.setdefault(uid, {})
            for k in ("stage1", "stage2", "recursive"):
                if k in bundles and bundles[k] is not None:
                    cache[k] = bundles[k]
        if PRINT_PREVIEW_TIMING and result.get("timing"):
            t = result["timing"]
            print(
                f"[PageProcessResult] uid={uid} variant={result.get('variant')} "
                f"pid={result.get('pid')} total={t.get('total_sec')}s",
                flush=True,
            )
            for row in t.get("steps", []):
                print(
                    f"[PageProcessResult]   {row.get('step'):<38} "
                    f"dt={row.get('dt_sec')}s total={row.get('total_sec')}s",
                    flush=True,
                )

    def _start_page_variant_job(self, variant, reason="render", after_done=None):
        uids = list(self.current_page_uids())
        if not uids:
            messagebox.showinfo("No page items", "当前页没有待处理字体。")
            return

        if self.page_compute_jobs and any(not f.done() for f in self.page_compute_jobs):
            messagebox.showwarning("Busy", "当前页已有批量计算任务正在运行。")
            return

        workers = max(1, min(int(self.batch_workers.get()), PAGE_BATCH_WORKERS_MAX, len(uids)))
        backend = (self.batch_backend.get() or PAGE_BATCH_BACKEND).strip().lower()
        if backend not in ("process", "thread"):
            backend = "process"

        timer = TimingProbe(
            f"PageBatch {reason} variant={variant} page={self.page_idx+1} n={len(uids)} backend={backend} workers={workers}"
        )
        ...
        if backend == "process":
            ...
            executor_kwargs = dict(
                max_workers=workers,
                initializer=_pcg_worker_init,
                initargs=(FORCE_CPU_FOR_PAGE_BATCH_PROCESS, PER_PROCESS_TORCH_THREADS, PER_PROCESS_INTEROP_THREADS),
            )
            if ctx is not None:
            ...
            for uid in uids:
                original = self.good_by_uid[uid]
                futures.append(executor.submit(_pcg_worker_compute, uid, original, variant))

        else:
            executor = ThreadPoolExecutor(max_workers=workers)
            for uid in uids:
                futures.append(executor.submit(self._compute_uid_variant, uid, variant))

        self.page_compute_jobs = futures
        self.status.set(f"当前页 {variant} 批量{reason}计算中：0/{len(futures)} | backend={backend} workers={workers}")

        def poll():
            done = sum(1 for f in futures if f.done())
            self.status.set(f"当前页 {variant} 批量{reason}计算中：{done}/{len(futures)} | backend={backend} workers={workers}")
            if done < len(futures):
                self.root.after(200 if backend == "process" else 120, poll)
                return

            errors = []
            for uid, fut in zip(uids, futures):
                try:
                    res = fut.result()
                    if backend == "process":
                        self._apply_process_worker_result(res)
                except Exception as e:
            ...
            timer.mark(f"all_futures_done errors={len(errors)} backend={backend} workers={workers}")
            if PRINT_PREVIEW_TIMING:
                timer.print(prefix="[PageBatchTiming]")

            if errors:
                msg = "\\n".join([f"{u}: {e}" for u, e in errors[:8]])
                messagebox.showerror("Page batch failed", f"部分字体计算失败：{len(errors)}\\n\\n{msg}")
                self.status.set(f"当前页 {variant} 计算失败 {len(errors)} 个 | backend={backend}")
                return

            if after_done is not None:
                after_done(uids)
            else:
                self.status.set(f"当前页 {variant} 计算完成。backend={backend} workers={workers}")

        self.root.after(200 if backend == "process" else 120, poll)

    def render_page_variant(self, variant):
        def done(_uids):
            self.page_preview_variant = variant
            self.render_grid()
            self.status.set(f"当前页示意图已替换为：{variant}。结果已暂存，可直接批量保存。")
        self._start_page_variant_job(variant, reason="render", after_done=done)

    def restore_page_original_images(self):
        self.page_preview_variant = "original"
        self.render_grid()
        self.status.set("当前页示意图已还原为原图。暂存结果未删除。")

    def _save_page_variant_after_cache(self, variant, uids):
        missing = []
        for uid in uids:
            if variant == "original":
                continue
            if variant not in self.page_bundle_cache.get(uid, {}):
                missing.append(uid)
        if missing:
            messagebox.showerror("Missing cache", f"仍有 {len(missing)} 个样本缺少 {variant} 缓存，保存中止。")
            return

        save_timer = TimingProbe(f"SavePageVariant variant={variant} n={len(uids)}")
        for uid in uids:
            if variant == "original":
                bundle = self.good_by_uid[uid]
            else:
                bundle = self.page_bundle_cache[uid][variant]
            self.cleaned[uid] = self._prepare_cleaned_bundle(uid, bundle, variant)
        save_timer.mark("prepare_cleaned_bundles_from_cache")

        self.write_cleaned(False)
        save_timer.mark("write_cleaned_pool")
        if PRINT_PREVIEW_TIMING:
            save_timer.print(prefix="[PageBatchSaveTiming]")
        self.refresh()
        messagebox.showinfo(
            "Batch Save Done",
            f"已将当前页 {len(uids)} 个字体保存为 cleaned。\\n保存版本：{variant}\\n原 good 未删除/未覆盖。"
        )

    def save_page_variant(self, variant):
        uids = list(self.current_page_uids())
        if not uids:
            messagebox.showinfo("No page items", "当前页没有待处理字体。")
            return
        ...
        if not messagebox.askyesno(
            "确认批量保存",
            f"将当前页 {len(uids)} 个字体以 [{variant}] 版本写入 cleaned？\\n\\n"
            "这不会删除/覆盖 good；但这些 uid 会进入 cleaned，刷新后将不再出现在 pending 中。"
        ):
            return

        if variant == "original":
            self._save_page_variant_after_cache(variant, uids)
            return

        missing = [u for u in uids if variant not in self.page_bundle_cache.get(u, {})]
        if not missing:
            # 关键修复：
            # 当前页对应阶段已经由“批量渲染”或单样本 preview 暂存过，
            # 直接保存缓存，不再重新启动 process/thread 计算。
            if PRINT_PREVIEW_TIMING:
                print(f"[PageBatchSave] cache_hit_direct_save variant={variant} n={len(uids)}", flush=True)
            self._save_page_variant_after_cache(variant, uids)
            return

        if not messagebox.askyesno(
            "需要先计算",
            f"当前页有 {len(missing)} 个字体还没有 [{variant}] 暂存结果。\\n\\n是否先多进程/多线程计算，再保存？"
        ):
            return

        self._start_page_variant_job(
            variant,
            reason="save",
            after_done=lambda _uids: self._save_page_variant_after_cache(variant, uids)
        )
    ...
    def recursive_stage1_stage2_clean(self, uid, original, cached_stage1=None, cached_stage2=None):
        """
        递归清理：
        ...
        加速版本：
            如果 preview_uid 已经算出了第一轮 stage1 / stage2，
            这里直接复用这两个结果作为 recursive iter 1，
            不再从 original 重跑一遍 Stage1 + Stage2。
        """
        total_timer = TimingProbe(f"RecursiveClean uid={uid}")
        cur = copy.deepcopy(original)
        total_timer.mark("copy_original")
        ...
        # ------------------------------------------------------------
        # 复用 preview 阶段已经算过的 Stage1/Stage2，避免重复计算第一轮。
        # ------------------------------------------------------------
        if cached_stage1 is not None and cached_stage2 is not None:
            iter_timer = TimingProbe("recursive_iter_1_cached_from_preview")
            before_iter = copy.deepcopy(cur)
            iter_timer.mark("copy_before_iter")

            s1 = cached_stage1
            iter_timer.mark("stage1_refine_reused_from_preview")
            s1_meta = s1.get("clean_meta", {}) if isinstance(s1, dict) else {}
            s1_changed = bool(s1_meta.get("changed", False))

            s2 = cached_stage2
            iter_timer.mark("stage2_refine_reused_from_preview")
            ...
            iter_timer.mark("compute_topo_before_iter")
            iter_topo_after = compute_topo(s2)
            iter_timer.mark("compute_topo_after_iter")

            if PRINT_PREVIEW_TIMING:
                iter_timer.print(prefix="[RecursiveTiming]")

            iterations.append({
                "iter": 1,
                "cached_from_preview": True,
        ...
        # ------------------------------------------------------------
        # 如果第一轮没有收敛，才从 stage2 结果继续递归。
        # 注意：这里从 iter=2 开始，不再重复 iter=1。
        # ------------------------------------------------------------
        if stopped_reason != "converged_no_stage1_or_stage2_change":
            for it in range(start_iter, RECURSIVE_CLEAN_MAX_ITERS + 1):
                before_iter = copy.deepcopy(cur)
                iter_timer = TimingProbe(f"recursive_iter_{it}")
                iter_timer.mark("copy_before_iter")

                s1 = self.refiner.refine(before_iter, uid)
                iter_timer.mark("stage1_refine")
                s1_meta = s1.get("clean_meta", {}) if isinstance(s1, dict) else {}
                s1_changed = bool(s1_meta.get("changed", False))

                s2 = self.phase2_refiner.refine(s1, uid)
                iter_timer.mark("stage2_refine")
                s2_meta = s2.get("clean_meta", {}) if isinstance(s2, dict) else {}
                s2_changed = bool(s2_meta.get("stage2_changed", False))
        ...
        total_timer.mark(f"recursive_loop_done iters={len(iterations)} reason={stopped_reason}")
        out = copy.deepcopy(cur)
        total_timer.mark("copy_recursive_output")
        topo_final = compute_topo(out)
        total_timer.mark("compute_topo_final")

        if PRINT_PREVIEW_TIMING:
            total_timer.print(prefix="[RecursiveTiming]")
        ...
        cm["recursive_timing"] = total_timer.as_dict()
        cm["recursive_used_preview_cache"] = bool(cached_stage1 is not None and cached_stage2 is not None)
        ...
        if isinstance(out["edit_history"], list):
            out["edit_history"].append({
                ...
                "used_preview_cache": bool(cached_stage1 is not None and cached_stage2 is not None),
                "auto": True,
    ...
    def preview_uid(self, uid):
        ...
        timer = TimingProbe(f"Preview uid={uid}")
        try:
            original = self.good_by_uid[uid]
            timer.mark("fetch_original_from_memory")
            ...
            self.before_stage1_stage2_recursive(uid, original, stage1, stage2, recursive, preview_timing=timer.as_dict())
            timer.mark("build_preview_window")
            if PRINT_PREVIEW_TIMING:
                timer.print(prefix="[PreviewTiming]")
        except Exception as e:
            traceback.print_exc()
    ...
    def _panel_text(self, title, bundle):
        ...
        if title.startswith("一阶段"):
            txt += "\\nStage1 Summary:\\n" + json.dumps({
                "changed": cm.get("changed"),
                "original_stroke_count": cm.get("original_stroke_count"),
                "cleaned_stroke_count": cm.get("cleaned_stroke_count"),
                "accepted_count": len(cm.get("accepted_ops", [])),
                "rejected_count": len(cm.get("rejected_ops", [])),
            ...
            if cm.get("stage2_model_selection"):
                txt += "\\n\\nStage2 Model Selection:\\n" + json.dumps(cm.get("stage2_model_selection"), ensure_ascii=False, indent=2)
            if cm.get("stage2_timing"):
                txt += "\\n\\nStage2 Timing:\\n" + json.dumps(cm.get("stage2_timing"), ensure_ascii=False, indent=2)
            ...
            if cm.get("recursive_timing"):
                txt += "\\n\\nRecursive Timing:\\n" + json.dumps(cm.get("recursive_timing"), ensure_ascii=False, indent=2)
            txt += f"\\n\\nStage2 Model={cm.get('stage2_model_status')}"
        return txt

    def before_stage1_stage2_recursive(self, uid, original, stage1, stage2, recursive, preview_timing=None):
        ...
        if preview_timing:
            ttk.Label(top, text=f"PreviewTiming={preview_timing.get('total_sec', 0.0):.3f}s", foreground="blue").pack(side="left", padx=12)

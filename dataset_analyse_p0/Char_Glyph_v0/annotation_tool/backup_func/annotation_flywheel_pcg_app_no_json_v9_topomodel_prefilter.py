...
# Topo model runtime defaults.
# 默认读取你当前 flywheel_model/model 里的 ckptselect 索引。
DEFAULT_TOPO_MODEL_INDEX = os.path.join(
    CHAR_GLYPH_DIR,
    "flywheel_model",
    "model",
    "flywheel_topohgt_v6_relationcalib_ckptselect_checkpoint_index.json",
)
DEFAULT_TOPO_MODEL_PURPOSE = "relation_overall"
TOPO_MODEL_PURPOSES = [
    "relation_overall",
    "relation_t",
    "relation_x",
...
DEFAULT_TOPO_MODEL_SCORE_MODE = "quality_best_f1"
DEFAULT_TOPO_MODEL_MANUAL_SCORE_THRESHOLD = 0.30
...
def candidate_has_model_topology(candidate: Dict[str, Any]) -> bool:
    """
    True 表示这个 candidate 的 topology_events/cycles 已经由 topo 模型生成。
    Commit 保存时不能再被第二阶段规则引擎覆盖。
    """
...
def _stroke_bundle_for_topo_model(candidate: Dict[str, Any]) -> Dict[str, Any]:
    """
    只构造模型输入所需的 strokes/glyph_info。
    topology_events 不参与输入特征，所以这里保持空，避免规则拓扑泄漏到模型输出。
    """
...
class TopoModelRuntime:
    """
    读取 flywheel topo 模型并用模型输出生成 topology_events/cycles。

    当前采用“程序生成 stroke 不变 + topo 模型打分生成拓扑”的方式：
      1. PCG 仍然按原逻辑生成笔画；
      2. v6 topo 模型对 E2E/T/X 三个 binary head 打分；
      3. 使用 checkpoint_index 里记录的 relation 阈值生成 topology_events；
      4. 后续预览、复杂度过滤、commit 保存流程不变。
    """
    ...
    def load(self):
        if not os.path.exists(self.index_path):
            raise FileNotFoundError(
                ...
                f"{self.index_path}\\n\\n"
                "请先运行 train_flywheel_topohgt_v6_relationcalib_ckptselect.py，"
        ...
        if not os.path.isabs(ckpt_path):
            ckpt_path = os.path.abspath(os.path.join(os.path.dirname(self.index_path), ckpt_path))
    ...
    def attach_model_topology(self, candidate: Dict[str, Any]) -> Dict[str, Any]:
        ...
        topo = self.decode_topology(pred)

        c["topology_events"] = topo["topology_events"]
        c["cycles"] = topo["cycles"]
        c.setdefault("pcg_meta", {})
        c["pcg_meta"]["stage2_topology_text"] = topo["topology_text"]
        c["pcg_meta"]["stage2_topology_stats"] = topo["stats"]
        c["pcg_meta"]["topology_source"] = "topo_model_v6"
...
class AnnotationFlywheelPCGNoJSONApp:
    def __init__(self, root):
        ...
        self.topo_model_status_var = tk.StringVar(value="Topo model: OFF")
    ...
    def _build_ui(self):
        ...
        # 显眼的 topo 模型开关：勾选后生成阶段会读取 topo 模型，
        # topology_events / cycles 由模型输出生成，后续笔画/保存流程不变。
        row_model = ttk.LabelFrame(top, text="🔥 Topo Model Control - 模型生成拓扑事件")
        row_model.pack(fill="x", pady=6)
        ttk.Checkbutton(
            row_model,
            text="启用 topo 模型生成 topology_events / cycles",
            variable=self.use_topo_model_var,
            command=self.on_toggle_topo_model,
        ).pack(side="left", padx=8)
        ttk.Label(row_model, text="Index:").pack(side="left", padx=(10, 2))
        ttk.Entry(row_model, textvariable=self.topo_model_index_var, width=78).pack(side="left", padx=2)
        ttk.Button(row_model, text="Browse", command=self.browse_topo_model_index).pack(side="left", padx=2)
        ttk.Label(row_model, text="Purpose:").pack(side="left", padx=(10, 2))
        ttk.Combobox(
            row_model,
        ...
        ttk.Label(row_model, text="Score:").pack(side="left", padx=(10, 2))
        ttk.Combobox(
            row_model,
        ...
        ttk.Label(row_model, text="Manual th:").pack(side="left", padx=(8, 2))
        ttk.Spinbox(
            row_model,
            from_=0.0,
            to=1.0,
        ...
        ttk.Button(row_model, text="Load Topo Model", command=self.load_topo_model_now).pack(side="left", padx=6)
        ttk.Label(row_model, textvariable=self.topo_model_status_var, foreground="purple").pack(side="left", padx=8)
    ...
    def on_toggle_topo_model(self):
        if self.use_topo_model_var.get():
            ...
            self.topo_model_status_var.set("Topo model: OFF")
            self.topo_model_runtime = None
    ...
    def load_topo_model_now(self):
        try:
            ...
            self.topo_model_runtime = None
            self.topo_model_status_var.set("Topo model: LOAD FAILED")
            messagebox.showerror("Topo Model Load Failed", traceback.format_exc())
    ...
    def passes_topo_model_score(self, candidate: Dict[str, Any]) -> Tuple[bool, str]:
        ...
        if not ok:
            return False, "topo_model_score_below_threshold"
        return True, "ok"
    ...
    def attach_topology_for_generation(self, candidate: Dict[str, Any], cfg: GeneratorConfig) -> Dict[str, Any]:
        ...
        if self.topo_model_runtime is None or not self.topo_model_runtime.is_loaded():
            raise RuntimeError("Topo model is enabled but not loaded.")
    ...
    def generate_batch(self):
        ...
        while len(selected) < max_items and attempts < max_attempts:
            ...
            for _key, _lo, _hi in [
                ...
                # 2) 通过人为拓扑配置后，再生成最终 topology 信息。
                try:
                    cand = self.attach_topology_for_generation(cand, cfg)
                except Exception:
                    self.status_var.set("Topo model topology generation failed.")
                    messagebox.showerror("Topo Model Topology Failed", traceback.format_exc())
                    return

                # 3) topo 模型模式下，用 quality_score 过滤候选。
                #    只有模型认为达到阈值的样本才进入候选池。
                if self.use_topo_model_var.get():
        ...
        topo_src = "topo_model" if self.use_topo_model_var.get() else "stage2_rule_engine"
        if self.use_topo_model_var.get():
            th, mode = self.get_topo_model_score_threshold()
            filter_src = f"human_config_then_model_score({mode}, th={th})"
        else:
            filter_src = "human_config_then_stage2_rule_engine"
        self.status_var.set(
            f"Generated {len(selected)}/{max_items} direct procedural candidates. "
            f"topology={topo_src}. filter={filter_src}. attempts={attempts}, reject={dict(reject)}"
        )
    ...
    def render_current_grid(self):
        ...
        for idx, cand in enumerate(self.current_batch):
            ...
            topo_badge = "MODEL" if candidate_has_model_topology(cand) else "RULE"
            qtxt = ""
            ...
            filter_txt = ""
            if isinstance(filter_comp, dict) and candidate_has_model_topology(cand):

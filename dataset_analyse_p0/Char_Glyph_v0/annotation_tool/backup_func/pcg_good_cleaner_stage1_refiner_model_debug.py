# -*- coding: utf-8 -*-
r"""
pcg_good_cleaner_stage1_refiner_model_debug.py
...
def find_pth(verbose: bool = False):
    # 脚本位置：
    #   dataset_analyse_p0/Char_Glyph_v0/annotation_tool/pcg_good_cleaner_stage1_refiner_model_debug.py
    # 模型位置：
...
class Stage1Refiner:
    def __init__(self):
        self.env = EnvLocal()
        print(f"[PathDebug] TOOL_DIR={TOOL_DIR}")
        print(f"[PathDebug] CHAR_GLYPH_DIR={CHAR_GLYPH_DIR}")
        print(f"[PathDebug] DATASET_ANALYSE_DIR={DATASET_ANALYSE_DIR}")
        self.model_path = find_pth(verbose=True)
        print(f"[PathDebug] selected_model_path={self.model_path}")
        ...
        try:
            print(f"[Stage1Refiner] torch={torch.__version__}")
        except Exception:
            print("[Stage1Refiner] torch imported, version unknown")

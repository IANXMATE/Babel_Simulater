...
DEFAULT_MIN_HUMAN_LINE_LENGTH = 78.0
...
OUTLINE_FAMILIES = {"outline_box", "semi_enclosure", "box", "hanzi_block"}
LAYOUT_FAMILIES = {"left_right_layout", "top_bottom_layout", "center_spine_branches", "hanzi_block"}
...
def generate_anchor_edges_for_family(
    ...
    if family in ("vertical_column", "parallel_verticals"):
...
def candidate_min_split_fragment_ratio(candidate: Dict[str, Any]) -> Tuple[float, Dict[str, Any]]:
    """检查线条被 X/T 切分后的最短片段比例；默认要求 >= 1/6。"""
    nodes = _get_nodes(candidate)
    if not nodes:
        return 1.0, {"ok": True, "reason": "no_nodes"}
    by_sid = {}
    ...
    def add_t(sid, t):
        try:
            ...
            return
        if sid in by_sid and 0.02 < t < 0.98:
            by_sid[sid].append(t)

    for ev in candidate.get("topology_events", []) or []:
        if not isinstance(ev, dict):
            continue
        typ = ev.get("type")
        if typ == "X":
            add_t(ev.get("stroke_a"), ev.get("t_a"))
            add_t(ev.get("stroke_b"), ev.get("t_b"))
        elif typ == "T":
            add_t(ev.get("host"), ev.get("host_t"))
            add_t(ev.get("guest"), ev.get("guest_t"))

    worst_ratio = 1.0
    worst = None
    for sid, ts in by_sid.items():
        vals = sorted(set(round(float(t), 4) for t in ts if -1e-6 <= float(t) <= 1.0 + 1e-6))
        if len(vals) <= 2:
            continue
        gaps = [vals[i + 1] - vals[i] for i in range(len(vals) - 1)]
        if gaps and min(gaps) < worst_ratio:
            worst_ratio = float(min(gaps))
            worst = {"stroke_id": sid, "t_values": vals, "gaps": [round(float(x), 4) for x in gaps]}
    return worst_ratio, {"worst": worst}
...
def passes_min_split_fragment_filter(candidate: Dict[str, Any], cfg: GeneratorConfig) -> Tuple[bool, str]:
    ...
    ratio, info = candidate_min_split_fragment_ratio(candidate)
    ...
    if ratio < threshold:
        return False, "split_fragment_too_short"
    return True, "split_fragment_ok"
...
class AnnotationFlywheelPCGNoJSONApp:
    def __init__(self, root):
        ...
        self.use_axis_orthogonal_var = tk.BooleanVar(value=DEFAULT_USE_AXIS_ORTHOGONAL_GENERATION)
        self.use_grid_snap_var = tk.BooleanVar(value=DEFAULT_USE_GRID_SNAP_GENERATION)
        self.use_outline_bias_var = tk.BooleanVar(value=DEFAULT_USE_OUTLINE_GENERATION_BIAS)
        self.use_human_layout_bias_var = tk.BooleanVar(value=DEFAULT_USE_HUMAN_LAYOUT_BIAS)
        self.axis_snap_prob_var = tk.StringVar(value=str(DEFAULT_AXIS_SNAP_PROB))
        self.grid_snap_prob_var = tk.StringVar(value=str(DEFAULT_GRID_SNAP_PROB))
        self.axis_angle_jitter_var = tk.StringVar(value=str(DEFAULT_AXIS_ANGLE_JITTER_DEG))
        self.axis_weight_mult_var = tk.StringVar(value=str(DEFAULT_AXIS_WEIGHT_MULT))
        self.outline_weight_mult_var = tk.StringVar(value=str(DEFAULT_OUTLINE_WEIGHT_MULT))
        self.layout_weight_mult_var = tk.StringVar(value=str(DEFAULT_LAYOUT_WEIGHT_MULT))
        self.axis_grid_step_var = tk.StringVar(value=str(DEFAULT_AXIS_GRID_STEP))
        self.min_human_line_length_var = tk.StringVar(value=str(DEFAULT_MIN_HUMAN_LINE_LENGTH))
        self.forbid_near_axis_slant_deg_var = tk.StringVar(value=str(DEFAULT_FORBID_NEAR_AXIS_SLANT_DEG))
        self.use_short_split_filter_var = tk.BooleanVar(value=DEFAULT_USE_SHORT_SPLIT_FILTER)
        self.min_split_fragment_frac_var = tk.StringVar(value=str(round(DEFAULT_MIN_SPLIT_FRAGMENT_FRAC, 4)))
        ...
        self.use_novelty_filter_var = tk.BooleanVar(value=DEFAULT_USE_NOVELTY_FILTER)
        ...
        self.use_morpheme_diffusion_var = tk.BooleanVar(value=DEFAULT_USE_MORPHEME_DIFFUSION)
        self.use_marked_library_shift_var = tk.BooleanVar(value=DEFAULT_USE_MARKED_LIBRARY_SHIFT)
        self.use_discovered_rule_influence_var = tk.BooleanVar(value=DEFAULT_USE_DISCOVERED_RULE_INFLUENCE)
    ...
    def _build_ui(self):
        ...
        ttk.Checkbutton(row4b, text="短碎片过滤≥1/6", variable=self.use_short_split_filter_var).pack(side="left", padx=6)

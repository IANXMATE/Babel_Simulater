# -*- coding: utf-8 -*-
r"""
annotation_flywheel_pcg_app_no_json_v8_fast_x_stable_local_pool_incremental_commit_compact.py
...
def candidate_to_stage2_bundle_direct(candidate: Dict[str, Any], label: str, hex_key: str) -> Tuple[str, Dict[str, Any]]:
    ...
    """
    c = attach_stage2_topology(copy.deepcopy(candidate))
...
def candidate_complexity(candidate: Dict[str, Any]) -> Dict[str, Any]:
    ...
    也就是说 GUI 中的 CC / Cy 与人工二阶段 debug 文本/落盘 topology_events 采用同一套逻辑。
    """
    try:
    ...
    if compact_event:
        ...
        if isinstance(bundle["edit_history"], list):
            bundle["edit_history"].append({
                ...
                "label": label,
                "timestamp": time.strftime("%Y-%m-%d %H:%M:%S"),
                "source": "annotation_flywheel_pcg_app_no_json_v8_fast_x_stable_local_pool_incremental_commit_compact.py",
            })
...
def compact_reassign_pool(out_root: str, max_json_mb: int = 80) -> Dict[str, Any]:
    """
    Compact Pool:
    ...
    for label, src_dict, dst_dict in [
        ...
        for old_key, bundle in sorted(src_dict.items(), key=lambda kv: str(kv[0])):
            ...
            b2 = set_bundle_codepoint(b2, new_hex, label=label, old_key=old_key, compact_event=True)
            dst_dict[new_hex] = b2
...
class AnnotationFlywheelPCGNoJSONApp:
    ...
    def compact_pool(self):
        ...
        ok = messagebox.askyesno(
            "Compact Pool",
            "Compact Pool 会读取全部 Good/Bad 分片，统一重分配字符编码，"
            "并把增量分片合并成少量大 JSON 文件。\n\n"
            "它不会改变 strokes/topology_events/cycles 的结构，"
            "但会更新 glyph_info.hex_key / unicode_hex / char，并重写 good/bad pool 文件。\n\n"
            "建议确认没有其他窗口正在 Commit。\n\n"
            "是否继续？"
        )
        if not ok:
        ...
        try:
            self.status_var.set("Compacting pool... please wait.")
            self.root.update_idletasks()

            summary = compact_reassign_pool(out_root, max_json_mb=max_mb)

            self.status_var.set(
                f"Compact done: good={summary['good_count_after']}, bad={summary['bad_count_after']}, "
                f"next={summary['next_code']} | mapping={summary['mapping_path']}"
            )
            messagebox.showinfo(
                "Compact Pool Done",
                f"Compact 完成。\n\n"

# -*- coding: utf-8 -*-
r"""
annotation_flywheel_pcg_app_no_json_v7_stage2_topology_engine.py
...
POOL_PREFIX = "PCG_Direct_Stage2TopoEngine"
        ...
        for j, e2 in enumerate(edges):
            ...
            if not is_e2e and not local_t_events:
                ...
                if min_dist < collision_threshold:
                    m_idx1, m_idx2 = np.unravel_index(np.argmin(dist_mat), dist_mat.shape)
                    t1 = m_idx1 / float(sample_n - 1)
                    t2 = m_idx2 / float(sample_n - 1)
                    ang = get_angle_stage2(get_bezier_derivative_stage2(p1, t1), get_bezier_derivative_stage2(p2, t2))
                    pos = c1[m_idx1]
                    topology_events.append({
                        "type": "X",
            ...
            if (not PRESERVE_STAGE2_BUNDLE) and isinstance(bundle, dict):
                ...
                if isinstance(bundle["edit_history"], list):
                    bundle["edit_history"].append({
                        ...
                        "timestamp": time.strftime("%Y-%m-%d %H:%M:%S"),
                        "source": "annotation_flywheel_pcg_app_no_json_v7_stage2_topology_engine.py",
                    })

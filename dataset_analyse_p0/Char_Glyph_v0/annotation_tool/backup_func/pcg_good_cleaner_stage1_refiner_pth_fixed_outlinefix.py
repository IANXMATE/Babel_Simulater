# -*- coding: utf-8 -*-
r"""
pcg_good_cleaner_stage1_refiner_pth_fixed_outlinefix.py
...
def mask_view(edge_groups: List[List[Dict[str, Any]]], size=MASK_SIZE, pad=18):
    """
    生成多组 edge 共用的 viewport。

    outline guard 比较 before/after 的 mask_iou 时，二者必须使用同一个
    mn/scale/median_width；否则 before 和 after 会分别被归一化到整张画布，
    IoU 会被错误拉低。
    """
    pts = []
    widths = []
    for edges in edge_groups:
        for e in edges:
            ...
            if len(p):
                pts.append(p)
                widths.append(float(e.get("width", 10.0)))

    if not pts:
        return np.array([0.0, 0.0], dtype=np.float32), 1.0, 10.0
    ...
    return mn.astype(np.float32), float(scale), float(med)
...
def render_mask(edges: List[Dict[str, Any]], size=MASK_SIZE, pad=18, view=None) -> np.ndarray:
    ...
    if view is None:
        mn, scale, med = mask_view([edges], size=size, pad=pad)
    else:
        mn, scale, med = view
...
class Stage1Refiner:
    ...
    def try_merge(self, edges, ia, ib):
        ...
        before, after = render_mask([e1,e2], view=shared_view), render_mask([new], view=shared_view)
    ...
    def try_delete(self, edges, idx):
        ...
        target, rem_mask, full = render_mask([e], view=shared_view), render_mask(rem, view=shared_view), render_mask(edges, view=shared_view)

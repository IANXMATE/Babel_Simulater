...
class AnnotationFlywheelPCGNoJSONApp:
    ...
    def ensure_morpheme_diffusion_loaded(self, force: bool = False) -> Dict[str, Any]:
        ...
        if (not force) and self.morpheme_context is not None and self.morpheme_diffusion_result is not None:
            return {
                "enabled": True,
                "source": "memory",
                "tree_dir": tree_dir,
    ...
    def generate_batch(self):
        ...
        if self.use_morpheme_diffusion_var.get():
            try:
                t0 = time.perf_counter()
                setup_info["morpheme_diffusion"] = self.ensure_morpheme_diffusion_loaded(force=True)

...
class App:
    ...
    def _ui(self):
        ...
        ttk.Button(row, text="Preview Stage2 Topo Selected", command=self.preview_selected).pack(side="left", padx=3)
        ...
        ttk.Label(note, text="待 clean = good 中 stable_uid 不在 cleaned 中；保存只写 cleaned/，不删除/覆盖 good；预览为 原图 → 一阶段 clean → 二阶段 topo clean。").pack(side="left", padx=8, pady=4)
    ...
    def render_grid(self):
        ...
        for local_idx, uid in enumerate(visible):
            ...
            ttk.Button(frame, text="Stage2 Topo", command=lambda u=uid:self.preview_uid(u)).pack(side="left", padx=4, pady=4)
    ...
    def preview_uid(self, uid):
        ...
        try:
            ...
            self.before_stage1_stage2(uid, original, stage1, stage2)
        except Exception as e:
            traceback.print_exc()
    ...
    def before_stage1_stage2(self, uid, original, stage1, stage2):
        win = tk.Toplevel(self.root)
        win.title("Clean Preview — Original → Stage1 → Stage2 — " + uid)
        win.geometry("1780x920")
        ...
        for title, bundle, photo in panels:
            f = ttk.LabelFrame(body, text=title); f.pack(side="left", fill="both", expand=True, padx=5, pady=6)
            ttk.Label(f, image=photo).pack(side="top", padx=6, pady=6)
            t = tk.Text(f, height=26, width=62, font=("Consolas", 8))
    ...
    def _prepare_cleaned_bundle(self, uid, bundle, variant):
        ...
        if variant == "original":
            ...
            cm["changed"] = False
            cm["stage2_changed"] = False
            cm["saved_note"] = "Original good saved to cleaned without Stage1/Stage2 modification."
        elif variant == "stage1":
    ...
    def accept(self, uid, bundle, win=None, variant="stage2"):
    ...
    def open_cleaned(self):
        ...
        for idx,(uid,b) in enumerate(sorted(self.cleaned.items(), key=lambda kv:kv[0])):
            ...
            txt=f"{idx:04d}\n{uid}\nS={topo['stats']['stroke_count']} CC={topo['stats']['connected_components']} Cy={topo['stats']['cycle_count']} X={topo['stats']['x_count']}\nvariant={cm.get('saved_variant')} s1={cm.get('changed')} s2={cm.get('stage2_changed')} ops={len(cm.get('accepted_ops', []))}+{len(cm.get('stage2_accepted_ops', []))}\n{topo['topology_text'][:90]}"
            ttk.Label(frame, text=txt, font=("Consolas",8), justify="center", wraplength=size*2+50).pack(side="top")

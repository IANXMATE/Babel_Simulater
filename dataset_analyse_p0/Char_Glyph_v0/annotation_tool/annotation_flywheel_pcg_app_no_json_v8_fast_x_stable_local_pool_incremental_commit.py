# -*- coding: utf-8 -*-
r"""
annotation_flywheel_pcg_app_no_json_v8_fast_x_stable_local_pool_incremental_commit.py
...
def build_pool_commit_index(out_root: str) -> Dict[str, Any]:
    ...
    但只在第一次运行、Refresh Disk Count、或者索引丢失时触发。
    """
    ...
    for d in [good, bad]:
        for k in d.keys():
            try:
                max_code = max(max_code, int(str(k), 16))
            except Exception:
                pass

    index = {
        "version": "pool_commit_index_v1",
        "pool_prefix": POOL_PREFIX,
        "next_code": int(max(PUA_BASE, max_code + 1)),
...
class AnnotationFlywheelPCGNoJSONApp:
    ...
    def refresh_disk_counts(self):
        ...
        self.status_var.set(
            ...
            f"next={int(idx.get('next_code', PUA_BASE)):04X} | out_root={out_root}"
        )
            ...
            if (not PRESERVE_STAGE2_BUNDLE) and isinstance(bundle, dict):
                ...
                if isinstance(bundle["edit_history"], list):
                    bundle["edit_history"].append({
                        ...
                        "timestamp": time.strftime("%Y-%m-%d %H:%M:%S"),
                        "source": "annotation_flywheel_pcg_app_no_json_v8_fast_x_stable_local_pool_incremental_commit.py",
                    })

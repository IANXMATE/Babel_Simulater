...
PAGE_BATCH_WORKERS = max(2, min(16, (os.cpu_count() or 4) - 1))
...
class App:
    def __init__(self, root):
        ...
        self.page_executor = ThreadPoolExecutor(max_workers=PAGE_BATCH_WORKERS)
    ...
    def render_grid(self):
        ...
        self.status.set(
            ...
            f"| Stage1: {self.refiner.status} | Stage2: {self.phase2_refiner.status}"
        )
    ...
    def _start_page_variant_job(self, variant, reason="render", after_done=None):
        ...
        timer = TimingProbe(f"PageBatch {reason} variant={variant} page={self.page_idx+1} n={len(uids)}")
        futures = []
        for uid in uids:
            futures.append(self.page_executor.submit(self._compute_uid_variant, uid, variant))
        self.page_compute_jobs = futures
        self.status.set(f"当前页 {variant} 批量{reason}计算中：0/{len(futures)} | workers={PAGE_BATCH_WORKERS}")

        def poll():
            done = sum(1 for f in futures if f.done())
            self.status.set(f"当前页 {variant} 批量{reason}计算中：{done}/{len(futures)} | workers={PAGE_BATCH_WORKERS}")
            if done < len(futures):
            ...
            timer.mark(f"all_futures_done errors={len(errors)}")
            if PRINT_PREVIEW_TIMING:
            ...
            if errors:
                ...
                self.status.set(f"当前页 {variant} 计算失败 {len(errors)} 个")
                return

            if after_done is not None:
                after_done(uids)
            else:
                self.status.set(f"当前页 {variant} 计算完成。")

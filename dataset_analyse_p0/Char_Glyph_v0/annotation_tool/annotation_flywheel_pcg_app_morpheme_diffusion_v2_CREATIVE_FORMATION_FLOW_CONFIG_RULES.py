...
DEFAULT_CONFIG_DIR = os.path.join(TOOL_DIR, "config")
...
DEFAULT_GUI_PROFILE = {
    ...
    "checkbox_defaults": {
        ...
        "use_creative_formation_flow": True,
        # 以下三项保持原始脚本默认：按需自己打开。
    ...
    "scalar_defaults": {
        ...
        # Creative Formation-Flow：更高表示越少机械模板、越像从统一结构场生长。
...
# =============================================================================
# 0.5 GUI 配置文件：读取 / 保存 / 内置风格配置
# =============================================================================
# 配置文件目录：dataset_analyse_p0/Char_Glyph_v0/annotation_tool/config
# 扩展名使用 .cfg，内部是 JSON，便于手工编辑和版本管理。
CONFIG_SCHEMA = "pcg_flywheel_gui_config_v1"
...
def _mk_gui_cfg(
    ...
    *,
    ...
    string_vars: Optional[Dict[str, Any]] = None,
    bool_vars: Optional[Dict[str, bool]] = None,
) -> Dict[str, Any]:
    return {
        "schema": CONFIG_SCHEMA,
        "name": name,
        "description": description,
        "selected_style_modes": list(selected_styles),
        "selected_topology_families": list(selected_families),
        "string_vars": {k: str(v) for k, v in (string_vars or {}).items()},
        "bool_vars": {k: bool(v) for k, v in (bool_vars or {}).items()},
    }


def builtin_gui_configs() -> Dict[str, Dict[str, Any]]:
    old_styles = ["straight", "mild_left", "mild_right", "s_curve_left", "s_curve_right", "hook_left", "hook_right", "mixed"]
    old_families = ["chain", "fork", "star", "zigzag", "triangle", "box", "rune_cross", "ladder", "parallel_slash", "arc_spine", "random_tree", "cycle_with_tail"]
    human_families = [
        "vertical_column", "horizontal_bar_stack", "orthogonal_cross", "center_spine_branches",
        "parallel_verticals", "stacked_horizontals", "outline_box", "semi_enclosure",
        "left_right_layout", "top_bottom_layout", "hanzi_block",
        "formation_skeleton_field", "layout_skeleton_field", "organic_seal_field",
        "creative_formation_flow", "recursive_portal_field", "interlock_rune_flow", "orbit_weave_field",
    ]
    base_scalars = {
        "n_var": 5, "preview_size_var": 170, "seed_var": "",
        "stroke_min_var": 3, "stroke_max_var": 8, "cc_min_var": 1, "cc_max_var": 1,
        "cycles_min_var": 0, "cycles_max_var": 2,
        "width_min_var": 7, "width_max_var": 14, "jitter_var": 16,
        "max_json_mb_var": 80,
        "novelty_endpoint_tol_var": 4.0, "novelty_geometry_bins_var": 24,
        "novelty_max_family_per_batch_var": 2, "novelty_workers_var": "auto",
        "morpheme_depth_var": 4, "morpheme_discover_k_var": 16, "morpheme_auto_rule_weight_var": 0.35,
        "topo_model_manual_score_threshold_var": 0.40,
        "axis_snap_prob_var": 0.72, "grid_snap_prob_var": 0.58,
        "axis_weight_mult_var": 3.1, "outline_weight_mult_var": 3.8,
        "layout_weight_mult_var": 3.2, "axis_grid_step_var": 45.0,
        "min_human_line_length_var": 104.0, "forbid_near_axis_slant_deg_var": 10.0,
        "min_split_fragment_frac_var": round(1.0 / 6.0, 4),
        "physical_contact_tol_var": 3.1,
        "hanzi_rune_blend_ratio_var": 0.58, "rune_accent_prob_var": 0.46,
        "organic_layout_mutation_prob_var": 0.42,
        "fusion_field_strength_var": 0.62, "integrated_rune_attach_prob_var": 0.72,
        "creative_operator_depth_var": 3,
        "creative_non_hanzi_operator_prob_var": 0.58,
        "creative_curve_flow_strength_var": 0.62,
        "creative_port_coupling_var": 0.76,
        "creative_nested_enclosure_prob_var": 0.46,
        "creative_interlock_prob_var": 0.44,
        "creative_orbit_weave_prob_var": 0.34,
    }
    base_bools = {
        "use_topo_model_var": False,
        "use_novelty_filter_var": True,
        "use_morpheme_diffusion_var": False,
        "use_marked_library_shift_var": False,
        "use_discovered_rule_influence_var": False,
        "use_axis_orthogonal_var": True,
        "use_grid_snap_var": True,
        "use_outline_bias_var": True,
        "use_human_layout_bias_var": True,
        "use_short_split_filter_var": True,
        "use_style_field_fusion_var": True,
        "use_creative_formation_flow_var": True,
    }

    # 这个配置刻意模拟 annotation_flywheel_pcg_app_morpheme_diffusion_v2_FAST_CACHE_Last_valueable.py：
    # 只保留原始 PCG families + 原始 style 勾选；新增 Axis/Human/Creative 逻辑全部关闭。
    runic_scalars = dict(base_scalars)
    runic_scalars.update({
        "axis_snap_prob_var": 0.0, "grid_snap_prob_var": 0.0,
        "axis_weight_mult_var": 1.0, "outline_weight_mult_var": 1.0, "layout_weight_mult_var": 1.0,
        "min_human_line_length_var": 0.0,
        "physical_contact_tol_var": 3.2,
        "creative_operator_depth_var": 0,
        "creative_non_hanzi_operator_prob_var": 0.0,
        "creative_curve_flow_strength_var": 0.0,
        "creative_port_coupling_var": 0.0,
        "creative_nested_enclosure_prob_var": 0.0,
        "creative_interlock_prob_var": 0.0,
        "creative_orbit_weave_prob_var": 0.0,
    })
    runic_bools = dict(base_bools)
    runic_bools.update({
        "use_axis_orthogonal_var": False,
        "use_grid_snap_var": False,
        "use_outline_bias_var": False,
        "use_human_layout_bias_var": False,
        "use_short_split_filter_var": False,
        "use_style_field_fusion_var": False,
        "use_creative_formation_flow_var": False,
    })

    balanced_scalars = dict(base_scalars)
    balanced_scalars.update({
        "stroke_min_var": 5, "stroke_max_var": 10, "cycles_min_var": 0, "cycles_max_var": 3,
        "axis_snap_prob_var": 0.72, "grid_snap_prob_var": 0.58,
        "axis_weight_mult_var": 3.1, "outline_weight_mult_var": 3.3, "layout_weight_mult_var": 3.4,
        "creative_operator_depth_var": 3,
        "creative_non_hanzi_operator_prob_var": 0.60,
        "creative_curve_flow_strength_var": 0.68,
        "creative_port_coupling_var": 0.80,
        "creative_nested_enclosure_prob_var": 0.42,
        "creative_interlock_prob_var": 0.58,
        "creative_orbit_weave_prob_var": 0.35,
    })

    seal_scalars = dict(base_scalars)
    seal_scalars.update({
        "stroke_min_var": 6, "stroke_max_var": 12, "cycles_min_var": 1, "cycles_max_var": 5,
        "axis_snap_prob_var": 0.82, "grid_snap_prob_var": 0.72,
        "axis_weight_mult_var": 4.0, "outline_weight_mult_var": 5.2, "layout_weight_mult_var": 3.0,
        "min_human_line_length_var": 100,
        "physical_contact_tol_var": 2.9,
        "creative_operator_depth_var": 4,
        "creative_non_hanzi_operator_prob_var": 0.55,
        "creative_curve_flow_strength_var": 0.60,
        "creative_port_coupling_var": 0.82,
        "creative_nested_enclosure_prob_var": 0.68,
        "creative_interlock_prob_var": 0.50,
        "creative_orbit_weave_prob_var": 0.40,
    })

    alien_scalars = dict(base_scalars)
    alien_scalars.update({
        "stroke_min_var": 7, "stroke_max_var": 14, "cc_min_var": 1, "cc_max_var": 2,
        "cycles_min_var": 1, "cycles_max_var": 6,
        "axis_snap_prob_var": 0.56, "grid_snap_prob_var": 0.45,
        "axis_weight_mult_var": 2.1, "outline_weight_mult_var": 2.3, "layout_weight_mult_var": 2.5,
        "min_human_line_length_var": 92,
        "physical_contact_tol_var": 2.6,
        "creative_operator_depth_var": 4,
        "creative_non_hanzi_operator_prob_var": 0.70,
        "creative_curve_flow_strength_var": 0.75,
        "creative_port_coupling_var": 0.70,
        "creative_nested_enclosure_prob_var": 0.45,
        "creative_interlock_prob_var": 0.70,
        "creative_orbit_weave_prob_var": 0.55,
    })

    clean_scalars = dict(base_scalars)
    clean_scalars.update({
        "stroke_min_var": 4, "stroke_max_var": 8, "cycles_min_var": 0, "cycles_max_var": 2,
        "axis_snap_prob_var": 0.88, "grid_snap_prob_var": 0.82,
        "axis_weight_mult_var": 4.5, "outline_weight_mult_var": 3.8, "layout_weight_mult_var": 4.2,
        "min_human_line_length_var": 108,
        "physical_contact_tol_var": 3.2,
        "creative_operator_depth_var": 2,
        "creative_non_hanzi_operator_prob_var": 0.42,
        "creative_curve_flow_strength_var": 0.50,
        "creative_port_coupling_var": 0.86,
        "creative_nested_enclosure_prob_var": 0.30,
        "creative_interlock_prob_var": 0.40,
        "creative_orbit_weave_prob_var": 0.22,
    })

    return {
        "runic_default.cfg": _mk_gui_cfg(
            "runic_default",
            "兼容 FAST_CACHE_Last_valueable.py 的原始符文随机逻辑；新增 Axis/Human/Creative 分支全部关闭。",
            selected_styles=old_styles,
            selected_families=old_families,
            string_vars=runic_scalars,
            bool_vars=runic_bools,
        ),
        "creative_fusion_balanced.cfg": _mk_gui_cfg(
            "creative_fusion_balanced",
            "主工作区：可读骨架 + 曲线符文流 + 中等互锁。",
            selected_styles=["straight", "mild_left", "mild_right", "s_curve_left", "s_curve_right", "mixed"],
            selected_families=["chain", "fork", "zigzag", "box", "rune_cross", "arc_spine", "cycle_with_tail"] + human_families,
            string_vars=balanced_scalars,
            bool_vars=base_bools,
        ),
        "ancient_seal_portal.cfg": _mk_gui_cfg(
            "ancient_seal_portal",
            "封印 / 门洞 / 多层嵌套风格。",
            selected_styles=["straight", "mild_left", "mild_right", "s_curve_left", "s_curve_right"],
            selected_families=["box", "rune_cross", "cycle_with_tail", "outline_box", "semi_enclosure", "organic_seal_field", "recursive_portal_field", "orbit_weave_field"],
            string_vars=seal_scalars,
            bool_vars=base_bools,
        ),
        "alien_totem_explore.cfg": _mk_gui_cfg(
            "alien_totem_explore",
            "探索模式：更外星、更互锁、更环绕/编织，通过率会下降。",
            selected_styles=["mild_left", "mild_right", "strong_left", "strong_right", "s_curve_left", "s_curve_right", "hook_left", "hook_right", "mixed"],
            selected_families=["fork", "star", "zigzag", "rune_cross", "parallel_slash", "arc_spine", "random_tree", "cycle_with_tail", "creative_formation_flow", "interlock_rune_flow", "orbit_weave_field"],
            string_vars=alien_scalars,
            bool_vars=base_bools,
        ),
        "clean_readable_axis.cfg": _mk_gui_cfg(
            "clean_readable_axis",
            "干净可读：横竖/网格强，符文残差少，适合积累低噪声好样本。",
            selected_styles=["straight", "mild_left", "mild_right", "s_curve_left", "s_curve_right"],
            selected_families=["chain", "fork", "box", "ladder", "arc_spine", "vertical_column", "horizontal_bar_stack", "orthogonal_cross", "center_spine_branches", "parallel_verticals", "stacked_horizontals", "hanzi_block", "layout_skeleton_field"],
            string_vars=clean_scalars,
            bool_vars=base_bools,
        ),
    }


def ensure_default_gui_config_files(force: bool = False) -> List[str]:
    ensure_dir(DEFAULT_CONFIG_DIR)
    written = []
    for fn, cfg in builtin_gui_configs().items():
        fp = os.path.join(DEFAULT_CONFIG_DIR, fn)
...
# Creative Formation-Flow defaults.
# 目标：不再枚举“汉字模板”，而是采样可递归的 formation program，
# 再由同一个 skeleton/flow field 同时生长可读构造线与曲线符文线。
DEFAULT_USE_CREATIVE_FORMATION_FLOW = _profile_checkbox("use_creative_formation_flow", True)
DEFAULT_CREATIVE_OPERATOR_DEPTH = int(round(_profile_scalar("creative_operator_depth", 3)))
DEFAULT_CREATIVE_NON_HANZI_OPERATOR_PROB = _profile_scalar("creative_non_hanzi_operator_prob", 0.58)
DEFAULT_CREATIVE_CURVE_FLOW_STRENGTH = _profile_scalar("creative_curve_flow_strength", 0.62)
DEFAULT_CREATIVE_PORT_COUPLING = _profile_scalar("creative_port_coupling", 0.76)
DEFAULT_CREATIVE_NESTED_ENCLOSURE_PROB = _profile_scalar("creative_nested_enclosure_prob", 0.46)
DEFAULT_CREATIVE_INTERLOCK_PROB = _profile_scalar("creative_interlock_prob", 0.44)
DEFAULT_CREATIVE_ORBIT_WEAVE_PROB = _profile_scalar("creative_orbit_weave_prob", 0.34)

CREATIVE_FORMATION_FLOW_FAMILIES = {
    "creative_formation_flow",
    "recursive_portal_field",
    "interlock_rune_flow",
    "orbit_weave_field",
...
PCG_FAMILY_TO_PATTERN_RULES = {
    ...
    "creative_formation_flow": ["grid_ladder", "box_cycle", "cycle_with_chord", "branch_chain", "dense_cluster"],
    "recursive_portal_field": ["box_cycle", "cycle_ladder", "cycle_with_chord", "theta_graph", "dense_cluster"],
    "interlock_rune_flow": ["cross_with_tail", "dense_crossing", "cycle_with_chord", "radial_fork", "branch_chain"],
    "orbit_weave_field": ["radial", "fan", "star_with_tail", "cycle_with_chord", "dense_cluster"],
}
...
class GeneratorConfig:
    ...
    use_creative_formation_flow: bool = DEFAULT_USE_CREATIVE_FORMATION_FLOW
    creative_operator_depth: int = DEFAULT_CREATIVE_OPERATOR_DEPTH
    creative_non_hanzi_operator_prob: float = DEFAULT_CREATIVE_NON_HANZI_OPERATOR_PROB
    creative_curve_flow_strength: float = DEFAULT_CREATIVE_CURVE_FLOW_STRENGTH
    creative_port_coupling: float = DEFAULT_CREATIVE_PORT_COUPLING
    creative_nested_enclosure_prob: float = DEFAULT_CREATIVE_NESTED_ENCLOSURE_PROB
    creative_interlock_prob: float = DEFAULT_CREATIVE_INTERLOCK_PROB
    creative_orbit_weave_prob: float = DEFAULT_CREATIVE_ORBIT_WEAVE_PROB
...
# =============================================================================
# 3.6 Creative Formation-Flow Generator
# =============================================================================

def _creative_anchor_id(anchors: List[np.ndarray], p: np.ndarray, tol: float = 4.0) -> int:
    """带复用的 anchor，保证 formation program 的 component 真正连成图。"""
    p = np.asarray(p, dtype=np.float32)
    for i, q in enumerate(anchors):
        try:
            if float(np.linalg.norm(np.asarray(q, dtype=np.float32) - p)) <= tol:
                return int(i)
        except Exception:
            pass
    return add_anchor(anchors, p)
...
def _creative_line(
    ...
    if len(edges) >= int(stroke_count):
        return None
    p0 = np.asarray(p0, dtype=np.float32).copy()
    p1 = np.asarray(p1, dtype=np.float32).copy()
    min_len = float(getattr(cfg, "min_human_line_length", DEFAULT_MIN_HUMAN_LINE_LENGTH)) * float(min_mult)
    p0, p1 = _extend_segment_to_min_length(p0, p1, min_len)
    a = _creative_anchor_id(anchors, p0, reuse_tol)
    b = _creative_anchor_id(anchors, p1, reuse_tol)
    if a == b:
        return None
...
def _creative_shell(
    ...
    cfg: GeneratorConfig,
    stroke_count: int,
    open_sides: Optional[List[str]] = None,
    broken_prob: float = 0.0,
) -> List[Tuple[int, int]]:
    """参数化壳层：不是固定‘口/区’，支持破口、多开口、错位。"""
    open_sides = set(open_sides or [])
    x0, y0, x1, y1 = bbox
    pts = {
        "tl": np.asarray([x0, y0], dtype=np.float32),
        "tr": np.asarray([x1, y0], dtype=np.float32),
        "br": np.asarray([x1, y1], dtype=np.float32),
        "bl": np.asarray([x0, y1], dtype=np.float32),
    }
    segs = [
        ("top", pts["tl"], pts["tr"]),
        ("right", pts["tr"], pts["br"]),
        ("bottom", pts["bl"], pts["br"]),
        ("left", pts["tl"], pts["bl"]),
    ]
    out = []
    for side, p, q in segs:
        if side in open_sides:
            continue
        if rng.random() < broken_prob and len(edges) + 1 < stroke_count:
            # 破口壳层：同一边拆成两段，但每段不短；不是短碎片。
            r0, r1 = (0.0, 0.42) if rng.random() < 0.5 else (0.58, 1.0)
            a = p * (1 - r0) + q * r0
            b = p * (1 - r1) + q * r1
            e = _creative_line(anchors, edges, a, b, rng, cfg, stroke_count, min_mult=0.85)
        else:
            e = _creative_line(anchors, edges, p, q, rng, cfg, stroke_count, min_mult=0.95)
        if e is not None:
            out.append(e)
    return out


def _shrink_bbox(bbox, ratio: float, dx: float = 0.0, dy: float = 0.0):
    x0, y0, x1, y1 = bbox
    cx, cy = (x0 + x1) * 0.5 + dx, (y0 + y1) * 0.5 + dy
    w, h = (x1 - x0) * ratio, (y1 - y0) * ratio
    return (cx - w * 0.5, cy - h * 0.5, cx + w * 0.5, cy + h * 0.5)


def _creative_style_vector(family: str, rng: random.Random, cfg: GeneratorConfig) -> Dict[str, Any]:
    """连续风格/统计向量：替代 hanzi_template + rune_template 的硬拼接。"""
    base = build_global_style_vector(family, cfg, rng)
    non_hanzi = clamp(float(getattr(cfg, "creative_non_hanzi_operator_prob", DEFAULT_CREATIVE_NON_HANZI_OPERATOR_PROB)), 0.0, 1.0)
    curve = clamp(float(getattr(cfg, "creative_curve_flow_strength", DEFAULT_CREATIVE_CURVE_FLOW_STRENGTH)), 0.0, 1.0)
    port = clamp(float(getattr(cfg, "creative_port_coupling", DEFAULT_CREATIVE_PORT_COUPLING)), 0.0, 1.0)
    return {
        **base,
        "non_hanzi_operator_prob": round(non_hanzi, 4),
        "curve_flow_strength": round(curve, 4),
        "port_coupling": round(port, 4),
        "nested_enclosure_prob": round(clamp(float(getattr(cfg, "creative_nested_enclosure_prob", DEFAULT_CREATIVE_NESTED_ENCLOSURE_PROB)), 0.0, 1.0), 4),
        "interlock_prob": round(clamp(float(getattr(cfg, "creative_interlock_prob", DEFAULT_CREATIVE_INTERLOCK_PROB)), 0.0, 1.0), 4),
        "orbit_weave_prob": round(clamp(float(getattr(cfg, "creative_orbit_weave_prob", DEFAULT_CREATIVE_ORBIT_WEAVE_PROB)), 0.0, 1.0), 4),
    }


def _sample_creative_program(family: str, rng: random.Random, cfg: GeneratorConfig, style_vec: Dict[str, Any]) -> List[str]:
    """采样递归/组合 operator 序列。不是固定汉字规则，而是 formation operators。"""
    depth = max(1, min(5, int(getattr(cfg, "creative_operator_depth", DEFAULT_CREATIVE_OPERATOR_DEPTH))))
    ops = []
    # 第一个 operator 负责整体可读 scaffold。
    if family == "recursive_portal_field" or rng.random() < float(style_vec.get("nested_enclosure_prob", 0.46)):
        ops.append("NESTED_PORTAL")
    elif family == "interlock_rune_flow":
        ops.append("INTERLOCK")
    elif family == "orbit_weave_field":
        ops.append("ORBIT")
    else:
        ops.append(rng.choice(["SPINE_GROW", "PORTAL", "ASYM_FRAME"]))

    imaginative = ["INTERLOCK", "ORBIT", "WEAVE", "HINGE", "ECHO", "FLOW_BRANCH"]
    readable = ["SPINE_GROW", "BALANCED_STACK", "ANCHOR_FRAME", "PORTAL"]
    for _ in range(depth - 1):
        p_non = float(style_vec.get("non_hanzi_operator_prob", 0.58))
        if rng.random() < p_non:
            ops.append(rng.choice(imaginative))
        else:
            ops.append(rng.choice(readable))
    return ops


def _execute_creative_op(
    op: str,
    anchors: List[np.ndarray],
    edges: List[Tuple[int, int]],
    bbox,
    ...
    cfg: GeneratorConfig,
    stroke_count: int,
    style_vec: Dict[str, Any],
):
    x0, y0, x1, y1 = bbox
    min_len = float(getattr(cfg, "min_human_line_length", DEFAULT_MIN_HUMAN_LINE_LENGTH))
    flow = np.asarray(style_vec.get("flow", np.asarray([1.0, 1.0], dtype=np.float32)), dtype=np.float32)
    flow = flow / max(1e-6, float(np.linalg.norm(flow)))
    perp = np.asarray([-flow[1], flow[0]], dtype=np.float32)

    if op == "ANCHOR_FRAME":
        open_sides = [] if rng.random() < 0.45 else [rng.choice(["left", "right", "top", "bottom"])]
        _creative_shell(anchors, edges, bbox, rng, cfg, stroke_count, open_sides=open_sides, broken_prob=0.18)
        return

    if op == "NESTED_PORTAL":
        open_side = rng.choice(["right", "left", "top", "bottom", None])
        _creative_shell(anchors, edges, bbox, rng, cfg, stroke_count, open_sides=[] if open_side is None else [open_side], broken_prob=0.10)
        if len(edges) < stroke_count:
            inner = _shrink_bbox(bbox, rng.choice([0.52, 0.58, 0.64]), dx=rng.uniform(-8, 8), dy=rng.uniform(-8, 8))
            # 二层包围不是固定回字；允许错位和破口。
            _creative_shell(anchors, edges, inner, rng, cfg, stroke_count, open_sides=[rng.choice(["left", "right", "top", "bottom"])] if rng.random() < 0.55 else [], broken_prob=0.28)
        if len(edges) < stroke_count:
            inner2 = _shrink_bbox(bbox, 0.36, dx=rng.uniform(-10, 10), dy=rng.uniform(-10, 10))
            _execute_creative_op("SPINE_GROW", anchors, edges, inner2, rng, cfg, stroke_count, style_vec)
        return

    if op == "PORTAL":
        side = rng.choice(["left", "right", "top", "bottom"])
        _creative_shell(anchors, edges, bbox, rng, cfg, stroke_count, open_sides=[side], broken_prob=0.22)
        core = _bbox_pt(bbox, rng.uniform(0.38, 0.62), rng.uniform(0.38, 0.62))
        # 门洞到内部核心的“结构流”而不是随机斜线。
        ports = [_bbox_pt(bbox, 0.18, 0.22), _bbox_pt(bbox, 0.82, 0.22), _bbox_pt(bbox, 0.18, 0.78), _bbox_pt(bbox, 0.82, 0.78)]
        rng.shuffle(ports)
        for p in ports[:2]:
            if len(edges) >= stroke_count:
                break
            if _angle_dist_to_axis_deg(p, core) >= 14.0:
                _creative_line(anchors, edges, p, core, rng, cfg, stroke_count, min_mult=0.80)
        return

    if op == "SPINE_GROW":
        vertical = rng.random() < 0.72
        if vertical:
            x = x0 + (x1 - x0) * rng.uniform(0.42, 0.58)
            ys = [y0 + (y1 - y0) * r for r in [0.10, 0.32, 0.50, 0.68, 0.90]]
            ids = [_creative_anchor_id(anchors, np.asarray([x, y], dtype=np.float32)) for y in ys]
            for a, b in zip(ids[:-1], ids[1:]):
                if len(edges) < stroke_count and not _edge_exists(edges, a, b):
                    edges.append((a, b))
            for r, aid in zip([0.32, 0.50, 0.68], ids[1:-1]):
                if len(edges) >= stroke_count:
                    break
                side = rng.choice([-1, 1])
                p = anchors[aid]
                q = p + np.asarray([side * rng.uniform(min_len * 0.75, min_len * 1.35), rng.uniform(-14, 14)], dtype=np.float32)
                _creative_line(anchors, edges, p, q, rng, cfg, stroke_count, min_mult=0.70)
        else:
            y = y0 + (y1 - y0) * rng.uniform(0.42, 0.58)
            xs = [x0 + (x1 - x0) * r for r in [0.10, 0.32, 0.50, 0.68, 0.90]]
            ids = [_creative_anchor_id(anchors, np.asarray([x, y], dtype=np.float32)) for x in xs]
            for a, b in zip(ids[:-1], ids[1:]):
                if len(edges) < stroke_count and not _edge_exists(edges, a, b):
                    edges.append((a, b))
            for aid in ids[1:-1]:
                if len(edges) >= stroke_count:
                    break
                p = anchors[aid]
                q = p + np.asarray([rng.uniform(-14, 14), rng.choice([-1, 1]) * rng.uniform(min_len * 0.75, min_len * 1.20)], dtype=np.float32)
                _creative_line(anchors, edges, p, q, rng, cfg, stroke_count, min_mult=0.70)
        return

    if op == "BALANCED_STACK":
        rows = rng.choice([2, 3])
        for i in range(rows):
            if len(edges) >= stroke_count:
                break
            r = (i + 1) / float(rows + 1)
            p = _bbox_pt(bbox, rng.uniform(0.10, 0.20), r)
            q = _bbox_pt(bbox, rng.uniform(0.80, 0.92), r + rng.uniform(-0.03, 0.03))
            _creative_line(anchors, edges, p, q, rng, cfg, stroke_count, min_mult=0.95)
        if len(edges) < stroke_count:
            _execute_creative_op("FLOW_BRANCH", anchors, edges, bbox, rng, cfg, stroke_count, style_vec)
        return

    if op == "INTERLOCK":
        # 两个错位的 C/门洞壳互相咬合，不是左右模板。
        b1 = (x0, y0 + (y1-y0)*0.06, x0 + (x1-x0)*0.62, y1 - (y1-y0)*0.06)
        b2 = (x0 + (x1-x0)*0.38, y0 + (y1-y0)*0.12, x1, y1 - (y1-y0)*0.12)
        _creative_shell(anchors, edges, b1, rng, cfg, stroke_count, open_sides=["right"], broken_prob=0.18)
        _creative_shell(anchors, edges, b2, rng, cfg, stroke_count, open_sides=["left"], broken_prob=0.18)
        if len(edges) < stroke_count:
            p = _bbox_pt(b1, 0.72, rng.choice([0.30, 0.70]))
            q = _bbox_pt(b2, 0.28, rng.choice([0.70, 0.30]))
            _creative_line(anchors, edges, p, q, rng, cfg, stroke_count, min_mult=0.75)
        return

    if op == "ORBIT":
        c = _bbox_pt(bbox, 0.5 + rng.uniform(-0.06, 0.06), 0.5 + rng.uniform(-0.06, 0.06))
        radius = min(x1-x0, y1-y0) * rng.uniform(0.27, 0.37)
        angles = [rng.uniform(0, 2*math.pi) + k * 2*math.pi/3 for k in range(3)]
        pts = [c + radius * np.asarray([math.cos(a), math.sin(a)], dtype=np.float32) for a in angles]
        # orbit 不是圆，而是中心组织方式；保留少量可读 chord。
        for p in pts:
            if len(edges) >= stroke_count:
                break
            _creative_line(anchors, edges, c, p, rng, cfg, stroke_count, min_mult=0.70)
        if len(edges) < stroke_count:
            _creative_line(anchors, edges, pts[0], pts[1], rng, cfg, stroke_count, min_mult=0.75)
        return

    if op == "WEAVE":
        # 只用安全比例的长线交织，避免产生 <1/6 的物理短碎片。
        p1 = _bbox_pt(bbox, 0.12, 0.34)
        q1 = _bbox_pt(bbox, 0.88, 0.34)
        p2 = _bbox_pt(bbox, 0.22, 0.16)
        q2 = _bbox_pt(bbox, 0.72, 0.86)
        _creative_line(anchors, edges, p1, q1, rng, cfg, stroke_count, min_mult=0.95)
        if len(edges) < stroke_count:
            _creative_line(anchors, edges, p2, q2, rng, cfg, stroke_count, min_mult=0.95)
        return

    if op == "HINGE":
        joint = _bbox_pt(bbox, rng.uniform(0.38, 0.62), rng.uniform(0.38, 0.62))
        for a in [rng.uniform(-0.95, -0.35), rng.uniform(0.25, 0.90)]:
            if len(edges) >= stroke_count:
                break
            d = rotate(flow, a)
            q = joint + d * rng.uniform(min_len * 0.9, min_len * 1.55)
            _creative_line(anchors, edges, joint, q, rng, cfg, stroke_count, min_mult=0.75)
        return

    if op == "ECHO":
        base_y = rng.uniform(0.28, 0.42)
        for k in range(2):
            if len(edges) >= stroke_count:
                break
            y = base_y + k * rng.uniform(0.18, 0.26)
            p = _bbox_pt(bbox, 0.16 + 0.04*k, y)
            q = _bbox_pt(bbox, 0.84 - 0.04*k, y + rng.uniform(-0.04, 0.04))
            _creative_line(anchors, edges, p, q, rng, cfg, stroke_count, min_mult=0.90)
        return

    # FLOW_BRANCH fallback：曲线符文动势从已有 port/edge 生长。
    if len(edges) >= stroke_count:
        return
    if edges and rng.random() < float(style_vec.get("port_coupling", 0.76)):
        host = rng.choice(edges)
        r = rng.choice(DEFAULT_SAFE_ATTACH_RATIOS)
        p = _segment_point(anchors, host, r)
    else:
        p = _bbox_pt(bbox, rng.uniform(0.25, 0.75), rng.uniform(0.25, 0.75))
    q = p + flow * rng.uniform(min_len * 0.85, min_len * 1.45) + perp * rng.uniform(-min_len * 0.45, min_len * 0.45)
    if _angle_dist_to_axis_deg(p, q) < 14.0:
        q = p + rotate(flow, rng.choice([0.55, -0.55])) * min_len * 1.15
    _creative_line(anchors, edges, p, q, rng, cfg, stroke_count, min_mult=0.75)


def generate_creative_formation_flow_anchor_edges(
    family: str,
    stroke_count: int,
    cycles_target: int,
    rng: random.Random,
    ...
    """
    Creative Formation-Flow 数据飞轮 proposal：
      continuous style/statistics vector
      -> recursive formation program
      -> skeleton field + ports
      -> structure-aware rune flow
      -> physical projector.

    这仍是非神经随机生成，但生成分布不再是 trivial 模板枚举。
    """
    anchors: List[np.ndarray] = []
    edges: List[Tuple[int, int]] = []
    style_vec = _creative_style_vector(family, rng, cfg)
    program = _sample_creative_program(family, rng, cfg, style_vec)
    bbox = _jittered_bbox(68, 68, 332, 332, rng, 10 + 8 * float(style_vec.get("field_strength", 0.6)))

    # 先执行 program；每个 op 从同一 bbox/field 中长出来。
    for depth, op in enumerate(program):
        if len(edges) >= stroke_count:
            break
        # 递归感：后续 op 在 shrink/offset bbox 中执行，形成多层/互锁/内部符文流。
        local_ratio = max(0.42, 1.0 - 0.13 * depth)
        local = _shrink_bbox(bbox, local_ratio, dx=rng.uniform(-10, 10), dy=rng.uniform(-10, 10))
        _execute_creative_op(op, anchors, edges, local, rng, cfg, stroke_count, style_vec)

    # 用 FLOW_BRANCH / HINGE 补足 strokes，使 rune residual 从已有结构端口生长。
    attempts = 0
    while len(edges) < stroke_count and attempts < 60:
        attempts += 1
        op = rng.choice(["FLOW_BRANCH", "HINGE", "ECHO", "WEAVE"])
        _execute_creative_op(op, anchors, edges, bbox, rng, cfg, stroke_count, style_vec)

    # 不够时退回 skeleton field，保证至少有可读骨架。
    if len(edges) < max(1, min(stroke_count, 2)):
        _execute_creative_op("SPINE_GROW", anchors, edges, bbox, rng, cfg, stroke_count, style_vec)

    # 若多了，保留前段 program skeleton + 少量 residual。
    edges = edges[:stroke_count]

    # 统一做横竖吸附/长度保障；斜向 flow 不会被吸成近轴假斜。
    enforce_human_readable_axis_geometry(anchors, edges, cfg, rng, family=family)
    return anchors, edges
...
def generate_anchor_edges_for_family(
    ...
    if (
        family in CREATIVE_FORMATION_FLOW_FAMILIES
        and getattr(cfg, "use_creative_formation_flow", DEFAULT_USE_CREATIVE_FORMATION_FLOW)
    ):
        anchors, edges = generate_creative_formation_flow_anchor_edges(family, stroke_count, cycles_target, rng, cfg)

    elif family in SKELETON_FIELD_FAMILIES and getattr(cfg, "use_skeleton_field_generator", DEFAULT_USE_SKELETON_FIELD_GENERATOR):
...
def generate_procedural_candidate(cfg: GeneratorConfig, rng: random.Random, seq_idx: int) -> Dict[str, Any]:
    ...
    # Creative Formation-Flow 的核心：横竖骨架仍强制 straight，
    # 但非横竖 rune-flow edge 可以使用 mild / s-curve，从同一结构场里长出曲线动势。
    if (
        any(f in CREATIVE_FORMATION_FLOW_FAMILIES for f in families)
        and getattr(cfg, "use_creative_formation_flow", DEFAULT_USE_CREATIVE_FORMATION_FLOW)
        and rng.random() < clamp(float(getattr(cfg, "creative_curve_flow_strength", DEFAULT_CREATIVE_CURVE_FLOW_STRENGTH)), 0.0, 1.0)
    ):
        curve_pool = [m for m in cfg.style_modes if m in ("mild_left", "mild_right", "s_curve_left", "s_curve_right", "hook_left", "hook_right")]
        if curve_pool:
            style_mode = rng.choice(curve_pool)
    ...
    candidate = {
        ...
        "pcg_meta": {
            ...
            "axis_layout_prior": {
                ...
                "use_creative_formation_flow": bool(getattr(cfg, "use_creative_formation_flow", DEFAULT_USE_CREATIVE_FORMATION_FLOW)),
                "creative_operator_depth": int(getattr(cfg, "creative_operator_depth", DEFAULT_CREATIVE_OPERATOR_DEPTH)),
                "creative_non_hanzi_operator_prob": float(getattr(cfg, "creative_non_hanzi_operator_prob", DEFAULT_CREATIVE_NON_HANZI_OPERATOR_PROB)),
                "creative_curve_flow_strength": float(getattr(cfg, "creative_curve_flow_strength", DEFAULT_CREATIVE_CURVE_FLOW_STRENGTH)),
                "creative_port_coupling": float(getattr(cfg, "creative_port_coupling", DEFAULT_CREATIVE_PORT_COUPLING)),
                "creative_nested_enclosure_prob": float(getattr(cfg, "creative_nested_enclosure_prob", DEFAULT_CREATIVE_NESTED_ENCLOSURE_PROB)),
                "creative_interlock_prob": float(getattr(cfg, "creative_interlock_prob", DEFAULT_CREATIVE_INTERLOCK_PROB)),
                "creative_orbit_weave_prob": float(getattr(cfg, "creative_orbit_weave_prob", DEFAULT_CREATIVE_ORBIT_WEAVE_PROB)),
            },
...
# =============================================================================
# 7.5 独立拓扑规则派生：Good/Bad -> discriminative topology atoms
# =============================================================================

def _rule_count_events(obj: Dict[str, Any]) -> Counter:
    evs = obj.get("topology_events") or obj.get("topology") or []
    if isinstance(obj.get("pcg_meta"), dict):
        evs = obj.get("topology_events") or obj.get("pcg_meta", {}).get("stage2_topology", {}).get("topology_events", evs)
    c = Counter()
    if isinstance(evs, list):
        for e in evs:
            if not isinstance(e, dict):
                continue
            typ = str(e.get("type") or e.get("event_type") or e.get("relation") or "").upper()
            if "E2E" in typ or "END" in typ:
                c["E2E"] += 1
            elif typ == "T" or "T_JUNCTION" in typ:
                c["T"] += 1
            elif typ == "X" or "CROSS" in typ:
                c["X"] += 1
            elif typ:
                c[typ] += 1
    return c
...
def _rule_strokes(obj: Dict[str, Any]) -> List[Any]:
    for k in ["strokes", "solved_nodes", "nodes", "solved_segments"]:
        v = obj.get(k)
        if isinstance(v, list):
            return v
    if isinstance(obj.get("pcg_meta"), dict):
        v = obj.get("pcg_meta", {}).get("strokes")
        if isinstance(v, list):
            return v
    return []


def _independent_rule_features(obj: Dict[str, Any]) -> Dict[str, float]:
    strokes = _rule_strokes(obj)
    ev = _rule_count_events(obj)
    cycles = obj.get("cycles") if isinstance(obj.get("cycles"), list) else []
    stroke_count = len(strokes)
    cycle_count = len(cycles)
    event_total = max(1, sum(ev.values()))
    feats = {
        "stroke_count": float(stroke_count),
        "cycle_count": float(cycle_count),
        "E2E_count": float(ev.get("E2E", 0)),
        "T_count": float(ev.get("T", 0)),
        "X_count": float(ev.get("X", 0)),
        "event_total": float(sum(ev.values())),
        "T_ratio": float(ev.get("T", 0)) / event_total,
        "X_ratio": float(ev.get("X", 0)) / event_total,
        "has_cycle": 1.0 if cycle_count > 0 else 0.0,
    }
    # 如果是 PCG candidate 或 bundle 中保留了 meta，也把 family/style 变成离散 atom 候选。
    for meta_path in [obj, obj.get("glyph_info", {}) if isinstance(obj.get("glyph_info"), dict) else {}, obj.get("pcg_meta", {}) if isinstance(obj.get("pcg_meta"), dict) else {}]:
        if not isinstance(meta_path, dict):
            continue
        fam = meta_path.get("topology_family") or meta_path.get("family")
        sty = meta_path.get("style_mode") or meta_path.get("style")
        if fam:
            feats[f"family::{fam}"] = 1.0
        if sty:
            feats[f"style::{sty}"] = 1.0
    return feats


def _independent_atoms_from_features(feats: Dict[str, float]) -> List[str]:
    atoms = []
    for k, v in feats.items():
        if k.startswith("family::") or k.startswith("style::"):
            if v > 0:
                atoms.append(k)
            continue
        try:
            ...
            continue
        if k.endswith("_count") or k in {"stroke_count", "cycle_count", "event_total"}:
            atoms.append(f"{k}=={int(round(x))}")
            if x >= 1:
                atoms.append(f"{k}>=1")
            if x >= 2:
                atoms.append(f"{k}>=2")
...
def mine_independent_topology_rules(out_root: str, min_good_support: int = 5, min_precision: float = 0.65, max_rules: int = 32) -> Dict[str, Any]:
    good = load_pool_dict_from_disk(out_root, "good")
    bad = load_pool_dict_from_disk(out_root, "bad")
    good_items = list(good.values())
    bad_items = list(bad.values())
    atom_good = Counter()
    atom_bad = Counter()

    for obj in good_items:
        if isinstance(obj, dict):
            atom_good.update(set(_independent_atoms_from_features(_independent_rule_features(obj))))
    for obj in bad_items:
        if isinstance(obj, dict):
            atom_bad.update(set(_independent_atoms_from_features(_independent_rule_features(obj))))

    rules = []
    n_good = max(1, len(good_items))
    n_bad = max(1, len(bad_items))
    for atom, gp in atom_good.items():
        bp = atom_bad.get(atom, 0)
        if gp < int(min_good_support):
            continue
        precision = gp / max(1, gp + bp)
        recall = gp / n_good
        bad_rate = bp / n_bad
        lift = (gp / n_good) / max(1e-6, bad_rate)
        if precision < float(min_precision):
            continue
        rules.append({
            "rule_id": f"independent_topo_atom_{len(rules):04d}",
            "source": "good_bad_discriminative_atom_mining",
            "atom": atom,
            "support_good": int(gp),
            "support_bad": int(bp),
            ...
            "depends_on_existing_rule_id": False,
        })
    rules.sort(key=lambda r: (r["precision"], r["lift"], r["support_good"]), reverse=True)
    rules = rules[:int(max_rules)]

    ensure_dir(MORPHEME_NEW_RULE_CACHE_DIR)
    out = {
        "schema": "independent_topology_rule_mining_v1",
        "created_at": time.strftime("%Y-%m-%d %H:%M:%S"),
        "input": {"good_count": len(good_items), "bad_count": len(bad_items)},
        "params": {"min_good_support": min_good_support, "min_precision": min_precision, "max_rules": max_rules},
        "rules": rules,
    }
    fp = os.path.join(MORPHEME_NEW_RULE_CACHE_DIR, f"independent_topology_rules_{time.strftime('%Y%m%d_%H%M%S')}.json")
    with open(fp, "w", encoding="utf-8") as f:
...
class AnnotationFlywheelPCGNoJSONApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Annotation Flywheel PCG App - Creative Formation Flow + Config Profiles + Numbered Pipeline")
        self.root.geometry("1520x910")

        self.rng = random.Random(int(time.time()))

        ensure_default_gui_config_files(force=False)
        self.config_profile_var = tk.StringVar(value="creative_fusion_balanced.cfg")
        ...
        self.use_creative_formation_flow_var = tk.BooleanVar(value=_profile_checkbox("use_creative_formation_flow", DEFAULT_USE_CREATIVE_FORMATION_FLOW))
        self.creative_operator_depth_var = tk.StringVar(value=str(int(DEFAULT_CREATIVE_OPERATOR_DEPTH)))
        self.creative_non_hanzi_operator_prob_var = tk.StringVar(value=str(_profile_scalar("creative_non_hanzi_operator_prob", DEFAULT_CREATIVE_NON_HANZI_OPERATOR_PROB)))
        self.creative_curve_flow_strength_var = tk.StringVar(value=str(_profile_scalar("creative_curve_flow_strength", DEFAULT_CREATIVE_CURVE_FLOW_STRENGTH)))
        self.creative_port_coupling_var = tk.StringVar(value=str(_profile_scalar("creative_port_coupling", DEFAULT_CREATIVE_PORT_COUPLING)))
        self.creative_nested_enclosure_prob_var = tk.StringVar(value=str(_profile_scalar("creative_nested_enclosure_prob", DEFAULT_CREATIVE_NESTED_ENCLOSURE_PROB)))
        self.creative_interlock_prob_var = tk.StringVar(value=str(_profile_scalar("creative_interlock_prob", DEFAULT_CREATIVE_INTERLOCK_PROB)))
        self.creative_orbit_weave_prob_var = tk.StringVar(value=str(_profile_scalar("creative_orbit_weave_prob", DEFAULT_CREATIVE_ORBIT_WEAVE_PROB)))
        ...
        self.independent_rule_min_support_var = tk.StringVar(value="5")
        self.independent_rule_min_precision_var = tk.StringVar(value="0.65")
        self.independent_rule_max_rules_var = tk.StringVar(value="32")
        self.independent_rule_status_var = tk.StringVar(value="Independent rules: not mined")
    ...
    def _build_ui(self):
        ...
        row0 = ttk.LabelFrame(top, text="00 Config Profile - 从 annotation_tool/config 读取/保存 GUI 配置")
        row0.pack(fill="x", pady=4)
        ttk.Label(row0, text="Profile:").pack(side="left", padx=(8, 2))
        self.config_profile_combo = ttk.Combobox(
            row0,
            textvariable=self.config_profile_var,
            values=self.list_config_profiles(),
            width=38,
            state="normal",
        )
        self.config_profile_combo.pack(side="left", padx=2)
        ttk.Button(row0, text="Load Selected", command=self.load_selected_config_profile).pack(side="left", padx=4)
        ttk.Button(row0, text="Save Current As...", command=self.save_current_config_profile_as).pack(side="left", padx=4)
        ttk.Button(row0, text="Refresh List", command=self.refresh_config_profile_list).pack(side="left", padx=4)
        ttk.Button(row0, text="Write Built-ins", command=lambda: self.write_builtin_config_profiles(force=True)).pack(side="left", padx=4)
        ttk.Label(row0, text=f"dir={DEFAULT_CONFIG_DIR}", foreground="darkgreen").pack(side="left", padx=8)
        ...
        row_morph = ttk.LabelFrame(top, text="01 Stage 0M Morpheme Grammar Diffusion - 语素规则扩散 / 已标记库偏移 / 新规则缓存")
        ...
        row_indrule = ttk.LabelFrame(top, text="01B Independent Topology Rule Miner - Good/Bad 独立规则派生")
        row_indrule.pack(fill="x", pady=4)
        ttk.Label(row_indrule, text="min good:").pack(side="left", padx=(8, 2))
        ttk.Entry(row_indrule, textvariable=self.independent_rule_min_support_var, width=5).pack(side="left", padx=2)
        ttk.Label(row_indrule, text="min precision:").pack(side="left", padx=(8, 2))
        ttk.Entry(row_indrule, textvariable=self.independent_rule_min_precision_var, width=5).pack(side="left", padx=2)
        ttk.Label(row_indrule, text="max rules:").pack(side="left", padx=(8, 2))
        ttk.Entry(row_indrule, textvariable=self.independent_rule_max_rules_var, width=5).pack(side="left", padx=2)
        ttk.Button(row_indrule, text="Mine Independent Rules", command=self.mine_independent_rules_now).pack(side="left", padx=(12, 2))
        ttk.Label(row_indrule, textvariable=self.independent_rule_status_var, foreground="brown").pack(side="left", padx=8)

        row3 = ttk.LabelFrame(top, text="02 Stage 1A Style Modes - 曲线/笔画风格")
        ...
        row4 = ttk.LabelFrame(top, text="03 Stage 1B Base Topology Families - 基础拓扑 proposal")
        ...
        row4b = ttk.LabelFrame(top, text="04 Stage 1C Axis / Human-readable Construction - 几何构造投影/可读性 prior")
        row4b.pack(fill="x", pady=4)
        ttk.Checkbutton(row4b, text="04.1 启用横竖正交生成", variable=self.use_axis_orthogonal_var).pack(side="left", padx=6)
        ttk.Checkbutton(row4b, text="04.2 网格吸附", variable=self.use_grid_snap_var).pack(side="left", padx=6)
        ttk.Checkbutton(row4b, text="04.3 轮廓/外框偏置", variable=self.use_outline_bias_var).pack(side="left", padx=6)
        ttk.Checkbutton(row4b, text="04.4 左右/上下/主干排版偏置", variable=self.use_human_layout_bias_var).pack(side="left", padx=6)
        ...
        ttk.Checkbutton(row4b, text="04.5 物理短碎片过滤≥1/6", variable=self.use_short_split_filter_var).pack(side="left", padx=6)
        ...
        ttk.Checkbutton(row4b, text="04.6 结构场融合", variable=self.use_style_field_fusion_var).pack(side="left", padx=6)
        ...
        row4d = ttk.Frame(top)
        row4d.pack(fill="x", pady=2)
        ttk.Label(row4d, text="05 Stage 1D Creative Formation-Flow:").pack(side="left", padx=(8, 2))
        ttk.Checkbutton(row4d, text="05.1 递归构成流场", variable=self.use_creative_formation_flow_var).pack(side="left", padx=6)
        ttk.Label(row4d, text="depth:").pack(side="left", padx=(8, 2))
        ttk.Entry(row4d, textvariable=self.creative_operator_depth_var, width=3).pack(side="left", padx=2)
        ttk.Label(row4d, text="non-Hanzi op:").pack(side="left", padx=(8, 2))
        ttk.Entry(row4d, textvariable=self.creative_non_hanzi_operator_prob_var, width=4).pack(side="left", padx=2)
        ttk.Label(row4d, text="curve flow:").pack(side="left", padx=(8, 2))
        ttk.Entry(row4d, textvariable=self.creative_curve_flow_strength_var, width=4).pack(side="left", padx=2)
        ttk.Label(row4d, text="port:").pack(side="left", padx=(8, 2))
        ttk.Entry(row4d, textvariable=self.creative_port_coupling_var, width=4).pack(side="left", padx=2)
        ttk.Label(row4d, text="nest:").pack(side="left", padx=(8, 2))
        ttk.Entry(row4d, textvariable=self.creative_nested_enclosure_prob_var, width=4).pack(side="left", padx=2)
        ttk.Label(row4d, text="interlock:").pack(side="left", padx=(8, 2))
        ttk.Entry(row4d, textvariable=self.creative_interlock_prob_var, width=4).pack(side="left", padx=2)
        ttk.Label(row4d, text="orbit/weave:").pack(side="left", padx=(8, 2))
        ttk.Entry(row4d, textvariable=self.creative_orbit_weave_prob_var, width=4).pack(side="left", padx=2)

        row4c = ttk.Frame(top)
        row4c.pack(fill="x", pady=2)
        ttk.Label(row4c, text="06 Stage 1E Human/Skeleton/Creative Families:").pack(side="left", padx=(8, 2))
        ...
        row_novel = ttk.LabelFrame(top, text="07 Stage 1.5 Positive Topology Novelty - 去重：good + cleaned + annotations_topo，不读 bad")
        row_novel.pack(fill="x", pady=4)

        ttk.Checkbutton(
            row_novel,
            text="07.1 启用拓扑/粗几何去重",
        ...
        # 最终 topology_events / cycles 仍由第二阶段规则引擎生成。
        row_model = ttk.LabelFrame(top, text="08 Stage 2 V7 Stage1 Topology Gate - 召回粗筛，只作用第一阶段")
        ...
        ttk.Checkbutton(
            row_model_path,
            text="08.1 启用 V7 stage1 模型粗筛；最终 topology 仍由 stage2 规则生成",
    ...
    def mine_independent_rules_now(self):
        try:
            out_root = self.output_root.get().strip() or DEFAULT_OUTPUT_ROOT
            min_good = self.parse_int(self.independent_rule_min_support_var, 5)
            min_precision = self.parse_float(self.independent_rule_min_precision_var, 0.65)
            max_rules = self.parse_int(self.independent_rule_max_rules_var, 32)
            result = mine_independent_topology_rules(
                out_root,
                min_good_support=min_good,
                min_precision=min_precision,
                max_rules=max_rules,
            )
            rules = result.get("rules", [])
            self.independent_rule_status_var.set(f"Independent rules: {len(rules)} saved")
            messagebox.showinfo(
                "Independent Rules Mined",
                f"已从 Good/Bad 派生独立规则。\n\n"
                f"good={result.get('input',{}).get('good_count')} bad={result.get('input',{}).get('bad_count')}\n"
                f"rules={len(rules)}\n"
                f"output={result.get('output_path')}"
            )
        except Exception:
            self.independent_rule_status_var.set("Independent rules: FAILED")
            messagebox.showerror("Independent Rule Mining Failed", traceback.format_exc())

    def list_config_profiles(self) -> List[str]:
        ensure_default_gui_config_files(force=False)
        try:
            return sorted([fn for fn in os.listdir(DEFAULT_CONFIG_DIR) if fn.lower().endswith(".cfg")])
        except Exception:
            return []

    def refresh_config_profile_list(self):
        vals = self.list_config_profiles()
        if hasattr(self, "config_profile_combo"):
            self.config_profile_combo["values"] = vals
        if vals and not self.config_profile_var.get():
            self.config_profile_var.set(vals[0])
        self.status_var.set(f"Config profiles refreshed: {len(vals)} files in {DEFAULT_CONFIG_DIR}")

    def write_builtin_config_profiles(self, force: bool = True):
        try:
            files = ensure_default_gui_config_files(force=force)
            self.refresh_config_profile_list()
            messagebox.showinfo("Built-in Configs", "已写入内置配置：\n" + "\n".join(files))
        except Exception:
            messagebox.showerror("Write Built-ins Failed", traceback.format_exc())

    def _set_var_by_name(self, name: str, value: Any):
    ...
    def current_gui_config_dict(self, name: Optional[str] = None, description: str = "user saved config") -> Dict[str, Any]:
        string_var_names = [
            "output_root", "n_var", "preview_size_var", "seed_var",
            "stroke_min_var", "stroke_max_var", "cc_min_var", "cc_max_var",
            "cycles_min_var", "cycles_max_var", "width_min_var", "width_max_var", "jitter_var",
            "max_json_mb_var",
            "topo_model_index_var", "topo_model_purpose_var", "topo_model_score_mode_var",
            "topo_model_manual_score_threshold_var",
            "novelty_endpoint_tol_var", "novelty_geometry_bins_var", "novelty_max_family_per_batch_var", "novelty_workers_var",
            "morpheme_tree_dir_var", "morpheme_depth_var", "morpheme_discover_k_var", "morpheme_auto_rule_weight_var",
            "independent_rule_min_support_var", "independent_rule_min_precision_var", "independent_rule_max_rules_var",
            "axis_snap_prob_var", "grid_snap_prob_var", "axis_angle_jitter_var", "axis_weight_mult_var",
            "outline_weight_mult_var", "layout_weight_mult_var", "axis_grid_step_var",
            "min_human_line_length_var", "forbid_near_axis_slant_deg_var", "min_split_fragment_frac_var",
            "physical_contact_tol_var", "hanzi_rune_blend_ratio_var", "rune_accent_prob_var",
            "organic_layout_mutation_prob_var", "fusion_field_strength_var", "integrated_rune_attach_prob_var",
            "creative_operator_depth_var", "creative_non_hanzi_operator_prob_var", "creative_curve_flow_strength_var",
            "creative_port_coupling_var", "creative_nested_enclosure_prob_var", "creative_interlock_prob_var", "creative_orbit_weave_prob_var",
        ]
        bool_var_names = [
            "use_topo_model_var", "use_novelty_filter_var", "use_morpheme_diffusion_var",
            "use_marked_library_shift_var", "use_discovered_rule_influence_var",
            "use_axis_orthogonal_var", "use_grid_snap_var", "use_outline_bias_var", "use_human_layout_bias_var",
            "use_short_split_filter_var", "use_style_field_fusion_var", "use_creative_formation_flow_var",
        ]
        string_vars = {}
        for nm in string_var_names:
            if hasattr(self, nm):
                try:
                    string_vars[nm] = str(getattr(self, nm).get())
                except Exception:
                    pass
        bool_vars = {}
        for nm in bool_var_names:
            if hasattr(self, nm):
                try:
                    bool_vars[nm] = bool(getattr(self, nm).get())
                except Exception:
                    pass
        return {
            "schema": CONFIG_SCHEMA,
            "name": name or "user_config",
            "description": description,
            "selected_style_modes": [k for k, v in self.style_mode_vars.items() if v.get()],
            "selected_topology_families": [k for k, v in self.family_vars.items() if v.get()],
            "string_vars": string_vars,
            "bool_vars": bool_vars,
        }

    def apply_gui_config_dict(self, cfg: Dict[str, Any]):
        if not isinstance(cfg, dict):
            raise ValueError("Config must be a JSON object.")
        for nm, val in (cfg.get("string_vars") or {}).items():
            self._set_var_by_name(nm, val)
        for nm, val in (cfg.get("bool_vars") or {}).items():
            self._set_var_by_name(nm, val)

        if isinstance(cfg.get("selected_style_modes"), list):
            selected = set(map(str, cfg.get("selected_style_modes")))
            for k, v in self.style_mode_vars.items():
                v.set(k in selected)
        if isinstance(cfg.get("selected_topology_families"), list):
            selected = set(map(str, cfg.get("selected_topology_families")))
        ...
        self.novelty_filter = None
        self.novelty_cache = None
        self.on_toggle_morpheme_diffusion()
        self.on_toggle_topo_model()
        self.on_toggle_novelty_filter()

    def load_selected_config_profile(self):
        fn = self.config_profile_var.get().strip()
        if not fn:
            messagebox.showwarning("No config", "请选择或输入一个 .cfg 文件名。")
            return
        if not fn.lower().endswith(".cfg"):
            fn += ".cfg"
        fp = os.path.join(DEFAULT_CONFIG_DIR, os.path.basename(fn))
        if not os.path.exists(fp):
            messagebox.showerror("Config not found", f"找不到配置文件：\n{fp}")
            return
        try:
            with open(fp, "r", encoding="utf-8") as f:
                cfg = json.load(f)
            self.apply_gui_config_dict(cfg)
            self.config_profile_var.set(os.path.basename(fp))
            self.status_var.set(f"Loaded config: {fp}")
            messagebox.showinfo("Config Loaded", f"已加载配置：\n{fp}\n\n{cfg.get('description','')}")
        except Exception:
            messagebox.showerror("Load Config Failed", traceback.format_exc())

    def save_current_config_profile_as(self):
        ensure_default_gui_config_files(force=False)
        name = simpledialog.askstring("Save Config", "输入新配置名，不需要写 .cfg：", parent=self.root)
        if not name:
            return
        safe = re.sub(r"[^0-9A-Za-z_\-]+", "_", name.strip()).strip("_")
        if not safe:
            messagebox.showwarning("Invalid name", "配置名不能为空。")
            return
        fn = safe + ".cfg"
        fp = os.path.join(DEFAULT_CONFIG_DIR, fn)
        if os.path.exists(fp):
            if not messagebox.askyesno("Overwrite?", f"配置已存在，是否覆盖？\n{fp}"):
                return
        cfg = self.current_gui_config_dict(name=safe, description="saved from GUI")
        try:
            with open(fp, "w", encoding="utf-8") as f:
                json.dump(cfg, f, ensure_ascii=False, indent=2)
            self.refresh_config_profile_list()
            self.config_profile_var.set(fn)
            self.status_var.set(f"Saved config: {fp}")
            messagebox.showinfo("Config Saved", f"已保存当前配置：\n{fp}")
        except Exception:
            messagebox.showerror("Save Config Failed", traceback.format_exc())
    ...
    def get_generator_config(self) -> GeneratorConfig:
        ...
        use_creative_formation_flow = bool(self.use_creative_formation_flow_var.get())
        creative_operator_depth = int(round(self.parse_float(self.creative_operator_depth_var, DEFAULT_CREATIVE_OPERATOR_DEPTH)))
        creative_non_hanzi_operator_prob = self.parse_float(self.creative_non_hanzi_operator_prob_var, DEFAULT_CREATIVE_NON_HANZI_OPERATOR_PROB)
        creative_curve_flow_strength = self.parse_float(self.creative_curve_flow_strength_var, DEFAULT_CREATIVE_CURVE_FLOW_STRENGTH)
        creative_port_coupling = self.parse_float(self.creative_port_coupling_var, DEFAULT_CREATIVE_PORT_COUPLING)
        creative_nested_enclosure_prob = self.parse_float(self.creative_nested_enclosure_prob_var, DEFAULT_CREATIVE_NESTED_ENCLOSURE_PROB)
        creative_interlock_prob = self.parse_float(self.creative_interlock_prob_var, DEFAULT_CREATIVE_INTERLOCK_PROB)
        creative_orbit_weave_prob = self.parse_float(self.creative_orbit_weave_prob_var, DEFAULT_CREATIVE_ORBIT_WEAVE_PROB)
        ...
        topology_condition = {
            ...
            "use_creative_formation_flow": use_creative_formation_flow,
            "creative_operator_depth": creative_operator_depth,
            "creative_non_hanzi_operator_prob": creative_non_hanzi_operator_prob,
            "creative_curve_flow_strength": creative_curve_flow_strength,
            "creative_port_coupling": creative_port_coupling,
            "creative_nested_enclosure_prob": creative_nested_enclosure_prob,
            "creative_interlock_prob": creative_interlock_prob,
            "creative_orbit_weave_prob": creative_orbit_weave_prob,
        }

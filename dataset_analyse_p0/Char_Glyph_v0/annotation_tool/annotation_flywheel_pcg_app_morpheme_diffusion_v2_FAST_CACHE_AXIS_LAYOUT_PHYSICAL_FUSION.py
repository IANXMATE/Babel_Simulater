...
DEFAULT_HANZI_RUNE_BLEND_RATIO = 0.55
DEFAULT_RUNE_ACCENT_PROB = 0.42
DEFAULT_ORGANIC_LAYOUT_MUTATION_PROB = 0.35
    ...
    for i, si in enumerate(strokes):
        ...
        for ep_name, ep in [("start", P_i[0]), ("end", P_i[3])]:
            for j, sj in enumerate(strokes):
                ...
                if rj is not None and 0.0005 < rj < 0.9995:
...
class AnnotationFlywheelPCGNoJSONApp:
    def __init__(self, root):
        ...
        self.physical_contact_tol_var = tk.StringVar(value=str(DEFAULT_PHYSICAL_CONTACT_TOL))
        self.hanzi_rune_blend_ratio_var = tk.StringVar(value=str(DEFAULT_HANZI_RUNE_BLEND_RATIO))
        self.rune_accent_prob_var = tk.StringVar(value=str(DEFAULT_RUNE_ACCENT_PROB))
        self.organic_layout_mutation_prob_var = tk.StringVar(value=str(DEFAULT_ORGANIC_LAYOUT_MUTATION_PROB))

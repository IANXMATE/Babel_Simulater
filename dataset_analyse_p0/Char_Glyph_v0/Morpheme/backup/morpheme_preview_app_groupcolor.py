# -*- coding: utf-8 -*-
r"""
morpheme_preview_app_groupcolor.py

放置：
    dataset_analyse_p0/Char_Glyph_v0/Morpheme/morpheme_preview_app_groupcolor.py
...
作用：
    从 output_tree 读取拓扑语素库，随机预览一个语素：
      ...
      - 新增：child 在 parent 上的同组着色预览（同一 child 在 parent 上显示为相同颜色）
...
运行：
    ...
    python morpheme_preview_app_groupcolor.py

可选：
    python morpheme_preview_app_groupcolor.py --tree-dir output_tree
"""
...
COLOR_CHILD_A = "#d64b4b"   # red-ish
COLOR_CHILD_B = "#3d6fd6"   # blue-ish
COLOR_OVERLAP = "#8a45b8"   # purple
...
def seg_identity(seg, fallback_i=None):
    for k in ("source_sid", "local_id", "sid", "stroke_id"):
        if k in seg:
...
def draw_segments(canvas, segments, title="", width=3, jitter=False,
    ...
    for i, s in enumerate(segments):
        ...
        color = default_color
        if color_by_seg_id and sid in color_by_seg_id:
...
def infer_parent_child_color_map(parent_segments, comp):
    """
    根据 composition examples 里的 source_sid 对应关系，给 parent 中每条 segment 分配颜色。
    优先读取：
      examples[0].child_a_source_sids
      examples[0].child_b_source_sids
    如果没有 examples，则返回空映射。
    """
    color_map = {}
    if not comp:
...
class MorphemePreviewApp:
    ...
    def redraw(self):
        ...
        parent_title = f"Parent {self.current_parent_id} | {parent.get('stroke_count')} strokes"
        ...
        if comp:
            ...
            draw_segments(
                ...
                show_ids=True,
                title_suffix="A=red B=blue overlap=purple other=gray",
            )
            draw_segments(
                self.child_a_canvas,
                ca.get("prototype_segments", []) if ca else [],
                title=f"Child A {child_ids[0] if child_ids else '?'}",
                width=4,
                jitter=jitter,
            ...
            draw_segments(
                ...
                title=f"Child B {child_ids[1] if len(child_ids)>1 else '?'}",
                width=4,
                jitter=jitter,
            ...
            draw_segments(self.parent_group_canvas, [], title="Parent grouped by child")
        ...
        self._write_info(grouped)

    def _write_info(self, grouped):
        parent = self.nodes.get(self.current_parent_id, {})
        comp = self.current_comp
        ...
        lines.append(json.dumps({
            ...
            "other_parent_segments": grouped.get("other", []),
            "legend": {

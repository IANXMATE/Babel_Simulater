    ...
    for idx, inst in enumerate(raw_instances, 1):
        ...
        for A, B in split_partitions(inst.subset):
            ...
            if len(row["examples"]) < 10:
                row["examples"].append({"glyph_key": inst.glyph_key, "parent_source_sids": list(inst.source_sids),
                                        "child_a_source_sids": list(ia.source_sids), "child_b_source_sids": list(ib.source_sids)})
        if idx % 5000 == 0:

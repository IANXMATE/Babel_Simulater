# -*- coding: utf-8 -*-
r"""
train_flywheel_topohgt_v7_stage1_recall_ckptselect.py

建议放置位置：
    dataset_analyse_p0/Char_Glyph_v0/flywheel_model/train_flywheel_topohgt_v7_stage1_recall_ckptselect.py
...
class Cfg:
    ...
    # V7 stage1-only input:
    #   只使用第一阶段拓扑/骨架可见的信息：
    #     - stroke 端点位置
    #     - chord 长度、方向
    #     - node pair 距离、角度、端点到线段距离、线段到线段距离
    #   不使用：
    #     - Bezier 中间控制点形状 P1/P2
    #     - 曲率
    #     - width / width_bezier
    #     - raster 图像分支
    input_mode: str = "stage1_topology_only"
    use_raster_branch: bool = False
    ...
    # 训练结束后自动删除本次 preprocess_cache 文件，避免 model/preprocess_cache 越积越大。
    cleanup_preprocess_cache_after_train: bool = True
    ...
    # 第一阶段拓扑粗筛默认不追求 best F1，而是追求“召回足够高时 precision 尽量高”。
    # 可选: quality_high_recall_precision / quality_best_f1 / loss
    early_stop_metric: str = "quality_high_recall_precision"
    ...
    # 第一阶段保召回粗筛：在 recall >= target 的阈值中，选 precision 最高。
    recall_target: float = 0.85

    # debug / save
    num_workers: int = 0
    log_every: int = 5
    save_prefix: str = "flywheel_topohgt_v7_stage1_recall_ckptselect"
...
def choose_checkpoint_from_index(index_path: str, purpose: str = "quality_balanced") -> Dict[str, Any]:
    ...
    purpose 可选：
      ...
      - quality_high_recall:  recall>=target 时 precision 最优，适合第一阶段保召回粗筛
...
def cache_signature(records, cfg):
    payload = {
        "script": "train_flywheel_topohgt_v7_stage1_recall_ckptselect",
        ...
        "cfg": {
            ...
            "use_raster_branch": cfg.use_raster_branch,
            "stage1_topology_only": True,
            "stage1_recall_variant": True,
...
def point_to_segment_min_dist_and_t(pt, P):
    """
    V7 stage1-only 几何：端点到 stroke 骨架线段 P0-P3 的距离。
    不使用 Bezier 中间控制点和曲率。
    """
    ...
    q = a + t * ab
    return float(np.linalg.norm(pt - q)), t


def segment_segment_min_dist_stage1(P1, P2, sample_n=32):
    """
    V7 stage1-only 几何：两个 stroke 骨架线段 P0-P3 的最小距离。
    用线段均匀采样，避免引入 Bezier 曲线形状。
    """
    P1 = np.asarray(P1, dtype=np.float32)
    P2 = np.asarray(P2, dtype=np.float32)
    t = np.linspace(0.0, 1.0, sample_n, dtype=np.float32)[:, None]
    c1 = P1[0][None, :] * (1.0 - t) + P1[3][None, :] * t
    c2 = P2[0][None, :] * (1.0 - t) + P2[3][None, :] * t
...
def stroke_stats(s, cfg: Cfg):
    """
    V7 stage1-only stroke stats.

    关键约束：
      - 只读 mother_bezier 的 P0/P3 作为骨架边两个端点；
      - 不使用 P1/P2 形状控制点；
      - 不使用 width / raster / curvature。
    """
    P_raw = np.asarray(s["mother_bezier"], dtype=np.float32)
    if P_raw.shape != (4, 2):
        P_raw = np.zeros((4, 2), dtype=np.float32)

    P = np.zeros((4, 2), dtype=np.float32)
    P[0] = P_raw[0]
    P[3] = P_raw[3]
    # P1/P2 只设为线性插值占位，后续不作为形状特征使用。
    P[1] = P[0] * (2.0 / 3.0) + P[3] * (1.0 / 3.0)
    P[2] = P[0] * (1.0 / 3.0) + P[3] * (2.0 / 3.0)

    ts = np.linspace(0, 1, cfg.curve_sample_n, dtype=np.float32)[:, None]
    line = P[0][None, :] * (1.0 - ts) + P[3][None, :] * ts

    chord = P[3] - P[0]
    chord_len = float(np.linalg.norm(chord))
    chord_dir = norm_vec(chord)
    center = (P[0] + P[3]) * 0.5
    mn = np.minimum(P[0], P[3])
    mx = np.maximum(P[0], P[3])
...
def node_feature(meta: Dict[str, Any], idx: int, cfg: Cfg) -> np.ndarray:
    ...
    feat = np.concatenate([
        ...
        endpoint_kind,                                         # 2
        P[0] / cfg.canvas_norm,                                # 2
        P[3] / cfg.canvas_norm,                                # 2
...
def edge_feature(mi: Dict[str, Any], mj: Dict[str, Any], cfg: Cfg) -> np.ndarray:
    """
    V7 stage1-only edge feature.

    可以使用节点之间的距离、方向、夹角、端点到骨架线段距离、线段到线段距离；
    不使用曲线采样形状和 width。
    """
    ...
    ti_vec = norm_vec(mi["tangent"])
    tj_vec = norm_vec(mj["tangent"])
    tangent_cos = float(np.clip(np.dot(ti_vec, tj_vec), -1, 1))
    tangent_sin_abs = float(abs(ti_vec[0] * tj_vec[1] - ti_vec[1] * tj_vec[0]))

    endpoint_segment_dist = 0.0
    endpoint_segment_t = 0.0
    segment_segment_dist = 0.0

    if endpoint_to_stroke:
        endpoint_segment_dist, endpoint_segment_t = point_to_segment_min_dist_and_t(pi, mj["stats"]["P"])
    elif stroke_to_endpoint:
        endpoint_segment_dist, endpoint_segment_t = point_to_segment_min_dist_and_t(pj, mi["stats"]["P"])
    elif stroke_to_stroke:
        segment_segment_dist = segment_segment_min_dist_stage1(mi["stats"]["P"], mj["stats"]["P"], sample_n=32)
    ...
    feat = np.concatenate([
        ...
        np.array([
            ...
            tangent_cos,
            tangent_sin_abs,
            float(np.dot(delta_dir, ti_vec)),
            float(abs(delta_dir[0] * ti_vec[1] - delta_dir[1] * ti_vec[0])),
            float(np.dot(delta_dir, tj_vec)),
            float(abs(delta_dir[0] * tj_vec[1] - delta_dir[1] * tj_vec[0])),
            endpoint_segment_dist / cfg.dist_norm,
            endpoint_segment_t,
            segment_segment_dist / cfg.dist_norm,
        ], dtype=np.float32),                                  # 19
...
def render_raster(bundle: Dict[str, Any], cfg: Cfg) -> np.ndarray:
    ...
    if getattr(cfg, "input_mode", "") == "stage1_topology_only" or not getattr(cfg, "use_raster_branch", False):
    ...
    for i in range(N):
        for j in range(N):
            ...
            if is_t_pair_type:
                ...
                is_candidate = positive
                if not is_candidate:
                    d, _ = point_to_segment_min_dist_and_t(mi["pos"], mj["stats"]["P"])
            ...
            if is_x_pair_type:
                ...
                is_candidate = positive
                if not is_candidate:
                    is_candidate = segment_segment_min_dist_stage1(mi["stats"]["P"], mj["stats"]["P"], sample_n=32) <= cfg.cand_x_curve_curve_dist_max
                if is_candidate:
                    masks["x"][i, j] = True
...
class FlywheelTopoDataset(Dataset):
    def __init__(self, records: List[Dict[str, Any]], cfg: Cfg):
        ...
        print("[Dataset] node_types: stroke + endpoint_start + endpoint_end | input=stage1_topology_only")
...
class RasterCNN(nn.Module):
    """
    保留空壳，兼容旧 runtime / checkpoint config。
    V7 stage1-only 默认不会使用 raster branch。
    """
...
class TopoHGTStage1(nn.Module):
    """
    V7: 第一阶段拓扑/骨架输入模型。

    输入:
      - node_features: stroke / endpoint 的骨架节点特征
      - edge_features: 节点 pair 的距离、角度、线段投影关系
      - node_mask

    不输入:
      - raster image
      - Bezier P1/P2 形状
      - width / curve curvature
    """
    ...
    def forward(self, batch):
        ...
        q_logit = self.quality_head(torch.cat([mean_pool, max_pool, node_count_norm], dim=-1)).squeeze(-1)
        return {
...
# Backward-compatible alias: GUI/runtime that imports TopoHGTv6 can still load this script.
TopoHGTv6 = TopoHGTStage1
...
def threshold_sweep(probs, labels, cfg: Cfg):
    ...
    feasible_precision = [r for r in rows if r["precision"] >= cfg.precision_target]
    best_at_precision = max(feasible_precision, key=lambda r: (r["recall"], r["f1"])) if feasible_precision else None

    # 第一阶段拓扑粗筛：保召回优先。
    # 在 recall >= target 的阈值中，选 precision 最高，避免混入太多负样本。
    feasible_recall = [r for r in rows if r["recall"] >= cfg.recall_target]
    if feasible_recall:
        best_at_recall = max(feasible_recall, key=lambda r: (r["precision"], r["f1"], r["threshold"]))
    else:
        # 没有阈值达到目标 recall 时，退化为 recall 最高，其次 precision。
        best_at_recall = max(rows, key=lambda r: (r["recall"], r["precision"], r["f1"]))

    return rows, best_f1, best_at_precision, best_at_recall
...
def evaluate(model, loader, device, cfg, bce_quality, bce_rel):
    ...
    rows, best_f1, best_at_precision, best_at_recall = threshold_sweep(all_probs, all_labels, cfg)
    ...
    m.update({
        ...
        "quality_best_at_recall_target": best_at_recall,
    })
...
def print_metrics(epoch, train_loss, m, lr, cfg):
    ...
    br = m.get("quality_best_at_recall_target")
    ...
    if br is not None:
        tag = "bestPrecision" if br["recall"] >= cfg.recall_target else "fallbackMaxRecall"
        print(
            f"  quality R>={cfg.recall_target:.2f} {tag} th={br['threshold']:.2f} "
            f"P/R/F1={br['precision']:.3f}/{br['recall']:.3f}/{br['f1']:.3f} "
            f"(tp={br['tp']} fp={br['fp']} fn={br['fn']} tn={br['tn']})"
        )
...
def cleanup_preprocess_cache(ds, cfg: Cfg):
    """
    删除本次训练用到的 preprocess cache 文件，避免 model/preprocess_cache 越积越大。
    只删当前 dataset 对应的 cache_path，不动 checkpoint / manifest / error analysis。
    """
    if not getattr(cfg, "cleanup_preprocess_cache_after_train", True):
        return
    cache_path = getattr(ds, "cache_path", None)
    if not cache_path:
        return
    try:
        if os.path.exists(cache_path):
            os.remove(cache_path)
            print(f"[DatasetCache] cleanup removed: {cache_path}")
        else:
            print(f"[DatasetCache] cleanup skipped, not found: {cache_path}")
    except Exception as e:
        print(f"[DatasetCache][WARN] cleanup failed: {cache_path} err={repr(e)}")
...
def train():
    ...
    print("=" * 88)
    print("[FlywheelTopoHGT-v7-stage1-recall-ckptselect] start")
    ...
    model = TopoHGTStage1(ds.node_dim, ds.edge_dim, cfg).to(device)
    params = sum(p.numel() for p in model.parameters())
    trainable = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(f"[Model] TopoHGTStage1 node_dim={ds.node_dim} edge_dim={ds.edge_dim} params={params:,} trainable={trainable:,}")
    ...
    ckpt_paths = {
        ...
        "quality_high_recall": os.path.join(MODEL_SAVE_DIR, f"{cfg.save_prefix}_best_quality_recall{int(cfg.recall_target * 100)}_precision.pth"),
    ...
    checkpoint_index = {
        ...
        "selection_rules": {
            ...
            "quality_high_recall": "max precision among thresholds with recall>=recall_target; first-stage coarse filter, recall-oriented",
    ...
    best_metric_for_stop = math.inf if cfg.early_stop_metric == "loss" else -math.inf
    bad_epochs = 0

    def metric_summary(m: Dict[str, Any]) -> Dict[str, Any]:
        qhp = m.get("quality_best_at_precision_target")
        qhr = m.get("quality_best_at_recall_target")
        ...
        out = {
            ...
            "quality_recall_target": None if qhr is None else {
                "score": float(qhr["precision"]),
                "threshold": float(qhr["threshold"]),
                "precision": float(qhr["precision"]),
                "recall": float(qhr["recall"]),
                "f1": float(qhr["f1"]),
                "tp": int(qhr["tp"]),
                "fp": int(qhr["fp"]),
                "fn": int(qhr["fn"]),
    ...
    def make_recommendation_record(tag: str, path: str, epoch: int, train_loss: float, score: float, m: Dict[str, Any]) -> Dict[str, Any]:
        ...
        rec = {
            ...
            "thresholds": {
                ...
                "quality_recall_target": None if ms["quality_recall_target"] is None else ms["quality_recall_target"]["threshold"],
    ...
    def make_ckpt(epoch, train_loss, m):
        return {
            ...
            "node_type": NODE_TYPE,
            "rel_map": REL_MAP,
            "input_mode": "stage1_topology_only",
            "stage1_only_notes": "No Bezier P1/P2 shape, no width, no raster branch. Uses endpoint/chord skeleton geometry plus pair distances/angles.",
            "selection_note": "For stage1 coarse filter, prefer purpose='quality_high_recall' to keep recall high and maximize precision under recall target.",
            "epoch": epoch,
    ...
    for epoch in range(1, cfg.epochs + 1):
        ...
        qhr = m.get("quality_best_at_recall_target")
        cur_quality_high_recall = float(qhr["precision"]) if qhr is not None else -math.inf
        ...
        if cur_quality_high_recall > best_scores["quality_high_recall"]:
            best_scores["quality_high_recall"] = cur_quality_high_recall
            save_selected_checkpoint("quality_high_recall", ckpt_paths["quality_high_recall"], cur_quality_high_recall, epoch, train_loss, m, ckpt, export_errors=True)
        ...
        elif cfg.early_stop_metric == "quality_high_recall_precision":
            cur_metric = cur_quality_high_recall
            is_better = cur_metric > best_metric_for_stop + 1e-6
            if is_better:
                best_metric_for_stop = cur_metric
        else:
    ...
    print(f"best_quality_hr:  {ckpt_paths['quality_high_recall']}")
    ...
    # Cleanup preprocess cache after a successful training run.
    cleanup_preprocess_cache(ds, cfg)

    print("[AutoSelect] import this script and call:")
    print(f"  choose_checkpoint_from_index(r'{checkpoint_index_path}', purpose='quality_high_recall')")

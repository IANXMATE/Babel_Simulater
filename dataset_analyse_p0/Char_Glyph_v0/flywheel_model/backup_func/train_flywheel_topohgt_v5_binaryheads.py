# -*- coding: utf-8 -*-
r"""
train_flywheel_topohgt_v5_binaryheads.py

建议放置位置：
    dataset_analyse_p0/Char_Glyph_v0/flywheel_model/train_flywheel_topohgt_v5_binaryheads.py
...
class Cfg:
    ...
    rel_pos_weight_clip: float = 50.0
    ...
    num_workers: int = 0
    log_every: int = 5
    save_prefix: str = "flywheel_topohgt_v5_binaryheads"
...
def cache_signature(records, cfg):
    payload = {
        "script": "train_flywheel_topohgt_v5_binaryheads",
...
def binary_head_metrics_from_logits(logit, label, mask, th=0.5):
    ...
    lab = lab[m]
    if len(prob) == 0:
        return {"precision": 0.0, "recall": 0.0, "f1": 0.0, "accuracy": 0.0, "tp": 0, "fp": 0, "fn": 0, "tn": 0, "count": 0}
    ...
    f1 = 2 * prec * rec / max(1e-8, prec + rec)
    acc = (tp + tn) / max(1, tp + fp + fn + tn)
    return {"precision": prec, "recall": rec, "f1": f1, "accuracy": acc, "tp": tp, "fp": fp, "fn": fn, "tn": tn, "count": int(len(prob))}


def merge_binary_metric_counts(rows):
    ...
    f1 = 2 * prec * rec / max(1e-8, prec + rec)
    acc = (tp + tn) / max(1, tp + fp + fn + tn)
    return {"precision": prec, "recall": rec, "f1": f1, "accuracy": acc, "tp": tp, "fp": fp, "fn": fn, "tn": tn, "count": count}
...
def evaluate(model, loader, device, cfg, bce_quality, bce_rel):
    ...
    for batch in loader:
        ...
        e2e_metric_rows.append(binary_head_metrics_from_logits(out["e2e_logit"], batch["e2e_label"], batch["e2e_mask"], th=cfg.rel_eval_threshold))
        t_metric_rows.append(binary_head_metrics_from_logits(out["t_logit"], batch["t_label"], batch["t_mask"], th=cfg.rel_eval_threshold))
        x_metric_rows.append(binary_head_metrics_from_logits(out["x_logit"], batch["x_label"], batch["x_mask"], th=cfg.rel_eval_threshold))
    ...
    rel_m = {
        "E2E": merge_binary_metric_counts(e2e_metric_rows),
        "T": merge_binary_metric_counts(t_metric_rows),
        "X": merge_binary_metric_counts(x_metric_rows),
    }
    for name, mm in rel_m.items():
        m[f"rel_{name}_precision"] = mm["precision"]
        m[f"rel_{name}_recall"] = mm["recall"]
        m[f"rel_{name}_f1"] = mm["f1"]
        m[f"rel_{name}_tp"] = mm["tp"]
        m[f"rel_{name}_fp"] = mm["fp"]
        m[f"rel_{name}_fn"] = mm["fn"]
        m[f"rel_{name}_tn"] = mm["tn"]
        m[f"rel_{name}_count"] = mm["count"]
...
def print_metrics(epoch, train_loss, m, lr, cfg):
    ...
    print(
        f"  relation binary F1 E2E/T/X={m['rel_E2E_f1']:.3f}/"
        f"{m['rel_T_f1']:.3f}/{m['rel_X_f1']:.3f}"
    )
    print(
        f"  relation binary PR E2E={m['rel_E2E_precision']:.3f}/{m['rel_E2E_recall']:.3f} "
...
def train():
    ...
    print("=" * 88)
    print("[FlywheelTopoHGT-v5-binaryheads] start")
    ...
    model = TopoHGTv5(ds.node_dim, ds.edge_dim, cfg).to(device)
    params = sum(p.numel() for p in model.parameters())
    trainable = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(f"[Model] TopoHGTv5 node_dim={ds.node_dim} edge_dim={ds.edge_dim} params={params:,} trainable={trainable:,}")

# -*- coding: utf-8 -*-
r"""
train_flywheel_topohgt_v1.py

建议放置位置：
    dataset_analyse_p0/Char_Glyph_v0/flywheel_model/train_flywheel_topohgt_v1.py

用途：
    第一版 Flywheel / TopoHGT 风格拓扑质量确认模型训练脚本。

读取数据：
    1) dataset_analyse_p0/AI_VECTOR_ROUTER_With_topo/annotations_topo
       - 人工 topology 标注记录，默认作为 positive 样本。
    2) dataset_analyse_p0/Char_Glyph_v0/annotation_tool/pcg_filebacked_stage2_schema/bad
       - bad 样本，quality_label=0。
    3) dataset_analyse_p0/Char_Glyph_v0/annotation_tool/pcg_filebacked_stage2_schema/good
       - good 样本，quality_label=1。
    4) dataset_analyse_p0/Char_Glyph_v0/annotation_tool/pcg_filebacked_stage2_schema/cleaned
       - cleaned 样本，quality_label=1，并且与 good 重复时 cleaned 优先。

去重：
    使用 annotation_tool 里的 stable_uid 思路：
      candidate_id + style_mode + topology_family + rounded strokes geometry
    不依赖 outer key / hex_key / char。
    source priority:
      cleaned > good > annotations_topo > bad

模型：
    第一版只做 stroke-node graph，不引入 PyG，纯 PyTorch 实现：
      - 每条 Bézier stroke 一个 node
      - 每个 stroke pair 一条 dense pair edge feature
      - Edge-aware attention message passing
      - 输出：
          1. glyph quality good/bad score
          2. stroke-pair relation: NONE / E2E / T / X

说明：
    这是第一版可跑 baseline，用于验证：
      - 数据读取是否健康
      - good/bad/cleaned 分布是否合理
      - topology relation head 是否有学习信号
      - graph message passing 是否优于普通 pair classifier

输出：
    flywheel_topohgt_v1_latest.pth
    flywheel_topohgt_v1_best_loss.pth
    flywheel_topohgt_v1_best_quality_f1.pth
    flywheel_topohgt_v1_data_manifest.json
"""
...
class Cfg:
    ...
    num_workers: int = 0
    log_every: int = 5
    save_prefix: str = "flywheel_topohgt_v1"
...
def stroke_node_feature(s: Dict[str, Any], idx: int, n: int, cfg: Cfg, topo_deg=None, in_cycle=False) -> np.ndarray:
...
def pair_edge_feature(sa: Dict[str, Any], sb: Dict[str, Any], cfg: Cfg) -> np.ndarray:
    ...
    # curve-curve min distance: 小样本 enough，max_strokes 不大
...
def relation_labels_and_degrees(bundle: Dict[str, Any], max_strokes: int) -> Tuple[np.ndarray, Dict[int, Dict[str, int]], set, Dict[str, int]]:
            ...
            try:
                if typ == "E2E":
                    ...
                    if g_id in idmap and h_id in idmap:
                        ...
                        rel[g, h] = REL_MAP["T"]  # directed: guest -> host
                        deg[g]["t_guest"] += 1
                        deg[h]["t_host"] += 1
            except Exception:
                continue
    ...
    # counts for valid off-diagonal pairs
    for i in range(n):
        for j in range(n):
            if i == j:
                continue
...
def iter_bundles_from_json_file(fp: str):
    ...
    if isinstance(data, dict):
        # common format: outer_key -> bundle
        for k, v in data.items():
            if isinstance(v, dict) and isinstance(v.get("strokes"), list):
                yield str(k), v
            elif isinstance(v, dict):
                # tolerate one-layer nested, but don't recurse too aggressively
...
def load_source_records(cfg: Cfg, annotations_dir: str, pool_root: str) -> Tuple[List[Dict[str, Any]], Dict[str, Any]]:
    ...
    # Deduplicate by stable_uid, higher priority wins.
    chosen = {}
    all_by_uid = {}
    for r in raw_records:
        uid = r["uid"]
        all_by_uid.setdefault(uid, []).append(r)
...
class FlywheelTopoDataset(Dataset):
    ...
    def _make_item(self, idx):
        ...
        node_features = np.zeros((S, 0), dtype=np.float32)
        node_list = []
        for i, s in enumerate(strokes):
        ...
        for i in range(n):
            ...
            for j in range(n):
                if i == j:
                    # edge self feature is zeros but valid as self-attention support
                    row.append(np.zeros_like(pair_edge_feature(strokes[0], strokes[0], cfg)))
                else:
                    row.append(pair_edge_feature(strokes[i], strokes[j], cfg))
            edge_list.append(row)

        edge_features_valid = np.asarray(edge_list, dtype=np.float32)
        edge_dim = edge_features_valid.shape[-1]
        edge_features = np.zeros((S, S, edge_dim), dtype=np.float32)
        edge_features[:n, :n] = edge_features_valid
...
def split_dataset(ds: FlywheelTopoDataset, cfg: Cfg):
    ...
    if n < 10:
        ...
        return Subset(ds, idxs), Subset(ds, idxs), {"note": "too_few_samples", "train": n, "val": n}

    val_n = max(1, int(round(n * cfg.val_ratio)))

    # stratified by quality if possible
...
class EdgeAwareGraphBlock(nn.Module):
    ...
    def forward(self, h, edge, node_mask):
        # h: [B,S,H], edge: [B,S,S,E], node_mask: [B,S]
        B, S, H = h.shape
        hi = h.unsqueeze(2).expand(B, S, S, H)
        hj = h.unsqueeze(1).expand(B, S, S, H)
        score_in = torch.cat([hi, hj, edge], dim=-1)
        score = self.score_mlp(score_in).squeeze(-1)  # [B,S,S]

        # allow self-attention for stability; mask invalid j
        ...
        msg = self.msg_mlp(msg_in)  # [B,S,S,H]
...
class TopoHGTv1(nn.Module):
    ...
    def forward(self, batch):
        ...
        # quality head: masked mean + max + stroke_count
...
def binary_metrics(probs, labels, th=0.5):
    ...
    if len(pos_scores) > 0 and len(neg_scores) > 0:
        ...
        # crude tie handling ignored for first debug version
        rank_pos = ranks[:len(pos_scores)].sum()
        auc = (rank_pos - len(pos_scores) * (len(pos_scores) + 1) / 2) / (len(pos_scores) * len(neg_scores))
...
def evaluate(model, loader, device, cfg, bce, rel_ce):
    ...
    qm = binary_metrics(all_probs, all_labels, th=0.5)
    m.update({
        "quality_precision": qm["precision"],
        "quality_recall": qm["recall"],
        "quality_f1": qm["f1"],
        "quality_accuracy": qm["accuracy"],
        "quality_auc": qm["auc"],
        "quality_tp": qm["tp"],
        "quality_fp": qm["fp"],
        "quality_fn": qm["fn"],
...
def print_metrics(epoch, train_loss, m, lr):
    ...
    print(
        ...
        f"  quality P/R/F1/Acc/AUC={m['quality_precision']:.3f}/"
        f"{m['quality_recall']:.3f}/{m['quality_f1']:.3f}/"
        f"{m['quality_accuracy']:.3f}/{auc_str} "
        f"(tp={m['quality_tp']} fp={m['quality_fp']} fn={m['quality_fn']} tn={m['quality_tn']})\n"
...
def train():
    ...
    print("=" * 88)
    print("[FlywheelTopoHGT-v1] start")
    ...
    print(f"[SaveDir] {MODEL_SAVE_DIR}")
    ...
    model = TopoHGTv1(ds.node_dim, ds.edge_dim, cfg).to(device)
    params = sum(p.numel() for p in model.parameters())
    trainable = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(f"[Model] TopoHGTv1 node_dim={ds.node_dim} edge_dim={ds.edge_dim} params={params:,} trainable={trainable:,}")
    ...
    for epoch in range(1, cfg.epochs + 1):
        ...
        if do_eval:
            ...
            if m["quality_f1"] > best_f1:
                best_f1 = m["quality_f1"]
                torch.save(ckpt, best_f1_path)
                print(f"  ✅ best-quality-f1 saved: {best_f1_path} | f1={best_f1:.4f}")

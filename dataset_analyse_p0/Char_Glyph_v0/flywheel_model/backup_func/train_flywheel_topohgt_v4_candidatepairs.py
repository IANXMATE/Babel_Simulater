# -*- coding: utf-8 -*-
r"""
train_flywheel_topohgt_v4_candidatepairs.py

建议放置位置：
    dataset_analyse_p0/Char_Glyph_v0/flywheel_model/train_flywheel_topohgt_v4_candidatepairs.py
...
class Cfg:
    ...
    num_workers: int = 0
    log_every: int = 5
    save_prefix: str = "flywheel_topohgt_v4_candidatepairs"
...
def cache_signature(records, cfg):
    payload = {
        "script": "train_flywheel_topohgt_v4_candidatepairs",
...
def relation_candidate_mask_for_typed_nodes(metas: List[Dict[str, Any]], rel: np.ndarray, cfg: Cfg) -> Tuple[np.ndarray, Dict[str, int]]:
    """
    候选 pair mask：
      - 正例 E2E/T/X 永远保留；
      - NONE 只保留几何上可能相关的 hard negatives：
          endpoint-endpoint 近距离 -> E2E 候选
          endpoint -> stroke 近投影 -> T 候选
          stroke-stroke 近曲线 -> X 候选
    这样 relation head 不会被大量远距离 NONE 淹没。
    """
    N = len(metas)
    ...
    stats = {
        "candidate_pairs": 0,
        "positive_pairs": 0,
        "none_candidate_pairs": 0,
        "e2e_candidate_none": 0,
        "t_candidate_none": 0,
        "x_candidate_none": 0,
    }
    ...
    for i in range(N):
        for j in range(N):
            ...
            if cfg.cand_include_all_positive and lab != REL_MAP["NONE"]:
                mask[i, j] = True
                stats["positive_pairs"] += 1
                continue

            if not cfg.relation_candidate_only:
                mask[i, j] = True
                if lab == REL_MAP["NONE"]:
                    stats["none_candidate_pairs"] += 1
                continue

            is_candidate = False

            # endpoint <-> endpoint: E2E hard negative
            if mi["node_type"].startswith("endpoint") and mj["node_type"].startswith("endpoint"):
                if mi["stroke_idx"] != mj["stroke_idx"]:
                    d = float(np.linalg.norm(np.asarray(mi["pos"]) - np.asarray(mj["pos"])))
                    if d <= cfg.cand_e2e_endpoint_dist_max:
                        is_candidate = True
                        stats["e2e_candidate_none"] += 1

            # endpoint -> stroke: T hard negative, directed
            elif mi["node_type"].startswith("endpoint") and mj["node_type"] == "stroke":
                if mi["stroke_idx"] != mj["stroke_idx"]:
                    d, _ = point_to_curve_min_dist_and_t(mi["pos"], mj["stats"]["P"], sample_n=48)
                    if d <= cfg.cand_t_endpoint_curve_dist_max:
                        is_candidate = True
                        stats["t_candidate_none"] += 1

            # stroke <-> stroke: X hard negative
            elif mi["node_type"] == "stroke" and mj["node_type"] == "stroke":
                if mi["stroke_idx"] != mj["stroke_idx"]:
                    c1 = mi["stats"]["curve"][::2]
                    c2 = mj["stats"]["curve"][::2]
                    dd = np.linalg.norm(c1[:, None, :] - c2[None, :, :], axis=2)
                    if float(dd.min()) <= cfg.cand_x_curve_curve_dist_max:
                        is_candidate = True
                        stats["x_candidate_none"] += 1
    ...
    stats["candidate_pairs"] = int(mask.sum())
    return mask, stats
...
class FlywheelTopoDataset(Dataset):
    def __init__(self, records: List[Dict[str, Any]], cfg: Cfg):
        ...
        if not loaded_cache:
            ...
            if cfg.precompute_items:
                ...
                for i in range(len(self.records)):
                    ...
                    train_mask = it["rel_train_mask"].numpy().astype(bool)
                    raw_mask = rel != IGNORE
                    vals_raw = rel[raw_mask]
                    vals_train = rel[train_mask & raw_mask]
                    for k in range(4):
                        self.raw_rel_counts[k] += int((vals_raw == k).sum())
                        self.rel_counts[k] += int((vals_train == k).sum())
        ...
        print("[Dataset] rel_counts_candidate_for_loss:", {REL_INV[i]: int(self.rel_counts[i]) for i in range(4)})
    ...
    def _make_item(self, idx):
        ...
        rel_candidate_mask_raw, candidate_stats = relation_candidate_mask_for_typed_nodes(metas, rel_raw, cfg)

        rel_labels = np.full((max_nodes, max_nodes), IGNORE, dtype=np.int64)
        rel_labels[:n_nodes, :n_nodes] = rel_raw
        rel_train_mask = np.zeros((max_nodes, max_nodes), dtype=np.bool_)
        rel_train_mask[:n_nodes, :n_nodes] = rel_candidate_mask_raw

        for i in range(max_nodes):
            rel_labels[i, i] = IGNORE
            rel_train_mask[i, i] = False
        if n_nodes < max_nodes:
            rel_labels[n_nodes:, :] = IGNORE
            rel_labels[:, n_nodes:] = IGNORE
            rel_train_mask[n_nodes:, :] = False
            rel_train_mask[:, n_nodes:] = False
        ...
        return {
            ...
            "rel_train_mask": torch.tensor(rel_train_mask, dtype=torch.bool),
...
def compute_loss(out, batch, cfg: Cfg, bce, rel_ce):
    ...
    logits_flat = out["rel_logits"].reshape(-1, 4)
    labels_flat = batch["rel_labels"].reshape(-1)
    mask_flat = batch["rel_train_mask"].reshape(-1).bool() & (labels_flat != IGNORE)

    rel_loss_all = rel_ce(logits_flat, labels_flat)
    if bool(mask_flat.any().detach().cpu()):
        rel_loss = rel_loss_all[mask_flat].mean()
    else:
        rel_loss = out["rel_logits"].sum() * 0.0
...
def rel_confusion_from_logits(logits, labels, train_mask=None):
    pred = logits.argmax(dim=-1).detach().cpu().numpy().reshape(-1)
    lab = labels.detach().cpu().numpy().reshape(-1)
    mask = lab != IGNORE
    if train_mask is not None:
        tm = train_mask.detach().cpu().numpy().reshape(-1).astype(bool)
        mask = mask & tm
    pred = pred[mask]
    lab = lab[mask]
...
def evaluate(model, loader, device, cfg, bce, rel_ce):
    ...
    for batch in loader:
        ...
        rel_conf += rel_confusion_from_logits(out["rel_logits"], batch["rel_labels"], batch.get("rel_train_mask"))
...
def train():
    ...
    print("=" * 88)
    print("[FlywheelTopoHGT-v4-candidatepairs] start")
    ...
    model = TopoHGTv4(ds.node_dim, ds.edge_dim, cfg).to(device)
    params = sum(p.numel() for p in model.parameters())
    trainable = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(f"[Model] TopoHGTv4 node_dim={ds.node_dim} edge_dim={ds.edge_dim} params={params:,} trainable={trainable:,}")
    ...
    rel_ce = nn.CrossEntropyLoss(weight=rel_w, ignore_index=IGNORE, reduction="none")
    print(f"[Loss] quality_pos_weight={q_pos_weight_val:.4f}")
    print("[Loss] relation_weights(candidate-pair counts):", {REL_INV[i]: round(float(rel_w[i].detach().cpu()), 4) for i in range(4)})

# -*- coding: utf-8 -*-
r'''
morpheme_independent_rule_miner_v7_production_ready_grammar_rules.py

放置位置建议：
    dataset_analyse_p0/Char_Glyph_v0/Morpheme_Demo/morpheme_independent_rule_miner_v7_production_ready_grammar_rules.py

运行：
    cd dataset_analyse_p0/Char_Glyph_v0/Morpheme_Demo
    python morpheme_independent_rule_miner_v7_production_ready_grammar_rules.py
...
输出：
    - 控制台日志
    - Morpheme_Demo/rule_miner_v7_production_ready_outputs/independent_rule_miner_v7_production_ready_report.txt
    - Morpheme_Demo/rule_miner_v7_production_ready_outputs/independent_rule_miner_v7_production_ready_summary.json
...
OUTPUT_DIR = SCRIPT_DIR / 'rule_miner_v7_production_ready_outputs'
...
def mine_v4_strict_contextual_rules(good_rows, bad_rows, cleaned_rows, manual_rows, old_rules, min_good_hits=5, top_k=40):
    ...
    def add_candidate(combo, family):
        ...
        candidates.append({
            'score': score,
            'rule_name': name,
            'role': 'grammar_level_independent_rule',
            'rule_family': family,
            ...
            **ortho,
            'write_to_rule_cache': False,
            'review_note': 'V5 grammar prototype; convert accepted rule to typed-port graph grammar production after human review',
        })
...
# =============================================================================
# V5: grammar-level prototype mining
# =============================================================================
# V4 produced valid strict candidates, but the top list was still dominated by
# broad single-ingredient rules such as:
#   motif:proxy:orbit_or_seal_field + min_length + NOT_bad_x
# V5 promotes grammar-level candidates by requiring either:
#   1) two independent strict positive ingredients, or
#   2) one specific ingredient + strong structural context + quality context, or
#   3) one specific ingredient + bad-avoid context + quality context.
# It also canonicalizes bad-avoid and quality predicates so near-duplicate variants
# do not flood the report.

BROAD_INGREDIENT_HINTS_V5 = {
    'orbit_seal',
    'semantic_topology',
}

SPECIFIC_INGREDIENT_LABELS_V5 = {
    'nested_portal',
    'interlock_port',
...
def ingredient_specificity_v5(p):
    """0~1 specificity for grammar induction. Broad field predicates are useful but should not dominate."""
    label = ingredient_label_v4(p)
    n = str(p.name).lower()
    if label == 'ring_core_spine_port':
        return 1.00
    if label in {'interlock_port', 'nested_portal'}:
        return 0.92
    if label in {'relation_motif', 'enclosure_field', 'structure_field'}:
        return 0.86
    if label in {'spine_branch', 'role_port_motif'}:
        return 0.78
    if label == 'orbit_seal':
        # proxy:orbit_or_seal_field is broad; raw orbit_score is even broader.
        if 'proxy:orbit_or_seal_field' in n:
            return 0.55
        return 0.45
    if label == 'semantic_topology':
        return 0.50
    return 0.45


def context_label_v5(p):
    n = str(p.name).lower()
    f = str(p.feature).lower()
    if predicate_kind_v3(p) == 'bad_avoid_context':
        return 'avoid_bad_failure'
    if f in {'min_length', 'mean_length'}:
        return 'line_length'
    if f == 'connected_components':
        return 'connectivity'
    if f == 'fragmentation_risk':
        return 'fragmentation'
    if f in {'cycle_count'} or 'cycle' in n:
        return 'cycle_context'
    if f in {'x_count', 'x_ratio'} or '_x1' in n or '_x2' in n:
        return 'cross_context'
    if f in {'t_count', 't_ratio'} or '_t1' in n or '_t2' in n:
        return 't_context'
    if f in {'branch_ratio', 'max_degree', 'topology_event_count'}:
        return 'graph_complexity'
    if f in {'mean_curvature', 'curve_ratio'}:
        return 'curve_context'
    if f in {'axis_ratio', 'diagonal_ratio'} or n.startswith('motif:orient:'):
...
def is_strong_structural_context_v5(p):
    if is_strict_positive_ingredient_v4(p):
        return False
    if not is_structural_context_v4(p):
        return False
    label = context_label_v5(p)
    return label in {
        'cycle_context', 'cross_context', 't_context', 'graph_complexity',
        'curve_context', 'orientation_context', 'field_context'
    }


def grammar_signature_v5(combo, family):
    ing_labels = sorted({ingredient_label_v4(p) for p in combo if is_strict_positive_ingredient_v4(p)})
    # Use exact motif names only for specific motif predicates; broad quality thresholds are grouped.
    specific_ing_names = []
    for p in combo:
        if is_strict_positive_ingredient_v4(p):
            lbl = ingredient_label_v4(p)
            if lbl in SPECIFIC_INGREDIENT_LABELS_V5 or str(p.name).lower().startswith('motif:'):
                specific_ing_names.append(str(p.name))
            else:
                specific_ing_names.append(lbl)
    ctx_labels = sorted({context_label_v5(p) for p in combo if not is_strict_positive_ingredient_v4(p)})
    return (family, tuple(sorted(specific_ing_names)), tuple(ctx_labels))


def grammar_hint_v5(preds):
    labels = {ingredient_label_v4(p) for p in preds if is_strict_positive_ingredient_v4(p)}
    names = ' '.join(str(p.name).lower() for p in preds)
    if 'ring_core_spine_port' in labels:
        return 'grammar_production: RING_CORE_SPINE_PORT(shell, core, spine, side_ports); mutate geometry through typed ports'
    if 'orbit_seal' in labels and 'nested_portal' in labels:
        return 'grammar_production: NESTED_ORBIT_SEAL(outer_seal, inner_orbit, core_ports); bind orbit chords to shell ports'
    if 'orbit_seal' in labels and 'interlock_port' in labels:
        return 'grammar_production: ORBIT_INTERLOCK_SEAL(seal, interlock_ports); attach secondary strokes via safe ports'
    if 'interlock_port' in labels and ('relation_motif' in labels or 'role_port_motif' in labels):
        return 'grammar_production: INTERLOCKED_PORT_MOTIF(primary_component, relation_pattern); enforce single connected component'
    if 'spine_branch' in labels and ('enclosure_field' in labels or 'structure_field' in labels):
        return 'grammar_production: SPINE_ENCLOSURE_FIELD(axis_spine, enclosure, curve_residuals); attach residuals to typed ports'
    if 'nested_portal' in labels:
        return 'grammar_production: NESTED_PORTAL(core, shell_layers); add optional hook/chord and project physical constraints'
    if 'relation_motif' in labels:
        return 'grammar_production: RELATION_HIST_MOTIF(E2E/T/X/cycle bins); instantiate strokes then solve ports'
    return 'grammar_production: DISCOVERED_TOPOLOGY_MOTIF(typed_ports, optional_shell, curve_flow); stochastic nonmechanical composition'


def rule_non_triviality_v5(preds):
    strict_pos = [p for p in preds if is_strict_positive_ingredient_v4(p)]
    labels = {ingredient_label_v4(p) for p in strict_pos}
    specificity = max([ingredient_specificity_v5(p) for p in strict_pos] or [0.0])
    avg_spec = sum(ingredient_specificity_v5(p) for p in strict_pos) / max(1, len(strict_pos))
    has_quality = any(is_quality_context_v3(p) for p in preds)
    has_avoid = any(predicate_kind_v3(p) == 'bad_avoid_context' for p in preds)
    has_strong_struct = any(is_strong_structural_context_v5(p) for p in preds)
    has_motif = any(str(p.name).lower().startswith('motif:') for p in preds)
    score = 0.08 + 0.18 * min(1.0, len(strict_pos) / 2.0) + 0.22 * avg_spec
    if len(labels) >= 2:
        score += 0.18
    if has_motif:
        score += 0.10
    if has_strong_struct:
        score += 0.10
    if has_quality:
        score += 0.06
    if has_avoid:
        score += 0.06
    # Penalize broad single-ingredient filter-like candidates.
    if len(strict_pos) == 1 and specificity < 0.70 and not has_strong_struct:
        score -= 0.22
    return max(0.0, min(1.0, score))


def mine_v5_grammar_rule_prototypes(good_rows, bad_rows, cleaned_rows, manual_rows, old_rules, min_good_hits=5, top_k=40):
    all_rows = good_rows + bad_rows + cleaned_rows + manual_rows
    atoms = build_atomic_predicates(all_rows)
    atoms += make_bad_avoid_predicates()
    ...
    quality_atoms = [p for p in atoms if is_quality_context_v3(p)]
    strict_pos_atoms = [p for p in atoms if is_strict_positive_ingredient_v4(p)]
    strong_struct_atoms = [p for p in atoms if is_strong_structural_context_v5(p)]
    avoid_atoms = [p for p in atoms if predicate_kind_v3(p) == 'bad_avoid_context']
    bad_atoms = [p for p in atoms if is_bad_failure_ingredient_v3(p)]
    ...
    for p in strict_pos_atoms:
        ev = eval_rule_v3([p], good_rows, bad_rows, cleaned_rows, manual_rows)
        if ev['good_hits'] >= max(2, min_good_hits // 2):
            if ev['good_support'] + 0.08 < ev['bad_support'] and ev['cleaned_support'] < 0.22:
                continue
            row = score_strict_positive_ingredient_v4(p, good_rows, bad_rows, cleaned_rows, manual_rows, old_rules)
            row['specificity_v5'] = ingredient_specificity_v5(p)
            ingredient_scored.append(row)
    ingredient_scored.sort(key=lambda x: (x['score'], x.get('specificity_v5', 0)), reverse=True)

    structural_scored = []
    for p in strong_struct_atoms:
        ev = eval_rule_v3([p], good_rows, bad_rows, cleaned_rows, manual_rows)
        if ev['good_hits'] >= max(2, min_good_hits // 2):
            row = score_positive_ingredient_v3(p, good_rows, bad_rows, cleaned_rows, manual_rows, old_rules)
            row['role'] = 'strong_structural_context'
            row['kind'] = context_label_v5(p)
            structural_scored.append(row)
    structural_scored.sort(key=lambda x: x['score'], reverse=True)
    ...
    top_quality_names = {r['predicate'] for r in quality_scored[:45]}
    top_pos_names = {r['predicate'] for r in ingredient_scored[:60]}
    top_struct_names = {r['predicate'] for r in structural_scored[:45]}
    q_atoms = [p for p in quality_atoms if p.name in top_quality_names]
    pos_atoms = [p for p in strict_pos_atoms if p.name in top_pos_names]
    struct_atoms = [p for p in strong_struct_atoms if p.name in top_struct_names]
    avoid_atoms = avoid_atoms[:7]

    candidates = []
    best_by_sig = {}
    single_ingredient_filters = []

    def add_candidate(combo, family, allow_single_broad=False):
        non_bad_feats = [p.feature for p in combo if not str(p.feature).startswith('bad_subtype:')]
        if len(non_bad_feats) != len(set(non_bad_feats)):
            return
        strict_pos = [p for p in combo if is_strict_positive_ingredient_v4(p)]
        if not strict_pos:
            return
        labels = sorted({ingredient_label_v4(p) for p in strict_pos})
        specs = [ingredient_specificity_v5(p) for p in strict_pos]
        max_spec = max(specs or [0.0])
        has_quality = any(is_quality_context_v3(p) for p in combo)
        has_avoid = any(predicate_kind_v3(p) == 'bad_avoid_context' for p in combo)
        has_strong_struct = any(is_strong_structural_context_v5(p) for p in combo)
        # Core V5 gate: do not admit broad one-ingredient filter rules as grammar prototypes.
        grammar_like = (len(labels) >= 2) or (max_spec >= 0.78 and (has_strong_struct or has_avoid)) or (has_strong_struct and has_quality and max_spec >= 0.70)
        if not grammar_like and not allow_single_broad:
            return
        if allow_single_broad and len(labels) == 1 and max_spec < 0.70:
            family = 'single_broad_ingredient_filter_review_only'
        ev = eval_rule_v3(combo, good_rows, bad_rows, cleaned_rows, manual_rows)
        if ev['good_hits'] < min_good_hits or ev['good_support'] < 0.025:
            return
        contrast = ev['good_support'] - ev['bad_support']
        # V5 stricter contrast. A grammar rule should reduce Bad, not just preserve Good.
        if contrast < 0.045 and not (len(labels) >= 2 and ev['cleaned_support'] >= 0.25 and ev['precision_vs_bad_support'] >= 0.17):
            return
        ortho = old_rule_orthogonality_v3(combo, old_rules)
        nontriv = rule_non_triviality_v5(combo)
        gen = generality_score(ev['good_support'])
        div = diversity_bonus(combo)
        bad_penalty = min(0.25, 0.28 * ev['bad_support'])
        score = (
            0.30 * max(0.0, contrast) +
            0.18 * ev['precision_vs_bad_support'] +
            0.12 * gen +
            0.14 * ortho['orthogonality'] +
            0.16 * nontriv +
        ...
        row = {
            'score': score,
            'rule_name': ' AND '.join(p.name for p in combo),
            'role': 'grammar_level_independent_rule' if grammar_like else 'single_ingredient_filter_review_only',
            'rule_family': family,
            'ingredient_labels': '+'.join(labels),
            'ingredient_specificity': sum(specs) / max(1, len(specs)),
            'context_labels': '+'.join(sorted({context_label_v5(p) for p in combo if not is_strict_positive_ingredient_v4(p)})),
            'predicates': [{'name': p.name, 'feature': p.feature, 'op': p.op, 'value': p.value, 'kind': ('strict_positive_ingredient' if is_strict_positive_ingredient_v4(p) else predicate_kind_v3(p))} for p in combo],
            **ev,
            'bad_support_note': 'V5 penalizes high bad support; accepted candidates should explain a reusable grammar motif, not a quality filter.',
            'generality_score': gen,
            'non_triviality_score': nontriv,
            'diversity_bonus': div,
            'generation_hint': grammar_hint_v5(combo),
            **ortho,
            'write_to_rule_cache': False,
            'review_note': 'V5 grammar-level prototype; use for typed-port graph grammar after human review',
        }
        sig = grammar_signature_v5(combo, family)
        prev = best_by_sig.get(sig)
        if prev is None or row['score'] > prev['score']:
            best_by_sig[sig] = row

    # Highest priority: two strict ingredients + quality. This produces reusable grammar motifs.
    for i, p1 in enumerate(pos_atoms[:32]):
        for p2 in pos_atoms[i+1:32]:
            if ingredient_label_v4(p1) == ingredient_label_v4(p2) and ingredient_label_v4(p1) in BROAD_INGREDIENT_HINTS_V5:
                continue
            for q in q_atoms[:18]:
                add_candidate((p1, p2, q), 'two_strict_ingredients_plus_quality')
    # Two strict ingredients + bad avoid.
    for i, p1 in enumerate(pos_atoms[:28]):
        for p2 in pos_atoms[i+1:28]:
            if ingredient_label_v4(p1) == ingredient_label_v4(p2) and ingredient_label_v4(p1) in BROAD_INGREDIENT_HINTS_V5:
                continue
            for a in avoid_atoms[:6]:
                add_candidate((p1, p2, a), 'two_strict_ingredients_plus_bad_avoid')
    # One specific ingredient + strong structural context + quality.
    for p in pos_atoms[:40]:
        if ingredient_specificity_v5(p) < 0.70:
            continue
        for sctx in struct_atoms[:30]:
            for q in q_atoms[:16]:
                add_candidate((p, sctx, q), 'specific_ingredient_plus_structural_context_plus_quality')
    # One specific ingredient + quality + avoid. Use only for specific ingredients, not broad orbit-only.
    for p in pos_atoms[:36]:
        if ingredient_specificity_v5(p) < 0.78:
            continue
        for q in q_atoms[:18]:
            for a in avoid_atoms[:6]:
                add_candidate((p, q, a), 'specific_ingredient_plus_quality_plus_bad_avoid')
    # Keep broad orbit/seal filters separately for review, but do not mix them with grammar prototypes.
    for p in pos_atoms[:12]:
        if ingredient_specificity_v5(p) >= 0.70:
            continue
        for q in q_atoms[:8]:
            for a in avoid_atoms[:3]:
                add_candidate((p, q, a), 'single_broad_ingredient_filter_review_only', allow_single_broad=True)

    all_rows = sorted(best_by_sig.values(), key=lambda x: x['score'], reverse=True)
    grammar_rows = [r for r in all_rows if r['role'] == 'grammar_level_independent_rule']
    filter_rows = [r for r in all_rows if r['role'] != 'grammar_level_independent_rule']
    return {
        'quality_gate_candidates': quality_scored[:top_k],
        'positive_ingredient_candidates': ingredient_scored[:top_k],
        'structural_context_candidates': structural_scored[:top_k],
        'bad_failure_rules': failure_rows[:top_k],
        'contextual_independent_rules': grammar_rows[:top_k],
        'single_ingredient_filter_candidates': filter_rows[:top_k],
    }
...
# =============================================================================
# V6: alignment-stable grammar prototypes
# =============================================================================
# V5 successfully avoided trivial single-ingredient filters, but high-precision rare
# candidates could dominate even when they had zero cleaned/manual support. V6 keeps
# those as exploratory candidates and promotes rules that are also visible in cleaned
# or manual pools, or have broad enough Good coverage.

def _v6_alignment_tier(row):
    cs = float(row.get('cleaned_support', 0.0) or 0.0)
    ms = float(row.get('manual_support', 0.0) or 0.0)
    gs = float(row.get('good_support', 0.0) or 0.0)
    bs = float(row.get('bad_support', 0.0) or 0.0)
    precision = float(row.get('precision_vs_bad_support', 0.0) or 0.0)
    # Strongest: explicitly appears in cleaned or manual references.
    if cs >= 0.08 or ms >= 0.05:
        return 'production_aligned'
    # Weak but still useful alignment.
    if cs >= 0.03 or ms >= 0.02:
        return 'weak_aligned'
    # A broad Good rule may be useful even if manual/cleaned currently do not contain it.
    if gs >= 0.12 and (gs - bs) >= 0.06:
        return 'good_broad_experimental'
    # Good for exploration only; do not write to rule cache without visual review.
    if precision >= 0.30 and gs >= 0.04 and bs <= 0.06:
        return 'exploratory_precise'
    return 'exploratory_weak'


def _v6_alignment_bonus(row):
    cs = float(row.get('cleaned_support', 0.0) or 0.0)
    ms = float(row.get('manual_support', 0.0) or 0.0)
    gs = float(row.get('good_support', 0.0) or 0.0)
    bs = float(row.get('bad_support', 0.0) or 0.0)
    tier = _v6_alignment_tier(row)
    bonus = 0.00
    bonus += 0.30 * min(cs, 0.50)
    bonus += 0.12 * min(ms, 0.35)
    bonus += 0.06 * generality_score(gs)
    bonus += 0.08 * max(0.0, gs - bs)
    if tier == 'production_aligned':
        bonus += 0.10
    elif tier == 'weak_aligned':
        bonus += 0.055
    elif tier == 'good_broad_experimental':
        bonus += 0.025
    elif tier == 'exploratory_precise':
        bonus -= 0.055
    else:
        bonus -= 0.12
    if cs <= 0.0 and ms <= 0.0:
        bonus -= 0.10
    if gs < 0.055:
        bonus -= 0.045
    return bonus
...
def _v6_should_keep_main(row):
    tier = _v6_alignment_tier(row)
    return tier in {'production_aligned', 'weak_aligned', 'good_broad_experimental'}


def mine_v6_alignment_stable_grammar_rules(good_rows, bad_rows, cleaned_rows, manual_rows, old_rules, min_good_hits=5, top_k=40):
    base = mine_v5_grammar_rule_prototypes(
        good_rows, bad_rows, cleaned_rows, manual_rows, old_rules,
        min_good_hits=min_good_hits,
        top_k=max(top_k * 3, 120),
    )
    grammar = list(base.get('contextual_independent_rules', []))
    filters = list(base.get('single_ingredient_filter_candidates', []))

    rescored = []
    exploratory = []
    for row in grammar:
        r = dict(row)
        tier = _v6_alignment_tier(r)
        r['alignment_tier_v6'] = tier
        r['original_v5_score'] = r.get('score', 0.0)
        r['alignment_bonus_v6'] = _v6_alignment_bonus(r)
        r['score'] = float(r.get('score', 0.0) or 0.0) + r['alignment_bonus_v6']
        r['review_note'] = (
            'V6: production candidate; cleaned/manual or broad Good alignment supports promotion.'
            if _v6_should_keep_main(r)
            else 'V6: exploratory only; high precision but weak cleaned/manual alignment, do not write to rule cache yet.'
        )
        r['write_to_rule_cache'] = False
        if _v6_should_keep_main(r):
            rescored.append(r)
        else:
            r['role'] = 'exploratory_grammar_rule_review_only'
            exploratory.append(r)

    rescored.sort(key=lambda x: (x.get('score', 0.0), x.get('cleaned_support', 0.0), x.get('manual_support', 0.0), x.get('good_support', 0.0)), reverse=True)
    exploratory.sort(key=lambda x: (x.get('score', 0.0), x.get('precision_vs_bad_support', 0.0)), reverse=True)

    # If main list is too short, append a few clearly-marked exploratory rows so the report is still informative.
    main_rows = rescored[:top_k]
    if len(main_rows) < min(top_k, 20):
        main_rows = main_rows + exploratory[:max(0, min(top_k, 20) - len(main_rows))]

    # Demote broad single-ingredient filters further and keep them only for review.
    for r in filters:
        r['alignment_tier_v6'] = _v6_alignment_tier(r)
        r['role'] = 'single_ingredient_filter_review_only'
        r['score'] = float(r.get('score', 0.0) or 0.0) + _v6_alignment_bonus(r) - 0.12
        r['review_note'] = 'V6: broad ingredient filter; useful as generator bias, not as grammar rule.'
    filters.sort(key=lambda x: x.get('score', 0.0), reverse=True)

    base['contextual_independent_rules'] = main_rows
    base['exploratory_grammar_rule_candidates'] = exploratory[:top_k]
    base['single_ingredient_filter_candidates'] = filters[:top_k]
    return base


# =============================================================================
# V7: production-ready grammar prototypes
# =============================================================================
# V6 made alignment explicit, but one issue remained in the log: some rows could
# still contain no-op structural contexts such as T_count>=0. Those are true for
# nearly every sample and should never appear in a production grammar rule.
# V7 therefore:
#   1) removes no-op / always-on contexts from promoted rule prototypes,
#   2) ranks by production readiness rather than raw Good/Bad precision,
#   3) merges near-duplicate rule variants into production blueprints,
#   4) keeps exploratory PCG-only patterns separate from rule-cache candidates.

NOOP_CONTEXT_FEATURES_V7 = {
    't_count', 't_ratio', 'x_count', 'x_ratio', 'cycle_count',
    'topology_event_count', 'branch_ratio', 'leaf_ratio', 'max_degree',
    'spine_score', 'enclosure_score', 'interlock_score', 'orbit_score',
    'mean_curvature', 'curve_ratio', 'axis_ratio', 'diagonal_ratio',
    'horizontal_ratio', 'vertical_ratio', 'fragmentation_risk',
}


def _v7_float(x, default=None):
    try:
        return float(x)
    except Exception:
        return default
...
def _v7_pred_dicts(row):
    ps = row.get('predicates', [])
    return ps if isinstance(ps, list) else []


def _v7_pred_is_noop_context_dict(p):
    name = str(p.get('name', '')).lower()
    feature = str(p.get('feature', '')).lower()
    op = str(p.get('op', '')).lower()
    val = _v7_float(p.get('value'), None)
    kind = str(p.get('kind', '')).lower()
    # True-by-construction lower bounds. The log showed T_count>=0; guard the whole class.
    if op == '>=' and val is not None and val <= 1e-12 and feature in NOOP_CONTEXT_FEATURES_V7:
        return True
    for f in NOOP_CONTEXT_FEATURES_V7:
        if name.startswith(f + '>=0') or name.startswith(f + '>=0.0'):
            return True
    # between[0,max] with a very large upper bound often behaves like an absence of constraint.
    if '_between[0,' in name and feature in NOOP_CONTEXT_FEATURES_V7 and kind != 'strict_positive_ingredient':
        return True
    # <=1 on curve_ratio is always true in the current feature definition and should not be a context.
    if feature == 'curve_ratio' and op == '<=' and val is not None and val >= 0.999999:
        return True
    return False


def _v7_row_has_noop_context(row):
    return any(_v7_pred_is_noop_context_dict(p) for p in _v7_pred_dicts(row))


def _v7_ingredient_family_from_row(row):
    labels = str(row.get('ingredient_labels', '') or '')
    hint = str(row.get('generation_hint', '') or '').upper()
    rn = str(row.get('rule_name', '') or '').lower()
    if 'RING_CORE_SPINE_PORT' in hint or 'ring_core_spine_ports' in rn:
        return 'RING_CORE_SPINE_PORT'
    if 'NESTED_ORBIT_SEAL' in hint or ('orbit_seal' in labels and 'nested_portal' in labels):
        return 'NESTED_ORBIT_SEAL'
    if 'ORBIT_INTERLOCK_SEAL' in hint or ('orbit_seal' in labels and 'interlock_port' in labels):
        return 'ORBIT_INTERLOCK_SEAL'
    if 'INTERLOCKED_PORT_MOTIF' in hint or 'interlock_port' in labels:
        return 'INTERLOCKED_PORT_MOTIF'
    if 'SPINE_ENCLOSURE_FIELD' in hint or ('spine_branch' in labels and 'enclosure_field' in labels):
        return 'SPINE_ENCLOSURE_FIELD'
    if 'NESTED_PORTAL' in hint or 'nested_portal' in labels:
        return 'NESTED_PORTAL'
    if 'RELATION_HIST_MOTIF' in hint or 'relation_motif' in labels:
        return 'RELATION_HIST_MOTIF'
    return 'DISCOVERED_TOPOLOGY_MOTIF'


def _v7_context_blueprint(row):
    # Collapse threshold variants into stable blueprint slots.
    ctx = set()
    for p in _v7_pred_dicts(row):
        if str(p.get('kind', '')) == 'strict_positive_ingredient':
            continue
        name = str(p.get('name', '')).lower()
        feature = str(p.get('feature', '')).lower()
        if feature == 'connected_components' or 'connected_components' in name:
            ctx.add('connectivity')
        elif feature in {'min_length', 'mean_length'}:
            ctx.add('line_length')
        elif feature == 'stroke_count':
            ctx.add('stroke_capacity')
        elif feature == 'cycle_count' or 'cycle' in name:
            ctx.add('cycle_context')
        elif feature in {'x_count', 'x_ratio'} or '_x1' in name or '_x2' in name:
            ctx.add('cross_context')
        elif feature in {'t_count', 't_ratio'} or '_t1' in name or '_t2' in name:
            ctx.add('t_context')
        elif feature in {'max_degree', 'branch_ratio', 'topology_event_count'}:
            ctx.add('graph_complexity')
        elif feature.startswith('bad_subtype:') or name.startswith('not_bad_'):
            ctx.add('bad_avoid')
        else:
            ctx.add(feature or name[:24])
    return '+'.join(sorted(ctx))


def _v7_blueprint_signature(row):
    return (
        _v7_ingredient_family_from_row(row),
        str(row.get('ingredient_labels', '')),
        _v7_context_blueprint(row),
    )


def _v7_production_tier(row):
    cs = float(row.get('cleaned_support', 0.0) or 0.0)
    ms = float(row.get('manual_support', 0.0) or 0.0)
    gs = float(row.get('good_support', 0.0) or 0.0)
    bs = float(row.get('bad_support', 0.0) or 0.0)
    nontriv = float(row.get('non_triviality_score', 0.0) or 0.0)
    precision = float(row.get('precision_vs_bad_support', 0.0) or 0.0)
    contrast = gs - bs
    if cs >= 0.15 and gs >= 0.08 and bs <= 0.18 and nontriv >= 0.60:
        return 'production_ready'
    if (cs >= 0.08 or ms >= 0.05) and gs >= 0.05 and bs <= 0.24:
        return 'production_aligned'
    if gs >= 0.12 and contrast >= 0.055 and cs >= 0.03:
        return 'good_broad_aligned'
    if precision >= 0.30 and gs >= 0.04 and bs <= 0.07:
        return 'exploratory_precise'
    return 'exploratory_review_only'


def _v7_tier_bonus(tier):
    return {
        'production_ready': 0.18,
        'production_aligned': 0.11,
        'good_broad_aligned': 0.06,
        'exploratory_precise': -0.05,
        'exploratory_review_only': -0.14,
    }.get(tier, -0.10)


def _v7_production_hint(row):
    fam = _v7_ingredient_family_from_row(row)
    if fam == 'NESTED_ORBIT_SEAL':
        return 'production_blueprint: NESTED_ORBIT_SEAL(outer_shell, inner_cycle, core_ports); constraints: line_length + stroke_capacity + physical_projector'
    if fam == 'RING_CORE_SPINE_PORT':
        return 'production_blueprint: RING_CORE_SPINE_PORT(shell, core_spine, side_ports); constraints: single_component + safe_port_attachment'
    if fam == 'ORBIT_INTERLOCK_SEAL':
        return 'production_blueprint: ORBIT_INTERLOCK_SEAL(seal_field, interlock_ports); constraints: avoid_floating + avoid_short_fragment'
    if fam == 'INTERLOCKED_PORT_MOTIF':
        return 'production_blueprint: INTERLOCKED_PORT_MOTIF(primary_graph, relation_histogram); constraints: connected_components<=1/2 + port_solver'
    if fam == 'SPINE_ENCLOSURE_FIELD':
        return 'production_blueprint: SPINE_ENCLOSURE_FIELD(axis_spine, enclosure_layer, curve_residuals); constraints: readability_alignment + typed_ports'
    if fam == 'NESTED_PORTAL':
        return 'production_blueprint: NESTED_PORTAL(core, shell_layers); constraints: cycle_context + length_gate + avoid_low_context_cycle'
    if fam == 'RELATION_HIST_MOTIF':
        return 'production_blueprint: RELATION_HIST_MOTIF(E/T/X/C bins, typed_ports); constraints: instantiate topology first, then geometry projection'
    return 'production_blueprint: DISCOVERED_TOPOLOGY_MOTIF(typed_ports, structural_context); constraints: review visually before rule-cache promotion'


def mine_v7_production_ready_grammar_rules(good_rows, bad_rows, cleaned_rows, manual_rows, old_rules, min_good_hits=5, top_k=40):
    base = mine_v5_grammar_rule_prototypes(
        good_rows, bad_rows, cleaned_rows, manual_rows, old_rules,
        min_good_hits=min_good_hits,
        top_k=max(top_k * 5, 180),
    )
    ...
    for row in grammar:
        r = dict(row)
        if _v7_row_has_noop_context(r):
            r['role'] = 'dropped_noop_context_rule'
            r['production_tier_v7'] = 'dropped_noop_context'
            r['review_note'] = 'V7 dropped: contains no-op context such as T_count>=0 / curve_ratio<=1.'
            dropped_noop.append(r)
            continue

        tier = _v7_production_tier(r)
        r['production_tier_v7'] = tier
        r['production_family_v7'] = _v7_ingredient_family_from_row(r)
        r['context_blueprint_v7'] = _v7_context_blueprint(r)
        r['blueprint_signature_v7'] = '|'.join(_v7_blueprint_signature(r))
        r['original_v5_score'] = r.get('score', 0.0)
        r['production_bonus_v7'] = _v7_tier_bonus(tier)
        r['score'] = float(r.get('score', 0.0) or 0.0) + r['production_bonus_v7']
        r['generation_hint'] = _v7_production_hint(r)
        r['write_to_rule_cache'] = False
        r['review_note'] = (
            'V7: production-ready blueprint candidate; still review visually before writing to new_rule_cache.'
            if tier in {'production_ready', 'production_aligned', 'good_broad_aligned'}
            else 'V7: exploratory only; keep for PCG exploration, not rule-cache promotion.'
        )
        if tier in {'production_ready', 'production_aligned', 'good_broad_aligned'}:
            sig = _v7_blueprint_signature(r)
            prev = promoted_by_sig.get(sig)
            if prev is None or r['score'] > prev['score']:
                promoted_by_sig[sig] = r
        else:
            r['role'] = 'exploratory_grammar_rule_review_only'
            exploratory.append(r)

    promoted = sorted(promoted_by_sig.values(), key=lambda x: (
        x.get('score', 0.0), x.get('cleaned_support', 0.0), x.get('manual_support', 0.0), x.get('good_support', 0.0)
    ), reverse=True)
    exploratory.sort(key=lambda x: (x.get('score', 0.0), x.get('precision_vs_bad_support', 0.0)), reverse=True)

    # If promoted list is too short, show a few exploratory rows, but keep their role/tier explicit.
    main_rows = promoted[:top_k]
    if len(main_rows) < min(top_k, 20):
        main_rows = main_rows + exploratory[:max(0, min(top_k, 20) - len(main_rows))]

    # Re-score filters, but keep them separate.
    for r in filters:
        r['role'] = 'single_ingredient_filter_review_only'
        r['production_tier_v7'] = _v7_production_tier(r)
        r['score'] = float(r.get('score', 0.0) or 0.0) + _v7_tier_bonus(r['production_tier_v7']) - 0.16
        r['review_note'] = 'V7: broad ingredient/generator bias only, not a grammar-level independent rule.'
    filters.sort(key=lambda x: x.get('score', 0.0), reverse=True)

    base['contextual_independent_rules'] = main_rows
    base['exploratory_grammar_rule_candidates'] = exploratory[:top_k]
    base['single_ingredient_filter_candidates'] = filters[:top_k]
    base['dropped_noop_context_rules'] = dropped_noop[:top_k]
    return base

# Preserve the existing main/report code while swapping in the V7 production-ready miner.
mine_v3_contextual_rules = mine_v7_production_ready_grammar_rules
...
def main():
    parser = argparse.ArgumentParser(description='V7 production-ready grammar-level independent topology rule miner: separates quality gates, positive ingredients, bad failure modes, and contextual rules.')
    ...
    t0 = time.time()
    rep = Reporter()
    rep.section('Morpheme Independent Rule Miner V7 Production-Ready Grammar Rule Prototypes')
    ...
    rep.section('07 V7 production-ready grammar rule mining: gates / ingredients / failure modes / production prototypes')
    ...
    exploratory_rules = v3.get('exploratory_grammar_rule_candidates', [])
    dropped_noop_rules = v3.get('dropped_noop_context_rules', [])
    ...
    rep.write('  - grammar-level rules suppress no-op contexts such as T_count>=0, merge near-duplicate signatures, and rank production-ready blueprints first.')
    ...
    rep.subsection('07D Production-ready grammar rule prototypes: no-op contexts suppressed, production blueprints first')
    print_table(rep, contextual_rules, [
        'score', 'rule_name', 'role', 'production_tier_v7', 'production_family_v7', 'context_blueprint_v7',
        'good_support', 'bad_support', 'cleaned_support', 'manual_support',
        'precision_vs_bad_support', 'generality_score', 'non_triviality_score', 'orthogonality',
        'nearest_old_rule_by_token', 'max_old_token_jaccard', 'generation_hint'
    ], max_rows=args.top_k)

    rep.subsection('07E Dropped no-op context rules: should not become grammar rules')
    print_table(rep, dropped_noop_rules, [
        'score', 'rule_name', 'role', 'production_tier_v7', 'good_support', 'bad_support', 'cleaned_support', 'manual_support', 'review_note'
    ], max_rows=min(args.top_k, 30))

    rep.subsection('07F Exploratory grammar rules: PCG-only / weak-aligned, keep for exploration')
    print_table(rep, exploratory_rules, [
        'score', 'rule_name', 'role', 'production_tier_v7', 'production_family_v7',
        'good_support', 'bad_support', 'cleaned_support', 'manual_support', 'precision_vs_bad_support', 'review_note'
    ], max_rows=min(args.top_k, 30))
    ...
    summary = {
        ...
        'exploratory_grammar_rule_candidates': exploratory_rules if 'exploratory_rules' in locals() else [],
        'dropped_noop_context_rules': dropped_noop_rules if 'dropped_noop_rules' in locals() else [],
        'runtime_seconds': time.time() - t0,
    }

    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    txt_path = OUTPUT_DIR / 'independent_rule_miner_v7_production_ready_report.txt'
    json_path = OUTPUT_DIR / 'independent_rule_miner_v7_production_ready_summary.json'
    ...
    write_csv(OUTPUT_DIR / 'production_ready_grammar_rule_prototypes.csv', contextual_rules if 'contextual_rules' in locals() else [])
    write_csv(OUTPUT_DIR / 'exploratory_grammar_rule_candidates.csv', exploratory_rules if 'exploratory_rules' in locals() else [])
    write_csv(OUTPUT_DIR / 'dropped_noop_context_rules.csv', dropped_noop_rules if 'dropped_noop_rules' in locals() else [])
    if getattr(args, 'emit_rule_json', False):
        emit_review_rule_json_v3(contextual_rules if 'contextual_rules' in locals() else [], OUTPUT_DIR / 'candidate_independent_rules_v7_review_only.json')
    flat_motif_rows = []
    ...
    rep.write(f"Wrote production-ready grammar rule prototypes csv: {OUTPUT_DIR / 'production_ready_grammar_rule_prototypes.csv'}")
    rep.write(f"Wrote exploratory grammar rules csv: {OUTPUT_DIR / 'exploratory_grammar_rule_candidates.csv'}")
    rep.write(f"Wrote dropped no-op rules csv: {OUTPUT_DIR / 'dropped_noop_context_rules.csv'}")
    if getattr(args, 'emit_rule_json', False):
        rep.write(f"Wrote review rule json: {OUTPUT_DIR / 'candidate_independent_rules_v7_review_only.json'}")

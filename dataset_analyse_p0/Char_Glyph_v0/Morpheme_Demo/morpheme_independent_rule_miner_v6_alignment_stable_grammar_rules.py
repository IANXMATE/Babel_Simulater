# -*- coding: utf-8 -*-
r'''
morpheme_independent_rule_miner_v6_alignment_stable_grammar_rules.py

放置位置建议：
    dataset_analyse_p0/Char_Glyph_v0/Morpheme_Demo/morpheme_independent_rule_miner_v6_alignment_stable_grammar_rules.py

运行：
    cd dataset_analyse_p0/Char_Glyph_v0/Morpheme_Demo
    python morpheme_independent_rule_miner_v6_alignment_stable_grammar_rules.py
...
输出：
    - 控制台日志
    - Morpheme_Demo/rule_miner_v6_alignment_stable_outputs/independent_rule_miner_v6_alignment_stable_report.txt
    - Morpheme_Demo/rule_miner_v6_alignment_stable_outputs/independent_rule_miner_v6_alignment_stable_summary.json
...
OUTPUT_DIR = SCRIPT_DIR / 'rule_miner_v6_alignment_stable_outputs'
...
# Preserve the existing main/report code while swapping in the V5 grammar-level miner.
mine_v3_contextual_rules = mine_v6_alignment_stable_grammar_rules
...
def main():
    parser = argparse.ArgumentParser(description='V6 alignment-stable grammar-level independent topology rule miner: separates quality gates, positive ingredients, bad failure modes, and contextual rules.')
    ...
    t0 = time.time()
    rep = Reporter()
    rep.section('Morpheme Independent Rule Miner V6 Alignment-Stable Grammar Rule Prototypes')
    ...
    rep.section('07 V6 alignment-stable grammar rule mining: gates / ingredients / failure modes / stable prototypes')
    ...
    rep.write('  - grammar-level rules are split by alignment tier: production candidates need cleaned/manual alignment; rare precise PCG-only motifs are kept as exploratory, not promoted.')
    ...
    rep.subsection('07D Alignment-stable grammar rule prototypes: production candidates first, exploratory rules demoted')
    ...
    txt_path = OUTPUT_DIR / 'independent_rule_miner_v6_alignment_stable_report.txt'
    json_path = OUTPUT_DIR / 'independent_rule_miner_v6_alignment_stable_summary.json'
    ...
    write_csv(OUTPUT_DIR / 'alignment_stable_grammar_rule_prototypes.csv', contextual_rules if 'contextual_rules' in locals() else [])
    if getattr(args, 'emit_rule_json', False):
        emit_review_rule_json_v3(contextual_rules if 'contextual_rules' in locals() else [], OUTPUT_DIR / 'candidate_independent_rules_v6_review_only.json')
    flat_motif_rows = []
    ...
    rep.write(f"Wrote alignment-stable grammar rule prototypes csv: {OUTPUT_DIR / 'alignment_stable_grammar_rule_prototypes.csv'}")
    if getattr(args, 'emit_rule_json', False):
        rep.write(f"Wrote review rule json: {OUTPUT_DIR / 'candidate_independent_rules_v6_review_only.json'}")

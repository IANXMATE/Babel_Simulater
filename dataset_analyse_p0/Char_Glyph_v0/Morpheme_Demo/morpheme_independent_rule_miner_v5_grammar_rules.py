# -*- coding: utf-8 -*-
r'''
morpheme_independent_rule_miner_v5_grammar_rules.py

放置位置建议：
    dataset_analyse_p0/Char_Glyph_v0/Morpheme_Demo/morpheme_independent_rule_miner_v5_grammar_rules.py

运行：
    cd dataset_analyse_p0/Char_Glyph_v0/Morpheme_Demo
    python morpheme_independent_rule_miner_v5_grammar_rules.py
...
输出：
    - 控制台日志
    - Morpheme_Demo/rule_miner_v5_grammar_rules_outputs/independent_rule_miner_v5_grammar_rules_report.txt
    - Morpheme_Demo/rule_miner_v5_grammar_rules_outputs/independent_rule_miner_v5_grammar_rules_summary.json
...
OUTPUT_DIR = SCRIPT_DIR / 'rule_miner_v5_grammar_rules_outputs'
...
mine_v3_contextual_rules = mine_v5_grammar_rule_prototypes
...
def main():
    parser = argparse.ArgumentParser(description='V5 grammar-level independent topology rule miner: separates quality gates, positive ingredients, bad failure modes, and contextual rules.')
    ...
    t0 = time.time()
    rep = Reporter()
    rep.section('Morpheme Independent Rule Miner V5 Grammar Rule Prototypes')
    ...
    rep.section('07 V5 grammar-level rule mining: gates / ingredients / failure modes / rule prototypes')
    ...
    rep.write('  - grammar-level independent rules require two ingredients or a specific ingredient plus structural context; old rules are used only for de-dup/orthogonality.')
    ...
    rep.subsection('07D Grammar-level independent rule prototypes: review-first, possible new rule cache entries')
    ...
    txt_path = OUTPUT_DIR / 'independent_rule_miner_v5_grammar_rules_report.txt'
    json_path = OUTPUT_DIR / 'independent_rule_miner_v5_grammar_rules_summary.json'
    ...
    write_csv(OUTPUT_DIR / 'grammar_rule_prototypes.csv', contextual_rules if 'contextual_rules' in locals() else [])
    if getattr(args, 'emit_rule_json', False):
        emit_review_rule_json_v3(contextual_rules if 'contextual_rules' in locals() else [], OUTPUT_DIR / 'candidate_independent_rules_v5_review_only.json')
    flat_motif_rows = []
    ...
    rep.write(f"Wrote grammar rule prototypes csv: {OUTPUT_DIR / 'grammar_rule_prototypes.csv'}")
    if getattr(args, 'emit_rule_json', False):
        rep.write(f"Wrote review rule json: {OUTPUT_DIR / 'candidate_independent_rules_v5_review_only.json'}")

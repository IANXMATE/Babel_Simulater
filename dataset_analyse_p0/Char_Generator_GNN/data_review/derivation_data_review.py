...
# 🌟 修复：指向新的“拓扑先行”数据集
DATASET_FILE = os.path.abspath(os.path.join(SCRIPT_DIR, "../fontgpt_dataset_topology_first.json"))
...
class InspectionWidget(QWidget):
    ...
    def init_ui(self):
        ...
        self.vbox.addWidget(self.create_section_title("🚀 Row 2: 【条件约束先验】1D Token 序列生成流"))
    ...
    def parse_token_junctions(self, sequence):
        # 🌟 核心升级：利用 pending_junctions 悬停池，将约束映射到下一笔
        junctions = []
        stroke_count = 0
        pending_junctions = []

        for item in sequence:
            t_type = item.get("token_type")
            if t_type == "NEW_ROOT":
                pass 
            elif t_type == "JUNCTION":
                # 遇到条件令牌，先不画，存入缓存池
                j_type = item.get("j_type", "Unknown")
                target_dist = item.get("target_dist", 1)
                ta = item.get("t_self_bin", 0) / 32.0 if "t_self_bin" in item else item.get("t_self", 0.0)
                tb = item.get("t_target_bin", 0) / 32.0 if "t_target_bin" in item else item.get("t_target", 0.0)
                ...
                # 遇到实体笔画，将缓存池的约束全部倾泻在这笔身上
                idx_u = stroke_count # self: 当前正要画的笔画
                for pj in pending_junctions:
                    idx_v = stroke_count - pj["target_dist"] # target: 历史目标
                    junctions.append({
                        "u": idx_u,
                        "v": idx_v,
                        "ta": pj["ta"],
                ...
                pending_junctions.clear() # 倾泻完毕，清空池子
                stroke_count += 1
        return junctions
    ...
    def on_derivation_selected(self, item):
        ...
        info.append("<div style='margin-bottom:5px;'><b>Topology-First Sequence 生成流:</b></div>")
        
        stroke_idx = 0
        for i, token in enumerate(self.current_seq):
            t_type = token.get("token_type")
            if t_type == "NEW_ROOT":
                info.append(f"<div style='color:#757575; font-weight:bold; margin-top: 8px;'>[{i+1}] 🌿 [NEW_ROOT] (宣告新起笔，无拓扑依赖)</div>")
            elif t_type == "JUNCTION":
                info.append(f"<div style='margin-left:20px; color:#E91E63; font-weight:bold;'>↳ 🔗 [JUNCTION] 声明挂载条件: {token.get('j_type')} (挂载到往回数第 {token.get('target_dist')} 笔, t_self={token.get('t_self_bin', 0)/32.0:.2f}, t_target={token.get('t_target_bin', 0)/32.0:.2f})</div>")
            elif t_type == "STROKE":
                info.append(f"<div style='color:{get_hex_color(stroke_idx)}; font-weight:bold;'>[{i+1}] ✏️ 生成 Stroke {stroke_idx} 实体坐标 (Shape:{token.get('shape_code')}, Morph:{token.get('variant_id', 0)})</div>")
                stroke_idx += 1
    ...
    def on_junction_changed(self):
        ...
        if not j_data:
            ...
            def _span(eid): return f"<span style='color:{get_hex_color(eid)}; font-weight:bold;'>S{eid}</span>"
            html = [f"<b style='font-size:18px; color:#D32F2F;'>微观条件生成验证</b><hr>"]
            html.append(f"<div style='font-size:15px;'><b>生成实体 (Self)：</b> {_span(u)}</div>")
            html.append(f"<div style='font-size:15px;'><b>挂载目标 (Target)：</b> {_span(v)}</div>")
            html.append(f"<div style='font-size:15px; margin-top:10px;'><b>先验条件指令类型：</b> {j_data['type']}</div>")
            html.append(f"<div style='font-size:15px; margin-top:10px;'><b>解析还原施力点：</b></div>")
            html.append(f"<ul><li>{_span(u)} (Self): t = <b style='color:#E91E63'>{j_data['ta']:.3f}</b></li>")
            html.append(f"<li>{_span(v)} (Target): t = <b style='color:#E91E63'>{j_data['tb']:.3f}</b></li></ul>")
            html.append(f"<div style='margin-top:10px; color:#555;'>* 红色十字 ❌ 代表新生成的 {u} 号笔画，根据先验指令精准咬合到了历史 {v} 号笔画的绝对坐标点上。</div>")

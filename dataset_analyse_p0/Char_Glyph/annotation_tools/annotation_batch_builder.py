# -*- coding: utf-8 -*-
"""
annotation_batch_builder.py

PCG glyph 数据飞轮人工标注工具：
1. 从当前 PCG 生成结果随机抽取 N*N 个 glyph candidate，显示为缩略图网格。
2. 每个缩略图有勾选框：勾选 = 好样本；不勾选 = 坏样本。
3. 提供 Good / Bad 样本池管理界面，可预览并从池中剔除。
4. 预览默认使用“纯黑 + 带宽度渲染”的人类审美判断视图。
5. 保存为与 AI_VECTOR_ROUTER_With_topo/annotations_topo 中人工 topo 标注相同的 JSON 内容格式：
   {
      "U+XXXXX": {
...
6. 为避免 GitHub 100MB 单文件限制，按 80MB 智能分文件保存。
7. 预留 aesthetic / relation model hook，用于后续数据飞轮的模型前置筛选。

放置位置建议：
    Char_Glyph/annotation_tools/annotation_batch_builder.py

因为本文件位于 Char_Glyph 文件夹下的子文件夹中，所以默认通过 ../ 回到 Char_Glyph 目录读取：
    ../gnn_scored_solved_glyphs.json
    ../solved_glyph_candidates.json

并通过：
    ../../AI_VECTOR_ROUTER_With_topo/annotations_topo
保存兼容人工 topo 标注格式的 JSON。
"""
...
# 私用 Unicode 区域，用于模拟 glyph_info.hex_key / char
...
class AestheticHooks:
    ...
    未来可以在这里接入：
    1. aesthetic_critic.pt
    2. stroke_relation_model.pt
    3. primitive_compatibility_model.pt
    ...
    def sample_weight(self, candidate):
        for key in ["combined_score", "gnn_layout_realness_score", "gnn_layout_score"]:
            if key in candidate:
                try:
...
def get_candidate_list_from_scored(data):
    ...
    for k in ["scored_items_by_combined", "scored_items_by_gnn", "scored_items_sorted"]:
        if k in data and isinstance(data[k], list) and len(data[k]) > 0:
            return data[k]
...
def load_candidates():
    ...
    if len(candidates) == 0:
        solved_data = load_json(SOLVED_FILE, required=True, default={})
...
def candidate_is_solver_allowed(candidate):
    ...
    status = candidate.get("quality_status", "")
    return str(status) in ["good", "usable_but_rough", "usable"]
...
def prefilter_candidates(candidates, mode=DEFAULT_PREFILTER_MODE):
    ...
    for c in candidates:
        ...
        if mode == "all":
            ...
            out.append(c)
    return out if len(out) > 0 else candidates
...
def read_layout_prior(node):
    ...
    return {"center_norm": center, "rotation_rad": theta, "length_norm": length}


def polyline_from_node(node):
    for k in [
        "solved_polyline_px", "polyline_px", "world_polyline_px", "transformed_polyline_px",
        "render_polyline_px", "points_px", "polyline", "points",
    ]:
        if k in node:
...
def extract_width_bezier(node):
    ...
    for key in ["width_px", "stroke_width_px", "render_width_px", "width_mean_px", "actual_width_px", "target_width_px"]:
        if key in node:
        ...
        for k in ["mother_bezier", "solved_mother_bezier", "bezier", "control_points"]:
            if k in node:
                ...
                if arr.ndim == 2 and arr.shape[0] == 4 and arr.shape[1] >= 2:
                    ...
                    break
        if mb is None:
            mb = bezier_from_polyline(polyline_from_node(node))
...
def topology_events_from_candidate(candidate, strokes, id_to_bezier_id):
    ...
    for edge in edges:
        ...
        pos = 0.5 * (point_on_bezier(p1, t_u) + point_on_bezier(p2, t_v))
...
def cycles_from_events(strokes, events):
    ...
    for members in valid_cycle_lists:
        ...
        cycles.append({"cycle_id": len(cycles), "members": [int(x) for x in members], "orientation": orient})
    return cycles
...
def candidate_to_char_bundle(candidate, label):
    ...
    edit_history = [{
        ...
        "label": label,
        "candidate_id": cid,
        "source": "annotation_batch_builder",
    ...
    return hex_key, {
        "glyph_info": {"hex_key": hex_key, "char": char},
        "strokes": strokes,
...
def save_annotation_outputs(labels, candidates_by_id):
    ...
    bad_dict = {}
    manifest = {
        "schema_version": "pcg_annotation_manifest_v1",
        "good_count": 0,
        "bad_count": 0,
        "items": {},
    ...
    for cid, label in labels.items():
        if cid not in candidates_by_id or label not in ["good", "bad"]:
            continue
    ...
    manifest["good_topo_files"] = write_split_topo_files(GOOD_FONT_PREFIX, good_dict)
    manifest["bad_topo_files"] = write_split_topo_files(BAD_FONT_PREFIX, bad_dict)
...
class CandidateTile(QWidget):
    def __init__(self, candidate, checked=False, parent=None):
        ...
        self.setStyleSheet("QWidget { background-color: #f6f6f6; border-radius: 6px; }")
...
class PoolManagerDialog(QDialog):
    ...
    def refresh(self):
        ...
        for idx, cid in enumerate(ids):
            ...
            img = QLabel()
            img.setPixmap(render_candidate_pixmap(cand, THUMB_SIZE))
...
# 🪟 Main Window
# =========================================================
class AnnotationBatchBuilder(QMainWindow):
    def __init__(self):
        super().__init__()
        ensure_dir(FLYWHEEL_DIR)
        ensure_dir(TOPO_OUT_DIR)

        self.setWindowTitle("PCG Glyph Annotation Batch Builder")
    ...
    def init_ui(self):
        ...
        self.btn_generate = QPushButton("🎲 Generate N×N Random Grid")
        self.btn_generate.clicked.connect(self.generate_batch)
        ctrl.addWidget(self.btn_generate)

        self.btn_commit = QPushButton("💾 Commit Current Grid Labels")
        ...
        help_text = QLabel(
            "勾选 = 好样本；未勾选 = 坏样本。点击缩略图也可切换勾选。"
            "保存后会在 AI_VECTOR_ROUTER_With_topo/annotations_topo 中生成 PCG_AestheticGood/Bad 分文件 topo JSON。"
        )
    ...
    def commit_current_batch(self):
        ...
        QMessageBox.information(self, "Committed", f"当前网格已提交：good={good_count}, bad={bad_count}\n已保存 topo split JSON。")
        self.generate_batch()

    def save_all_outputs(self, show_message=True):
        state = {
            "schema_version": "pcg_annotation_state_v1",
            "labels": self.labels,
...
def main():
    print("=" * 80)
    print("🚀 PCG Annotation Batch Builder")
    print("=" * 80)

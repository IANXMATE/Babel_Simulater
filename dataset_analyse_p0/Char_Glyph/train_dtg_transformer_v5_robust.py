...
默认输出：
    dtg_transformer_v5_robust.pt
    dtg_transformer_v5_robust_train_report.json
    dtg_transformer_v5_robust_val_predictions.json
"""
...
OUTPUT_BEST_MODEL_FILE = os.path.join(SCRIPT_DIR, "dtg_transformer_v5_robust_best.pt")
OUTPUT_FINAL_MODEL_FILE = os.path.join(SCRIPT_DIR, "dtg_transformer_v5_robust_final.pt")
# 兼容旧 inference：训练结束后会把 best 额外复制到 dtg_transformer_v5_robust.pt。
OUTPUT_MODEL_FILE = os.path.join(SCRIPT_DIR, "dtg_transformer_v5_robust.pt")
# 兼容现有 inference 脚本：训练完成后可选复制 best 到 dtg_transformer_v4_best.pt。
WRITE_COMPAT_V4_BEST = True
OUTPUT_COMPAT_V4_BEST_FILE = os.path.join(SCRIPT_DIR, "dtg_transformer_v4_best.pt")
OUTPUT_REPORT_FILE = os.path.join(SCRIPT_DIR, "dtg_transformer_v5_robust_train_report.json")
OUTPUT_VAL_PRED_FILE = os.path.join(SCRIPT_DIR, "dtg_transformer_v5_robust_val_predictions.json")
OUTPUT_CANONICAL_DATA_FILE = os.path.join(SCRIPT_DIR, "dtg_transformer_v5_robust_canonical_dataset_report.json")
...
MAX_CONTROL_DELTA = 0.70  # normalized canvas，允许网络修正更大几何偏移

# v5 robust：
# 推理日志显示 generated candidates 的 prior_maxJ mean ≈ 64px，
# 所以训练 corruption 需要覆盖 20~100px 的大错位，而不是 v4 的小扰动。
AUGMENT_PER_GLYPH = 32

# 默认值作为 medium corruption，实际训练会用下面的 mixture level 随机覆盖。
NOISE_CENTER_STD = 0.060
NOISE_ROTATION_STD_DEG = 16.0
NOISE_SCALE_STD = 0.120
NOISE_CONTROL_STD = 0.040
NOISE_EDGE_T_STD = 0.025
GLOBAL_TRANSLATE_STD = 0.040
GLOBAL_ROTATE_STD_DEG = 8.0
GLOBAL_SCALE_STD = 0.070

ROBUST_CORRUPTION = True
# mild / medium / strong，目标是让 val_prior_maxJ 接近 inference 的 30~80px 区间。
ROBUST_LEVEL_PROBS = [0.15, 0.40, 0.45]
ROBUST_LEVELS = {
    "mild": {
        "center": 0.025, "rot_deg": 8.0,  "scale": 0.055, "ctrl": 0.018,
        "g_trans": 0.018, "g_rot_deg": 4.0,  "g_scale": 0.040,
        "edge_t": 0.010,
    },
    "medium": {
        "center": 0.070, "rot_deg": 18.0, "scale": 0.130, "ctrl": 0.045,
        "g_trans": 0.045, "g_rot_deg": 9.0,  "g_scale": 0.080,
        "edge_t": 0.025,
    },
    "strong": {
        "center": 0.125, "rot_deg": 30.0, "scale": 0.220, "ctrl": 0.075,
        "g_trans": 0.080, "g_rot_deg": 16.0, "g_scale": 0.140,
        "edge_t": 0.045,
    },
}

# Loss weights：v5 目标是从 generated-candidate-style 大错位恢复。
# 降低 prior/supervised 对原始输入的黏性，增强 topology anchor。
LAMBDA_SUPERVISED = 1.2
LAMBDA_CURVE = 0.8
LAMBDA_JUNCTION = 260.0
LAMBDA_ANGLE = 1.0
LAMBDA_PRIOR = 0.004
LAMBDA_SMOOTH = 0.035
LAMBDA_CANVAS = 0.25
LAMBDA_OVERLAP = 0.02
LAMBDA_JUNCTION_MAX = 85.0
# v5 仍使用 junction-anchor decoder。
LAMBDA_ANCHOR = 180.0
...
def load_annotation_glyphs():
    ...
    save_json({
        "schema_version": "dtg_transformer_v5_robust_canonical_dataset_report",
        "stats": dict(stats),
...
# 5. Dataset
# =========================================================
def choose_robust_level():
    if not ROBUST_CORRUPTION:
        return {
            "center": NOISE_CENTER_STD,
            "rot_deg": NOISE_ROTATION_STD_DEG,
            "scale": NOISE_SCALE_STD,
            "ctrl": NOISE_CONTROL_STD,
            "g_trans": GLOBAL_TRANSLATE_STD,
            "g_rot_deg": GLOBAL_ROTATE_STD_DEG,
            "g_scale": GLOBAL_SCALE_STD,
            "edge_t": NOISE_EDGE_T_STD,
            "name": "default",
        }

    names = ["mild", "medium", "strong"]
    idx = int(np.random.choice(len(names), p=np.asarray(ROBUST_LEVEL_PROBS, dtype=np.float64)))
    level = dict(ROBUST_LEVELS[names[idx]])
    level["name"] = names[idx]
    return level


def corrupt_bezier(P, level=None):
    P = np.asarray(P, dtype=np.float32).reshape(4, 2)
    if level is None:
        level = choose_robust_level()

    center_noise = np.random.normal(0.0, level["center"], size=(2,)).astype(np.float32)
    rot = math.radians(np.random.normal(0.0, level["rot_deg"]))
    scale = float(np.exp(np.random.normal(0.0, level["scale"])))

    out = transform_bezier_np(P, center_noise, rot, scale)
    out += np.random.normal(0.0, level["ctrl"], size=out.shape).astype(np.float32)

    # 额外模拟 generated candidate 的 endpoint misalignment：
    # 对 P0/P3 加稍强扰动，但不完全破坏曲线。
    endpoint_jitter = np.random.normal(0.0, level["ctrl"] * 1.25, size=(2, 2)).astype(np.float32)
    out[0] += endpoint_jitter[0]
    out[3] += endpoint_jitter[1]

    out = np.clip(out, -0.35, 1.35)
    return out.astype(np.float32)


def apply_global_corruption(Ps, level=None):
    Ps = np.asarray(Ps, dtype=np.float32).copy()
    if level is None:
        level = choose_robust_level()

    g_trans = np.random.normal(0.0, level["g_trans"], size=(2,)).astype(np.float32)
    g_rot = math.radians(np.random.normal(0.0, level["g_rot_deg"]))
    g_scale = float(np.exp(np.random.normal(0.0, level["g_scale"])))
    ...
    return np.clip(Y.reshape(Ps.shape), -0.40, 1.40).astype(np.float32)
...
class DTGGlyphDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        robust_level_name = "unknown"
        if self.split_name == "train":
            level = choose_robust_level()
            robust_level_name = level.get("name", "unknown")
            prior_arr = np.stack([corrupt_bezier(P, level=level) for P in gt_arr], axis=0)
            prior_arr = apply_global_corruption(prior_arr, level=level)
        else:
            # v5：验证集使用固定随机种子 + robust corruption，
            # 直接测试模型能否从 candidate-style 大错位恢复。
        ...
        for k in range(E):
            ...
            edge_t_noise = float(level.get("edge_t", NOISE_EDGE_T_STD)) if "level" in locals() else NOISE_EDGE_T_STD
            if self.split_name == "train" and edge_t_noise > 0:
                tu = clamp(tu + np.random.normal(0.0, edge_t_noise), 0.0, 1.0)
                tv = clamp(tv + np.random.normal(0.0, edge_t_noise), 0.0, 1.0)
        ...
        return {
            ...
            "meta": {
                ...
                "robust_level": robust_level_name,
            },
        }
...
def compute_losses(model, batch, device):
    ...
    prior_mean_j, prior_max_j, _ = junction_metrics(P_prior, edge_index, edge_t, edge_mask)
    return {
        "total": total,
        ...
        "prior_mean_j_px": prior_mean_j.detach(),
        "prior_max_j_px": prior_max_j.detach(),
        "P_pred": P_pred,
    }
...
def evaluate(model, loader, device, max_batches=None):
    ...
    for bi, batch in enumerate(loader):
        ...
        prior_max_j = losses["prior_max_j_px"].detach().cpu().numpy()
        all_prior_max_j.extend(prior_max_j.tolist())
    ...
    prior_arr = np.asarray(all_prior_max_j, dtype=np.float32)
    out["prior_mean_max_j_px"] = float(np.mean(prior_arr))
    out["prior_p50_max_j_px"] = float(np.percentile(prior_arr, 50))
    out["prior_p90_max_j_px"] = float(np.percentile(prior_arr, 90))
    out["prior_max_j_px"] = float(np.max(prior_arr))
...
def save_checkpoint(model, optimizer, epoch, train_metrics, val_metrics, glyph_stats, output_path):
    ckpt = {
        "schema_version": "dtg_transformer_v5_robust",
        "epoch": epoch,
        ...
        "config": {
            ...
            "ROBUST_CORRUPTION": ROBUST_CORRUPTION,
            "ROBUST_LEVEL_PROBS": ROBUST_LEVEL_PROBS,
            "ROBUST_LEVELS": ROBUST_LEVELS,
            "ROBUST_CORRUPTION": ROBUST_CORRUPTION,
            "ROBUST_LEVEL_PROBS": ROBUST_LEVEL_PROBS,
            "ROBUST_LEVELS": ROBUST_LEVELS,
...
def main():
    ...
    print("\n" + "=" * 80)
    print("DTG-Transformer v5 Robust Training")
    ...
    print(f"  ROBUST_CORRUPTION: {ROBUST_CORRUPTION}")
    print(f"  ROBUST_LEVEL_PROBS: {ROBUST_LEVEL_PROBS}")
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if epoch == 1 or epoch % PRINT_EVERY_EPOCH == 0 or epoch == EPOCHS:
            ...
            print(
                ...
                f"train_priorMaxJ={train_metrics.get('prior_mean_max_j_px', 0):.3f}px -> "
                f"train_predMaxJ={train_metrics['mean_max_j_px']:.3f}px | "
                f"val_total={val_metrics.get('total', 0):.6f} | "
                f"val_priorMaxJ={val_metrics.get('prior_mean_max_j_px', 0):.3f}px -> "
                f"val_predMaxJ={val_metrics.get('mean_max_j_px', 0):.3f}px | "
    ...
    # 兼容 inference：dtg_transformer_v5_robust.pt 始终指向 best，而不是 final。
    if os.path.exists(OUTPUT_BEST_MODEL_FILE):
        import shutil
        shutil.copyfile(OUTPUT_BEST_MODEL_FILE, OUTPUT_MODEL_FILE)
        if WRITE_COMPAT_V4_BEST:
            shutil.copyfile(OUTPUT_BEST_MODEL_FILE, OUTPUT_COMPAT_V4_BEST_FILE)
    export_val_predictions(model, val_loader, device, max_items=32)

    report = {
        "schema_version": "dtg_transformer_v5_robust_train_report",
        "method": "DTG-Transformer v5 Robust / Candidate-distribution robust neural geometry solver",
        "claim": "robustly replace post-processing constraint solver under generated-candidate-style large topology errors",
        ...
        "outputs": {
            ...
            "compat_v4_best_model_for_existing_inference": OUTPUT_COMPAT_V4_BEST_FILE,
    ...
    print(f"  saved_compat_v4_best_for_inference: {OUTPUT_COMPAT_V4_BEST_FILE}")

...
输出：
    ...
    dtg_transformer_v4_inference_report.json
...
OUTPUT_REPORT_FILE = os.path.join(SCRIPT_DIR, "dtg_transformer_v4_inference_report.json")
...
def build_solved_candidate(candidate, graph, P_pred, aux_pred, idx=0):
    ...
    eval_info = evaluate_pred_geometry(P_pred, graph)
    max_j = eval_info["max_junction_px"]
    mean_j = eval_info["mean_junction_px"]
    ...
    for i, node in enumerate(graph["nodes"]):
        ...
        P_norm = P_pred[i].astype(np.float32)
    ...
    out["dtg_solver_debug"] = {
        "candidate_id": cid,
        "solver_type": "dtg_transformer_v4_junction_anchor",
        "num_nodes": int(N),
        "num_edges": int(graph["E"]),
        "prior_mean_junction_px": float(prior_eval_info["mean_junction_px"]),
        "prior_max_junction_px": float(prior_eval_info["max_junction_px"]),
        "prior_mean_junction_px": float(prior_eval_info["mean_junction_px"]),
        "prior_max_junction_px": float(prior_eval_info["max_junction_px"]),
...
def _apply_checkpoint_config(ckpt):
    ...
    ANCHOR_PROJECT_STRENGTH = float(cfg.get("ANCHOR_PROJECT_STRENGTH", ANCHOR_PROJECT_STRENGTH))
    ANCHOR_PRIOR_BLEND = float(cfg.get("ANCHOR_PRIOR_BLEND", ANCHOR_PRIOR_BLEND))
    ANCHOR_MAX_DELTA = float(cfg.get("ANCHOR_MAX_DELTA", ANCHOR_MAX_DELTA))
...
def load_model(device):
    ...
    """
    candidates = [BEST_MODEL_FILE, MODEL_FILE, FINAL_MODEL_FILE]
    model_path = None
    for p in candidates:
        if os.path.exists(p):
            model_path = p
...
def main():
    ...
    print("\n" + "=" * 80)
    print("DTG-Transformer v4 Inference AutoConfig + Topology Canonicalization")
    ...
    output_obj = {
        "schema_version": "solved_glyph_candidates_dtg_transformer_v4_inference",
        "solver_type": "dtg_transformer_v4_junction_anchor",
    ...
    report_obj = {
        "schema_version": "dtg_transformer_v4_inference_report",
        "solver_type": "dtg_transformer_v4_junction_anchor",
        "summary": summary,
        "model_info": model_info,

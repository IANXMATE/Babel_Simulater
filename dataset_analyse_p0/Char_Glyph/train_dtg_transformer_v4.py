...
def save_checkpoint(model, optimizer, epoch, train_metrics, val_metrics, glyph_stats):
    ckpt = {
    ...
    torch.save(ckpt, OUTPUT_MODEL_FILE)
...
def main():
    ...
    print("\n" + "=" * 80)
    print("DTG-Transformer v4 Training")
    print("End-to-End Neural Geometry Solver with Topology Canonicalization")
    print("=" * 80)
    print(f"  annotation_dir: {ANNOTATION_DIR}")
    print(f"  output_model:   {OUTPUT_MODEL_FILE}")
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if epoch == 1 or epoch % PRINT_EVERY_EPOCH == 0 or epoch == EPOCHS:
            ...
            if val_score < best_val:
                best_val = val_score
                save_checkpoint(model, optimizer, epoch, train_metrics, val_metrics, glyph_stats)
                print(f"    saved best model: val_mean_maxJ={best_val:.4f}px")

        if epoch % SAVE_EVERY_EPOCH == 0:
            val_metrics_tmp = evaluate(model, val_loader, device, max_batches=3)
            save_checkpoint(model, optimizer, epoch, train_metrics, val_metrics_tmp, glyph_stats)

    final_val = evaluate(model, val_loader, device)
    save_checkpoint(model, optimizer, EPOCHS, train_metrics, final_val, glyph_stats)
    ...
    print(f"  saved_model: {OUTPUT_MODEL_FILE}")

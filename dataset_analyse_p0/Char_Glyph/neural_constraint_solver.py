...
def projection_refine(graph, warm_centers, warm_thetas, warm_scales, device):
    N = int(graph["mask"].sum())
    edges = graph["edges"]; id_to_idx = graph["id_to_idx"]; nodes = graph["nodes"]
    ...
    opt = torch.optim.Adam([center, theta, log_scale], lr=PROJECTION_LR)
    last_loss = 0.0
    for _ in range(PROJECTION_ITERS):
        ...
        for e in edges:
            ...
            if iu >= N or iv >= N:
                continue
            tu = torch.tensor(clamp(safe_float(e.get("t_u", 0.0), 0.0), 0.0, 1.0), dtype=torch.float32, device=device)
            tv = torch.tensor(clamp(safe_float(e.get("t_v", 0.0), 0.0), 0.0, 1.0), dtype=torch.float32, device=device)
            li = interp_polyline_torch(locals_t[iu], tu); lj = interp_polyline_torch(locals_t[iv], tv)
            pi = transform_point_torch(li, center[iu], theta[iu], scale[iu]); pj = transform_point_torch(lj, center[iv], theta[iv], scale[iv])
            junction_terms.append(((pi - pj) ** 2).sum())
            if str(e.get("j_type", "UNKNOWN")) in ["T", "X"]:
                ti = tangent_polyline_torch(locals_t[iu], tu); tj = tangent_polyline_torch(locals_t[iv], tv)
                wi = transform_tangent_torch(ti, theta[iu]); wj = transform_tangent_torch(tj, theta[iv])
        ...
        prior_loss = ((center - prior_center_t) ** 2).mean() * (CANVAS_SIZE ** 2) + ((theta - prior_theta_t) ** 2).mean() * 8.0 + ((log_scale - prior_log_scale_t) ** 2).mean() * 8.0
        warm_loss = ((center - warm_center_t) ** 2).mean() * (CANVAS_SIZE ** 2) + ((theta - warm_theta_t) ** 2).mean() * 8.0 + ((log_scale - warm_log_scale_t) ** 2).mean() * 8.0
        scale_loss = (F.relu(log_scale - math.log(1.5)) ** 2 + F.relu(math.log(0.02) - log_scale) ** 2).mean()
        loss = junction_loss + PROJECTION_ANGLE_WEIGHT * angle_loss + PROJECTION_PRIOR_WEIGHT * prior_loss + PROJECTION_WARMSTART_WEIGHT * warm_loss + PROJECTION_SCALE_WEIGHT * scale_loss
        loss.backward(); torch.nn.utils.clip_grad_norm_([center, theta, log_scale], GRAD_CLIP_NORM); opt.step()
        with torch.no_grad():
            center.clamp_(0.02, 0.98); log_scale.clamp_(math.log(0.02), math.log(1.5))
    ...
    return out_centers, out_thetas, out_scales, {"projection_loss": last_loss}
...
def solve_candidate_neural(candidate, model, device, idx=0):
    ...
    if USE_PROJECTION:
        centers, thetas, scales, proj_info = projection_refine(g, warm_centers, warm_thetas, warm_scales, device)
    else:
        centers, thetas, scales = warm_centers, warm_thetas, warm_scales
        proj_info = {"projection_loss": 0.0}
    final_eval = evaluate_geometry(g["edges"], g["id_to_idx"], locals_, centers, thetas, scales)
    ...
    out["quality_report"] = {"quality_status": status, "quality_score": quality_score, "mean_junction_px": float(meanJ), "max_junction_px": float(maxJ), "mean_tx_angle_deg": float(meanA), "max_tx_angle_deg": float(maxA), "max_angle_diff_deg": float(maxA), "solver_type": "neural_warmstart_projection", "projection_iters": int(PROJECTION_ITERS), "hard_topology": bool(hard_topo)}
...
def main():
    ...
    for idx, cand in enumerate(candidates):
        ...
        if (idx + 1) % PRINT_EVERY == 0 or idx == 0 or idx + 1 == len(candidates):
            ...
            print(f"  progress {idx+1}/{len(candidates)} | {cps:.2f} cand/s | last={get_candidate_id(cand, idx)} N={dbg.get('num_nodes')} E={dbg.get('num_edges')} hard={dbg.get('hard_topology')} beforeMaxJ={dbg.get('before_max_junction_px'):.3f}px warmMaxJ={dbg.get('warm_max_junction_px'):.3f}px finalMaxJ={dbg.get('final_max_junction_px'):.3f}px maxTXA={dbg.get('final_max_tx_angle_deg'):.3f}° status={st}")
    elapsed = time.time() - t0

...
# ==========================================
CANVAS_SIZE = 400         # 🌟 UPGRADED: Much larger canvas to prevent clipping!
MAX_BEZIER_ERROR = 2  
MAX_SPUR_LENGTH = 20.0    # Adjusted to match the new larger canvas scale
...
# ==========================================
# 🖥️ Modern Annotation Application
# ==========================================
class ModernAnnotationApp(QMainWindow):
    def __init__(self, font_path):
        super().__init__()
        self.font_path = font_path
        self.setWindowTitle("AI Vector Router - The Ultimate Edition")
        # 🌟 UPGRADED: Expanded Window Size!
    ...
    def init_ui(self):
        ...
        # 🌟 UPGRADED: Adjusted Figsize for bigger visual display
    ...
    def action_add_dot(self):
        """🌟 NEW FEATURE: Add missing isolated points safely!"""
        if not self.last_click_coord:
            QMessageBox.warning(self, "Warning", "Please click on the canvas first to set a location for the missing dot.")
            return
        ...
        # 🌟 Entity Check: Probe a 5x5 neighborhood around the click for any real ink
        y_min, y_max = max(0, y-2), min(CANVAS_SIZE, y+3)
        x_min, x_max = max(0, x-2), min(CANVAS_SIZE, x+3)
        neighborhood = self.binary[y_min:y_max, x_min:x_max]

        if not np.any(neighborhood):
            QMessageBox.critical(self, "Action Failed", "Clicked outside the original character's ink area! Cannot add an invalid dot here.")
            return

        self.save_state()

        # Generate a micro-vertical path (3 pixels long) to represent the dot.
        # The DT-map width regressor will absorb the huge radius at this location automatically!
        ...
        self.last_click_coord = None
        self.update_canvas()
        print(f"📍 Successfully spawned a missing dot at ({x}, {y}).")
    ...
    def action_next_char(self):
        ...
        if not valid_char_found:
            QMessageBox.critical(self, "Fatal Error", "No valid rendering chars found in this font!")
            return

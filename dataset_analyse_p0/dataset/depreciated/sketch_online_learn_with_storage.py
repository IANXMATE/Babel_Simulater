...
def prune_spurs(G, max_length=MAX_SPUR_LENGTH, dist_threshold=5.0):
    changed = True
    while changed:
        changed = False
...
def stitch_paths(paths):
    ...
    while remaining:
        ...
        for i, p in enumerate(remaining):
            ...
            dists = [
                (np.linalg.norm(s_tail - p_head), 0), (np.linalg.norm(s_tail - p_tail), 1), 
                (np.linalg.norm(s_head - p_head), 2), (np.linalg.norm(s_head - p_tail), 3)  
            ]
...
def render_unicode_glyph(font_path, char):
    """🌟 修复：两步动态自适应缩放 (Two-pass Auto Scaling)"""
    # ================= 第一步：探针测试 =================
    # 先用一个标准的探测字号 (比如 100) 来加载，获取该字符真实的物理宽高比例
    test_size = 100

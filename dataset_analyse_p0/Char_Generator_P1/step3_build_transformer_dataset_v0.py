...
OUTPUT_FILE = os.path.join(SCRIPT_DIR, "fontgpt_dataset_topology_first.json") # 🌟 改名以示区分
...
# ==========================================
# 🚀 主程序：组装大一统图序列 (Topology-First 模式)
# ==========================================
def main():
        ...
        for hex_key, char_data in char_data_map.items():
            ...
            for rule_name, stroke_seq, derived_events in derived_samples:
                ...
                for ds in stroke_seq:
                    ...
                    # ==========================================
                    # 🌟 核心颠覆：先行探查即将发生的拓扑约束 (Conditioning)
                    # ==========================================
                    ...
                    for ev_idx, ev in enumerate(derived_events):
                        ...
                        # 如果这个事件牵扯到当前即将画的笔画 (bid)，且另一个参与者已经存在于历史 (drawn_bids)
                        if (b_a == bid and b_b in drawn_bids) or (b_b == bid and b_a in drawn_bids):
                            if b_a == bid:
                                t_self, t_target, target_bid = ta_val, tb_val, b_b
                            else:
                                t_self, t_target, target_bid = tb_val, ta_val, b_a
                                
                            # 相对距离：1 代表连到上一笔，2 代表连到上上笔
                    ...
                    # 1️⃣ 写入先验条件 (Conditioning Tokens)
                    if not events_for_this_stroke:
                        # 没有任何挂载对象，宣告新起笔
                        flat_token_sequence.append({"token_type": "NEW_ROOT"})
                    else:
                        # 存在挂载对象，将其作为约束发给大模型
                        for j_info in events_for_this_stroke:
                            flat_token_sequence.append({
                                ...
                                "t_self_bin": quantize_t(j_info["t_self"]),
                                "t_target_bin": quantize_t(j_info["t_target"])
                            })
                            emitted_events.add(j_info["ev_idx"])
                    
                    # 2️⃣ 在已知拓扑条件后，写入笔画的物理坐标
                    flat_token_sequence.append({
                        "token_type": "STROKE",
                        "bezier_id": bid,
    ...
    print(f"\n🎉 成功落盘！共组装 {len(final_dataset)} 个「条件约束先验」序列。")
